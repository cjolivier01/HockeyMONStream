...
This repo contains a DeepStream-style C++ app (`pipeline-app`) plus custom GStreamer plugins for dual-camera stitching, rink masking, and play tracking. It is the performance-oriented counterpart to the Python implementation in `../hm` (HockeyMOM) and reuses `hm` configs and (optionally) its Python CLI helpers for configuration steps.
...
3. Prepare a game directory:
   ...
   - Expected camera layout matches `../hm/hmlib/orientation.py`:
     - `${game_dir}/cam1/*.mp4`
     - `${game_dir}/cam2/*.mp4`
...
- `pretrained/` is a symlink by default (often to a large mounted volume); the script can `sudo`-create/chown the needed subdirectory when required.
...
Config root (HockeyMOM baseline):
- Preferred: a sibling checkout at `../hm/hmlib/config/baseline.yaml`
- Override explicitly via `HM_CONFIG_ROOT=/path/to/hm/hmlib/config`

## Repo Layout

- `src/apps/pipeline-app`: main DeepStream-style app
