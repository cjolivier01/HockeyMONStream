...
show_help() {
  cat <<'EOF'
Usage:
  ./run.sh --game-id=<game_id> [wrapper options] [hstream-cli options]
  ./hm_run.sh --game-id=<game_id> [wrapper options] [hstream-cli options]
...
Common:
  ...
  -c, --config FILE                    Additional hstream-cli config file
...
Notes:
  ...
  Unrecognized arguments are forwarded to hstream-cli.
EOF
}
...
if [ ! -x "${HSTREAM_CLI_BIN}" ]; then
  echo "hstream-cli is not built at ${HSTREAM_CLI_BIN}; build //src/apps/hstream-cli:hstream-cli first"
  exit 2
fi

"${HSTREAM_CLI_BIN}" "${pipeline_args[@]}"
