...
if [ ! -x "${PIPELINE_APP_BIN}" ]; then
  echo "hstream-cli is not built at ${PIPELINE_APP_BIN}; build //src/apps/pipeline-app:hstream-cli first"
  exit 2
fi

"${PIPELINE_APP_BIN}" "${pipeline_args[@]}"
# bazel-bin/src/apps/pipeline-app/hstream-cli -c configs/ds_hockey_app_config.yaml --enable-sources=URI-MULTIPLE --enable-sinks=ENCODE_FILE --options=pipeline.hmaudio.enable=1 $@
