...
absl::StatusOr<std::optional<AkazeCalibrationProfile>> read_akaze_calibration_profile(const fs::path& game_dir) {
  ...
  if (descriptor < 0) {
    ...
    return absl::InternalError(
        TO_STRING("Unable to open AKAZE lens calibration " << path.string() << ": " << std::strerror(errno)));
  }
  ...
  if (::fstat(descriptor, &before) != 0 || !S_ISREG(before.st_mode) || before.st_size <= 0 ||
    ...
    return absl::FailedPreconditionError(
        "AKAZE lens calibration must be a non-empty bounded regular file: " + path.string());
  }
  AkazeCalibrationProfile profile;
  profile.contents.resize(static_cast<size_t>(before.st_size));
  size_t offset = 0;
  while (offset < profile.contents.size()) {
    const ssize_t count = ::read(descriptor, profile.contents.data() + offset, profile.contents.size() - offset);
    if (count < 0) {
      if (errno == EINTR)
        continue;
      return absl::InternalError("Unable to read AKAZE lens calibration: " + path.string());
    }
    if (count == 0)
  ...
  if (context == nullptr)
    return absl::InternalError("Unable to initialize AKAZE calibration fingerprint");
  std::array<unsigned char, EVP_MAX_MD_SIZE> digest{};
  unsigned int digest_size = 0;
  const bool hashed = EVP_DigestInit_ex(context, EVP_sha256(), nullptr) == 1 &&
      EVP_DigestUpdate(context, profile.contents.data(), profile.contents.size()) == 1 &&
      EVP_DigestFinal_ex(context, digest.data(), &digest_size) == 1;
  EVP_MD_CTX_free(context);
  if (!hashed || digest_size != 32)
    return absl::InternalError("Unable to fingerprint AKAZE lens calibration");
  static constexpr char kHex[] = "0123456789abcdef";
  profile.fingerprint.reserve(digest_size * 2);
  for (unsigned int index = 0; index < digest_size; ++index) {
    profile.fingerprint.push_back(kHex[digest[index] >> 4]);
    profile.fingerprint.push_back(kHex[digest[index] & 0x0f]);
  }
  return profile;
}
...
absl::StatusOr<AkazeMatchingCalibration> load_akaze_matching_calibration_impl(const fs::path& game_dir) {
  ...
  HM_ASSIGN_OR_RETURN(profile, read_akaze_calibration_profile(game_dir));
  ...
  try {
    const YAML::Node document = YAML::Load(profile->contents);
    if (!document.IsMap())
    ...
    result.source_profile_fingerprint = std::move(profile->fingerprint);
...
absl::Status remove_stale_control_mask_snapshots(const fs::path& game_dir) {
  ...
  for (const auto& entry : *entries) {
    ...
    if (current_snapshot) {
      ...
      if (type_error)
        return absl::InternalError(
            "Unable to inspect visible control-mask snapshot collision: " + type_error.message());
    ...
    HM_RETURN_IF_ERROR(remove_pinned_directory(
        root, name, snapshot, current_snapshot ? kOwnedDirectoryMarkerName : std::string_view{}));
  }
  return absl::OkStatus();
}
...
absl::StatusOr<TiffPlacement> read_tiff_placement(const fs::path& path) {
  ...
  if (static_cast<long double>(x_px) < std::numeric_limits<int>::lowest() ||
      static_cast<long double>(x_px) > std::numeric_limits<int>::max() ||
      static_cast<long double>(y_px) < std::numeric_limits<int>::lowest() ||
      static_cast<long double>(y_px) > std::numeric_limits<int>::max()) {
...
absl::StatusOr<CanvasProvenanceCompatibility> check_stitch_algorithm_provenance_compatibility(
  ...
  if (!provenance->control_point_matcher.has_value() || !provenance->akaze_calibration_fingerprint.has_value()) {
    return CanvasProvenanceCompatibility{
  ...
  if (configured->control_point_matcher == ControlPointMatcher::kAkazeHamming) {
    std::optional<AkazeCalibrationProfile> profile;
    HM_ASSIGN_OR_RETURN(profile, read_akaze_calibration_profile(game_dir));
    expected_calibration = profile.has_value() ? "sha256:" + profile->fingerprint : "absent";
  }
  if (*provenance->akaze_calibration_fingerprint != expected_calibration)
    return CanvasProvenanceCompatibility{false, "the AKAZE lens calibration profile changed"};
  ...
  if (configured->mapping_backend == MappingBackend::kNona &&
      (!provenance->projection_framing.has_value() ||
       *provenance->projection_framing != configured->projection_framing)) {
    return CanvasProvenanceCompatibility{false, "the selected output projection framing changed"};
  }
  return CanvasProvenanceCompatibility{true, {}};
}
...
static absl::StatusOr<bool> validate_stitching_artifacts_locked(
  ...
  if (!p0_status.ok() || !p1_status.ok()) {
    ...
    std::cout << "Stitching artifacts exist but mapping TIFF placement metadata is invalid: " << placement_status
              << std::endl;
    if (absl::IsNotFound(placement_status) || absl::IsFailedPrecondition(placement_status) ||
        absl::IsInvalidArgument(placement_status)) {
      return false;
    }
  ...
  if (!canvas_status.ok()) {
    std::cout << "Stitching artifacts exist but mapping canvas metadata is invalid: " << canvas_status.status()
              << std::endl;
    if (absl::IsNotFound(canvas_status.status()) || absl::IsFailedPrecondition(canvas_status.status()) ||
        absl::IsInvalidArgument(canvas_status.status())) {
      return false;
    }
...
std::string read_scalar_or_default(
  ...
  for (const std::string& key : path) {
    if (!current || !current.IsMap())
      return fallback;
    const std::optional<YAML::Node> next = map_child(current, key);
    if (!next.has_value() || !next->IsDefined())
      return fallback;
    current.reset(*next);
  }
...
absl::Status create_control_points(
  ...
  options.run_autooptimizer = backend_choices.run_autooptimizer;
  options.projection = projection;
  options.projection_parameters = backend_choices.projection_parameters;
...
absl::Status recover_rink_transactions_locked(const fs::path& root) {
  ...
  for (const auto& entry : *root_entries) {
    ...
    auto cleanup = remove_pinned_directory(
        ...
        current_transaction ? kOwnedDirectoryMarkerName : std::string_view{});
    if (!cleanup.ok())
      return cleanup;
  }
