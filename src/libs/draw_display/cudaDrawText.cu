...
__global__ void drawGlyphKernel(uchar4* d_img, int imgWidth, int imgHeight,
                                const unsigned char* d_glyph, int glyphWidth, int glyphHeight,
                                int destX, int destY, uchar4 textColor, float threshold)
{
    ...
    if (pixelValue > threshold)
    {
        ...
        if (destPixelX >= 0 && destPixelX < imgWidth &&
            destPixelY >= 0 && destPixelY < imgHeight)
        {
