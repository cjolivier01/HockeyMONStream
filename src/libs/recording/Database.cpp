...
void Statement::Bind(int c, const std::string& v) {
  Check(sqlite3_bind_text(statement_, c, v.data(), v.size(), SQLITE_TRANSIENT));
}
void Statement::Blob(int c, const std::string& v) {
  Check(sqlite3_bind_blob(statement_, c, v.data(), v.size(), SQLITE_TRANSIENT));
}
void Statement::Bind(int c, double v) {
  Check(sqlite3_bind_double(statement_, c, v));
}
void Statement::Bind(int c, uint64_t v) {
  if (v > uint64_t(std::numeric_limits<int64_t>::max()))
    throw std::runtime_error("Telemetry integer exceeds SQLite range");
  Check(sqlite3_bind_int64(statement_, c, v));
}
void Statement::Bind(int c, int v) {
  Check(sqlite3_bind_int(statement_, c, v));
}
void Statement::Null(int c) {
  Check(sqlite3_bind_null(statement_, c));
}
bool Statement::Next() {
...
void Statement::Execute() {
  if (Next())
    throw std::runtime_error("Unexpected SQL row");
  Reset();
}
...
int64_t Statement::Int(int c) const {
  return sqlite3_column_int64(statement_, c);
}
double Statement::Real(int c) const {
  return sqlite3_column_double(statement_, c);
}
bool Statement::IsNull(int c) const {
  return sqlite3_column_type(statement_, c) == SQLITE_NULL;
}
Database::Database(const std::string& path, bool writable) {
  const int code =
      sqlite3_open_v2(path.c_str(), &db_, writable ? SQLITE_OPEN_READWRITE : SQLITE_OPEN_READONLY, nullptr);
  if (code != SQLITE_OK) {
...
std::string NewGuid() {
  ...
  while (offset < bytes.size()) {
    ...
    if (n < 0 && errno == EINTR)
      continue;
    if (n <= 0)
      throw std::runtime_error("Cannot generate recording GUID");
    offset += n;
  }
