...
std::optional<YAML::Node> get_node(const YAML::Node &n,
...
template <typename G_OBJ> class GstReferencedObject {
public:
  ...
  operator G_OBJ() { return obj_; }
  operator const G_OBJ() const { return obj_; }
  G_OBJ get() { return obj_; }
  const G_OBJ get() const { return obj_; }
  G_OBJ &operator->() {
    assert(obj_);
    return obj_;
  }
