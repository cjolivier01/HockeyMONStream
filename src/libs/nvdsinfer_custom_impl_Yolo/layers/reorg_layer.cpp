...
nvinfer1::ITensor* reorgLayer(
  ...
  if (block.at("type") == "reorg3d") {
    ...
    nvinfer1::ITensor* slice1 =
        sliceLayer(layerIdx, name1, input, start1, sizeAll, strideAll, network);
    assert(slice1 != nullptr);

    nvinfer1::ITensor* slice2 =
        sliceLayer(layerIdx, name2, input, start2, sizeAll, strideAll, network);
    assert(slice2 != nullptr);

    nvinfer1::ITensor* slice3 =
        sliceLayer(layerIdx, name3, input, start3, sizeAll, strideAll, network);
    assert(slice3 != nullptr);

    nvinfer1::ITensor* slice4 =
        sliceLayer(layerIdx, name4, input, start4, sizeAll, strideAll, network);
