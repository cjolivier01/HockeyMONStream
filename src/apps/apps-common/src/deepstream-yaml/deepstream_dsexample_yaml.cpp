...
gboolean parse_dsexample_yaml(NvDsDsExampleConfig *config,
  ...
  for (YAML::const_iterator itr = configyml["ds-example"].begin();
    ...
    if (paramKey == "enable") {
      ...
      strncpy(config->detection_mask_file, fn.c_str(),
              std::min(sizeof(config->detection_mask_file) /
                              sizeof(config->detection_mask_file[0]) -
                           1,
                       fn.size()));
      ...
      cout << "[WARNING] Unknown param found in dsexample: " << paramKey
           << endl;
    }
  }
