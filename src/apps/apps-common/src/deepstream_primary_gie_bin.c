...
gboolean create_primary_gie_bin(NvDsGieConfig* config, NvDsPrimaryGieBin* bin) {
  ...
  // BEGIN CAPS FILTER
  if (config->processing_width && config->processing_height) {
    // get_caps(bin->nvvidconv);

    bin->capsfilter = gst_element_factory_make(NVDS_ELEM_CAPS_FILTER, "primary_gie_capsfilter");
    if (!bin->capsfilter) {
      NVGSTDS_ERR_MSG_V("Failed to create 'primary_gie_capsfilter'");
      goto done;
    }
    auto caps = gst_caps_new_simple(
        ...
        "format",
        G_TYPE_STRING,
        //"{ I420, NV12, P010_10LE, BGRx, RGBA, Y444, GRAY8, GRAY16_LE, GBR, RGB, BGR, BGR10A2_LE, RGB10A2_LE, UYVP, UYVY }",
        "NV12",
        "width",
        G_TYPE_INT, config->processing_width,
        "height",
        G_TYPE_INT, config->processing_height,
        "framerate",
    ...
    // Convert the caps to a string and print it
    #define GST_CAPS_FEATURE_MEMORY_NVMM "memory:NVMM"
    // "{ NV12, RGBA, I420 }"
    auto feature = gst_caps_features_new(MEMORY_FEATURES, GST_CAPS_FEATURE_MEMORY_NVMM, NULL);
    ...
    // Both shpuld be zero
    assert(!config->processing_width && !config->processing_height);
  }
  // END CAPS FILTER
  ...
  g_object_set(
      G_OBJECT(bin->primary_gie), "config-file-path", GET_FILE_PATH(config->config_file_path), "process-mode", 1, NULL);
  ...
  if (NV_DS_GIE_PLUGIN_INFER == config->plugin_type) {
    ...
    if (config->model_engine_file_path) {
      g_object_set(
          G_OBJECT(bin->primary_gie), "model-engine-file", GET_FILE_PATH(config->model_engine_file_path), NULL);
    }
  }
  ...
  if (bin->capsfilter) {
    gst_bin_add_many(GST_BIN(bin->bin), bin->capsfilter, NULL);
  }
  ...
  if (bin->capsfilter) {
    NVGSTDS_LINK_ELEMENT(bin->nvvidconv, bin->capsfilter);
    NVGSTDS_LINK_ELEMENT(bin->capsfilter, bin->primary_gie);
  } else {
