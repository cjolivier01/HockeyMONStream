...
gboolean create_audio_classifier_bin(NvDsGieConfig* config, NvDsAudioClassifierBin* bin) {
  ...
  bin->classifier = gst_element_factory_make(NVDS_ELEM_INFER_AUDIO, "audio_classifier");
  if (!bin->classifier) {
    NVGSTDS_ERR_MSG_V("Failed to create 'audio_classifier'");
    goto done;
  }

  g_object_set(G_OBJECT(bin->classifier), "config-file-path", GET_FILE_PATH(config->config_file_path), NULL);

  if (config->is_batch_size_set)
    g_object_set(G_OBJECT(bin->classifier), "batch-size", config->batch_size, NULL);

  if (config->is_unique_id_set)
    g_object_set(G_OBJECT(bin->classifier), "unique-id", config->unique_id, NULL);

  if (config->is_gpu_id_set)
    g_object_set(G_OBJECT(bin->classifier), "gpu-id", config->gpu_id, NULL);

  if (config->model_engine_file_path)
    g_object_set(G_OBJECT(bin->classifier), "model-engine-file", GET_FILE_PATH(config->model_engine_file_path), NULL);

  if (config->is_frame_size_set)
    g_object_set(G_OBJECT(bin->classifier), "audio-framesize", config->frame_size, NULL);

  if (config->is_hop_size_set)
    g_object_set(G_OBJECT(bin->classifier), "audio-hopsize", config->hop_size, NULL);
