...
gboolean create_dewarper_bin(NvDsDewarperConfig* config, NvDsDewarperBin* bin) {
  ...
  bin->nvvidconv = gst_element_factory_make(NVDS_ELEM_VIDEO_CONV, "dewarper_conv");
  ...
  bin->src_queue = gst_element_factory_make(NVDS_ELEM_QUEUE, "dewarper_src_queue");
  if (!bin->src_queue) {
  ...
  bin->dewarper_caps_filter = gst_element_factory_make(NVDS_ELEM_CAPS_FILTER, "dewarper_caps_filter");
  ...
  g_object_set(G_OBJECT(bin->nvdewarper), "config-file", config->config_file, NULL);
