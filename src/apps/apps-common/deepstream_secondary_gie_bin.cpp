 ...
 */
static gboolean create_secondary_gie(
    NvDsGieConfig* configs1,
    NvDsSecondaryGieBinSubBin* subbins1,
    GstBin* bin,
    guint index) {
  gboolean ret = FALSE;
  gchar elem_name[50];
  ...
  for (i = 0; i < config->num_operate_on_class_ids; i++) {
    g_snprintf(
        operate_on_class_str + strlen(operate_on_class_str),
  ...
  g_object_set(
      G_OBJECT(subbin->secondary_gie),
      "config-file-path",
      GET_FILE_PATH(config->config_file_path),
      "process-mode",
      2,
      NULL);
  ...
  if (config->is_gpu_id_set && NV_DS_GIE_PLUGIN_INFER_SERVER == config->plugin_type) {
    NVGSTDS_WARN_MSG_V(
        "gpu-id: %u in sgie: %s is ignored, only accept nvinferserver config", config->gpu_id, elem_name);
  }

  if (NV_DS_GIE_PLUGIN_INFER == config->plugin_type) {
    if (config->model_engine_file_path) {
      g_object_set(
          G_OBJECT(subbin->secondary_gie), "model-engine-file", GET_FILE_PATH(config->model_engine_file_path), NULL);
    }
 ...
 */
static gboolean should_create_secondary_gie(
    NvDsGieConfig* config_array,
    guint num_configs,
    NvDsSecondaryGieBinSubBin* bins,
    guint index,
    gint primary_gie_id) {
...
gboolean create_secondary_gie_bin(
  ...
  bin->wait_for_sgie_process_buf_probe_id = gst_pad_add_probe(
      pad,
      (GstPadProbeType)(GST_PAD_PROBE_TYPE_BUFFER | GST_PAD_PROBE_TYPE_EVENT_BOTH),
      wait_queue_buf_probe,
      bin,
      NULL);
      ...
      if (bin->sub_bins[i].parent_index == -1) {
        ...
        if (bin->sub_bins[bin->sub_bins[i].parent_index].tee) {
          ...
          NVGSTDS_LINK_ELEMENT(
              bin->sub_bins[bin->sub_bins[i].parent_index].secondary_gie, bin->sub_bins[i].secondary_gie);
        }
      }
    }
  }
