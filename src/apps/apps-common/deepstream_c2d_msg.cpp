...
static void connect_cb(NvMsgBrokerClientHandle h_ptr, NvMsgBrokerErrorType status) {}

static void subscribe_cb(NvMsgBrokerErrorType flag, void* msg, int msg_len, char* topic, void* uData) {
  ...
  if (uData) {
    ...
    if (ctx->subscribeCb) {
      ...
      if (parsedMsg->type == NVDS_C2D_MSG_SR_START || parsedMsg->type == NVDS_C2D_MSG_SR_STOP) {
...
NvDsC2DContext* start_cloud_to_device_messaging(
  ...
  ctx->connHandle = nv_msgbroker_connect(ctx->connStr, ctx->protoLib, connect_cb, ctx->configFile);
  if (!ctx->connHandle) {
  ...
  if (nv_msgbroker_subscribe(ctx->connHandle, topicList, numTopic, subscribe_cb, (gpointer)ctx) !=
      NV_MSGBROKER_API_OK) {
...
gboolean stop_cloud_to_device_messaging(NvDsC2DContext* ctx) {
