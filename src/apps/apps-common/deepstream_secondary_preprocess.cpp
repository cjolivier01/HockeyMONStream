 ...
 */
static gboolean create_secondary_preprocess(
    NvDsPreProcessConfig* configs1,
 ...
 */
static gboolean should_create_secondary_preprocess(
...
gboolean create_secondary_preprocess_bin(
    guint num_secondary_preprocess,
  ...
  for (i = 0; i < num_secondary_preprocess; i++) {
    should_create_secondary_preprocess(config_array, num_secondary_preprocess, bin->sub_bins, i, primary_gie_unique_id);
  }
      ...
      if (bin->sub_bins[i].parent_index == -1) {
        ...
        if (bin->sub_bins[bin->sub_bins[i].parent_index].tee) {
          ...
          NVGSTDS_LINK_ELEMENT(
              bin->sub_bins[bin->sub_bins[i].parent_index].secondary_preprocess, bin->sub_bins[i].secondary_preprocess);
        }
      }
    }
  }
