...
gboolean create_segvisual_bin(NvDsSegVisualConfig* config, NvDsSegVisualBin* bin) {
  ...
  gst_bin_add_many(GST_BIN(bin->bin), bin->queue, bin->nvvidconv, bin->conv_queue, bin->nvsegvisual, NULL);
  ...
  if (config->width != 0 && config->height != 0) {
    g_object_set(G_OBJECT(bin->nvsegvisual), "width", config->width, "height", config->height, NULL);
  } else {
    g_object_set(G_OBJECT(bin->nvsegvisual), "width", SEG_OUTPUT_WIDTH, "height", SEG_OUTPUT_HEIGHT, NULL);
  }
