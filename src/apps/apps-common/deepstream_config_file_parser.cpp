...
gboolean parse_source(NvDsSourceConfig* config, GKeyFile* key_file, gchar* group, const gchar* cfg_file_path) {
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_TYPE)) {
      ...
      config->udp_buffer_size = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_UDP_BUFFER_SIZE, &error);
      CHECK_ERROR(error);
      ...
      config->extract_sei_type5_data =
          g_key_file_get_integer(key_file, CONFIG_GROUP_SOURCE, CONFIG_GROUP_SOURCE_EXTRACT_SEI_TYPE5_DATA, &error);
      CHECK_ERROR(error);
    }
#if defined(__aarch64__) && !defined(AARCH64_IS_SBSA)
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_COMPUTE_HW)) {
      ...
      config->buffer_pool_size =
          g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_BUFFER_POOL_SIZE, &error);
      CHECK_ERROR(error);
      ...
      config->attach_sys_ts_as_ntp =
          g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_ATTACH_SYS_TS_AS_NTP, &error);
      CHECK_ERROR(error);
      ...
      config->batched_push_timeout =
          g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_BATCHED_PUSH_TIMEOUT, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_NVBUF_MEMORY_TYPE)) {
      ...
      config->config_file_path = get_absolute_file_path(
          cfg_file_path,
          g_key_file_get_string(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_CONFIG_FILE_PATH, &error));
      CHECK_ERROR(error);
      ...
      config->no_pipeline_eos =
          g_key_file_get_boolean(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_DROP_PIPELINE_EOS, &error);
      CHECK_ERROR(error);
...
gboolean parse_dsanalytics(NvDsDsAnalyticsConfig* config, GKeyFile* key_file, const gchar* cfg_file_path) {
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
      config->enable = g_key_file_get_integer(key_file, CONFIG_GROUP_DSANALYTICS, CONFIG_GROUP_ENABLE, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_DSANALYTICS_CONFIG_FILE)) {
      config->config_file_path = get_absolute_file_path(
          cfg_file_path,
          g_key_file_get_string(key_file, CONFIG_GROUP_DSANALYTICS, CONFIG_GROUP_DSANALYTICS_CONFIG_FILE, &error));
      CHECK_ERROR(error);
    } else {
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
      ...
      config->processing_width =
          g_key_file_get_integer(key_file, CONFIG_GROUP_DSEXAMPLE, CONFIG_GROUP_DSEXAMPLE_PROCESSING_WIDTH, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_DSEXAMPLE_PROCESSING_HEIGHT)) {
      config->processing_height =
          g_key_file_get_integer(key_file, CONFIG_GROUP_DSEXAMPLE, CONFIG_GROUP_DSEXAMPLE_PROCESSING_HEIGHT, &error);
      CHECK_ERROR(error);
      ...
      config->gpu_id = g_key_file_get_integer(key_file, CONFIG_GROUP_DSEXAMPLE, CONFIG_GROUP_DSEXAMPLE_GPU_ID, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_NVBUF_MEMORY_TYPE)) {
      config->nvbuf_memory_type =
...
gboolean parse_segvisual(NvDsSegVisualConfig* config, GKeyFile* key_file) {
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
      config->enable = g_key_file_get_integer(key_file, CONFIG_GROUP_SEGVISUAL, CONFIG_GROUP_ENABLE, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_SEGVISUAL_WIDTH)) {
      config->width = g_key_file_get_integer(key_file, CONFIG_GROUP_SEGVISUAL, CONFIG_GROUP_SEGVISUAL_WIDTH, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_SEGVISUAL_HEIGHT)) {
      config->height = g_key_file_get_integer(key_file, CONFIG_GROUP_SEGVISUAL, CONFIG_GROUP_SEGVISUAL_HEIGHT, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_SEGVISUAL_BATCH_SIZE)) {
      config->max_batch_size =
          g_key_file_get_integer(key_file, CONFIG_GROUP_SEGVISUAL, CONFIG_GROUP_SEGVISUAL_BATCH_SIZE, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_NVBUF_MEMORY_TYPE)) {
      config->nvbuf_memory_type =
          g_key_file_get_integer(key_file, CONFIG_GROUP_SEGVISUAL, CONFIG_NVBUF_MEMORY_TYPE, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GPU_ID)) {
      config->gpu_id = g_key_file_get_integer(key_file, CONFIG_GROUP_SEGVISUAL, CONFIG_GPU_ID, &error);
      CHECK_ERROR(error);
    } else {
      NVGSTDS_WARN_MSG_V("Unknown key '%s' for group [%s]", *key, CONFIG_GROUP_SEGVISUAL);
    }
  }
...
gboolean parse_osd(NvDsOSDConfig* config, GKeyFile* key_file) {
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
      ...
      config->mode = (NvOSD_Mode)g_key_file_get_integer(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_MODE, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_OSD_BORDER_WIDTH)) {
      config->border_width = g_key_file_get_integer(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_BORDER_WIDTH, &error);
      CHECK_ERROR(error);
      ...
      config->enable_clock = g_key_file_get_integer(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_CLOCK_ENABLE, &error);
      CHECK_ERROR(error);
      ...
      config->nvbuf_memory_type = g_key_file_get_integer(key_file, CONFIG_GROUP_OSD, CONFIG_NVBUF_MEMORY_TYPE, &error);
      CHECK_ERROR(error);
...
gboolean parse_dewarper(NvDsDewarperConfig* config, GKeyFile* key_file, const gchar* cfg_file_path, gchar* dewarper) {
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
      config->enable = g_key_file_get_integer(key_file, dewarper, CONFIG_GROUP_ENABLE, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GPU_ID)) {
      config->gpu_id = g_key_file_get_integer(key_file, dewarper, CONFIG_GPU_ID, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_DEWARPER_CONFIG_FILE)) {
      config->config_file = get_absolute_file_path(
          cfg_file_path, g_key_file_get_string(key_file, dewarper, CONFIG_GROUP_DEWARPER_CONFIG_FILE, &error));
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_NVBUF_MEMORY_TYPE)) {
      config->nvbuf_memory_type = g_key_file_get_integer(key_file, dewarper, CONFIG_NVBUF_MEMORY_TYPE, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_DEWARPER_NUM_SURFACES_PER_FRAME)) {
      config->num_surfaces_per_frame =
          g_key_file_get_integer(key_file, dewarper, CONFIG_GROUP_DEWARPER_NUM_SURFACES_PER_FRAME, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_DEWARPER_SOURCE_ID)) {
      guint source_id = g_key_file_get_integer(key_file, dewarper, CONFIG_GROUP_DEWARPER_SOURCE_ID, &error);
      config->source_id = source_id;
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_DEWARPER_NUM_OUTPUT_BUFFERS)) {
      config->num_out_buffers =
          g_key_file_get_integer(key_file, dewarper, CONFIG_GROUP_DEWARPER_NUM_OUTPUT_BUFFERS, &error);
      CHECK_ERROR(error);
    } else {
      NVGSTDS_WARN_MSG_V("Unknown key '%s' for group [%s]", *key, dewarper);
    }
  }
  // g_free(dewarper);
  ret = TRUE;
...
gboolean parse_preprocess(NvDsPreProcessConfig* config, GKeyFile* key_file, gchar* group, const gchar* cfg_file_path) {
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
      ...
      config->operate_on_gie_id = g_key_file_get_integer(key_file, group, CONFIG_GROUP_GIE_ID_FOR_OPERATION, &error);
      CHECK_ERROR(error);
...
gboolean parse_gie(NvDsGieConfig* config, GKeyFile* key_file, gchar* group, const gchar* cfg_file_path) {
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
      ...
      config->input_tensor_meta = g_key_file_get_boolean(key_file, group, CONFIG_GROUP_GIE_INPUT_TENSOR_META, &error);
      CHECK_ERROR(error);
      ...
      config->label_file_path =
          get_absolute_file_path(cfg_file_path, g_key_file_get_string(key_file, group, CONFIG_GROUP_GIE_LABEL, &error));
      CHECK_ERROR(error);
      ...
      if (strlen(key1) > 0) {
        ...
        if (class_index == 0 && endptr == key1) {
          NVGSTDS_WARN_MSG_V("BBOX colors should be specified with key '%s%%d'", CONFIG_GROUP_GIE_BBOX_BORDER_COLOR);
          continue;
        }
      }
      ...
      if (strlen(key1) > 0) {
        ...
        if (class_index == 0 && endptr == key1) {
          NVGSTDS_WARN_MSG_V(
              "BBOX background colors should be specified with key '%s%%d'", CONFIG_GROUP_GIE_BBOX_BG_COLOR);
          continue;
        }
      }
...
gboolean parse_tracker(NvDsTrackerConfig* config, GKeyFile* key_file, const gchar* cfg_file_path) {
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
      ...
      config->height = g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_HEIGHT, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GPU_ID)) {
      ...
      gchar* temp = g_key_file_get_string(key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_LL_CONFIG_FILE, &error);
      CHECK_ERROR(error);
      ...
      config->tracking_surface_type =
          g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_TRACKING_SURFACE_TYPE, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_DISPLAY_TRACKING_ID)) {
      config->display_tracking_id =
          g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_DISPLAY_TRACKING_ID, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_TRACKING_ID_RESET_MODE)) {
      config->tracking_id_reset_mode =
          g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_TRACKING_ID_RESET_MODE, &error);
      CHECK_ERROR(error);
      ...
      config->user_meta_pool_size =
          g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_USER_META_POOL_SIZE, &error);
      CHECK_ERROR(error);
...
gboolean parse_sink(NvDsSinkSubBinConfig* config, GKeyFile* key_file, gchar* group, const gchar* cfg_file_path) {
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
      ...
      config->encoder_config.copy_meta = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_COPY_META, &error);
      CHECK_ERROR(error);
      ...
      config->encoder_config.sw_preset = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_SW_PRESET, &error);
      CHECK_ERROR(error);
      ...
      config->encoder_config.rtsp_port = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_RTSP_PORT, &error);
      CHECK_ERROR(error);
      ...
      config->render_config.plane_id = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_PLANE_ID, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_SET_MODE)) {
      config->render_config.set_mode = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_SET_MODE, &error);
      CHECK_ERROR(error);
      ...
      CHECK_ERROR(error);
        ...
        !g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_CONV_CONFIG) ||
      ...
      config->msg_conv_broker_config.broker_config_file_path = get_absolute_file_path(
          cfg_file_path, g_key_file_get_string(key_file, group, CONFIG_GROUP_SINK_MSG_BROKER_CONFIG_FILE, &error));
      CHECK_ERROR(error);
...
gboolean parse_tiled_display(NvDsTiledDisplayConfig* config, GKeyFile* key_file) {
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
      ...
      config->columns =
          g_key_file_get_integer(key_file, CONFIG_GROUP_TILED_DISPLAY, CONFIG_GROUP_TILED_DISPLAY_COLUMNS, &error);
      CHECK_ERROR(error);
      ...
      config->buffer_pool_size = g_key_file_get_integer(
          key_file, CONFIG_GROUP_TILED_DISPLAY, CONFIG_GROUP_TILED_DISPLAY_BUFFER_POOL_SIZE, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_TILED_DISPLAY_SQUARE_SEQ_GRID)) {
      config->square_seq_grid = g_key_file_get_boolean(
          key_file, CONFIG_GROUP_TILED_DISPLAY, CONFIG_GROUP_TILED_DISPLAY_SQUARE_SEQ_GRID, &error);
      CHECK_ERROR(error);
    } else {
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
      ...
      config->debug_payload_dir = get_absolute_file_path(
          cfg_file_path, g_key_file_get_string(key_file, group, CONFIG_GROUP_SINK_MSG_CONV_DEBUG_PAYLOAD_DIR, &error));
      CHECK_ERROR(error);
...
gboolean parse_msgconsumer(
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
      ...
      config->config_file_path = get_absolute_file_path(
          cfg_file_path, g_key_file_get_string(key_file, group, CONFIG_GROUP_MSG_CONSUMER_CONFIG, &error));
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_MSG_CONSUMER_PROTO_LIB)) {
      config->proto_lib = g_key_file_get_string(key_file, group, CONFIG_GROUP_MSG_CONSUMER_PROTO_LIB, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_MSG_CONSUMER_CONN_STR)) {
      config->conn_str = g_key_file_get_string(key_file, group, CONFIG_GROUP_MSG_CONSUMER_CONN_STR, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_MSG_CONSUMER_SENSOR_LIST_FILE)) {
      config->sensor_list_file = get_absolute_file_path(
          cfg_file_path, g_key_file_get_string(key_file, group, CONFIG_GROUP_MSG_CONSUMER_SENSOR_LIST_FILE, &error));
      CHECK_ERROR(error);
      ...
      topicList =
          g_key_file_get_string_list(key_file, group, CONFIG_GROUP_MSG_CONSUMER_SUBSCRIBE_TOPIC_LIST, &length, &error);
...
gboolean parse_image_save(NvDsImageSave* config, GKeyFile* key_file, gchar* group, const gchar* cfg_file_path) {
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
      ...
      config->min_confidence = g_key_file_get_double(key_file, group, CONFIG_GROUP_IMG_SAVE_MIN_CONFIDENCE, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_IMG_SAVE_MAX_CONFIDENCE)) {
      config->max_confidence = g_key_file_get_double(key_file, group, CONFIG_GROUP_IMG_SAVE_MAX_CONFIDENCE, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_IMG_SAVE_MIN_BOX_WIDTH)) {
      config->min_box_width = g_key_file_get_integer(key_file, group, CONFIG_GROUP_IMG_SAVE_MIN_BOX_WIDTH, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_IMG_SAVE_MIN_BOX_HEIGHT)) {
      config->min_box_height = g_key_file_get_integer(key_file, group, CONFIG_GROUP_IMG_SAVE_MIN_BOX_HEIGHT, &error);
      CHECK_ERROR(error);
    } else {
      NVGSTDS_WARN_MSG_V("Unknown key '%s' for group [%s]", *key, group);
    }
