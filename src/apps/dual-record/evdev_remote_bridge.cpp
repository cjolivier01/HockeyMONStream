...
static void usage() {
  std::cerr << "Usage: evdev_remote_bridge [--sock PATH] [--dev /dev/input/eventX | --name SUBSTR]"
            << " [--key-start LIST] [--key-stop LIST] [--key-toggle LIST] [--list-devices] [--print-events]"
            << std::endl;
  std::cerr << "  LIST can be comma-separated KEY_* names (e.g., KEY_PLAYPAUSE) or numeric codes (e.g., 164,0xa4)."
            << std::endl;
}
...
int main(int argc, char** argv) {
  ...
  auto key_name_to_code = [&](const std::string& token) -> int {
    // Try numeric
    if (!token.empty() &&
        (std::isdigit(token[0]) || (token.size() > 2 && token[0] == '0' && (token[1] == 'x' || token[1] == 'X')))) {
      char* end = nullptr;
      long v = std::strtol(token.c_str(), &end, 0);
      if (end && *end == '\0' && v >= 0 && v <= 0x7fff)
  ...
  auto parse_key_list = [&](const char* s, std::unordered_set<int>& out) {
    ...
    while (std::getline(ss, tok, ',')) {
      ...
      if (code >= 0)
        ...
        std::cerr << "Warning: unknown key token: " << tok << std::endl;
    }
  };
  ...
  for (int i = 1; i < argc; ++i) {
    if (!strcmp(argv[i], "-s") || !strcmp(argv[i], "--sock")) {
      ...
      if (i + 1 < argc)
        parse_key_list(argv[++i], keys_start);
      else {
        usage();
        return 1;
      }
      ...
      if (i + 1 < argc)
        parse_key_list(argv[++i], keys_stop);
      else {
        usage();
        return 1;
      }
      ...
      if (i + 1 < argc)
        parse_key_list(argv[++i], keys_toggle);
      else {
        usage();
        return 1;
      }
  ...
  if (dev_path) {
    ...
    for (const auto& n : nodes) {
      ...
      std::string nm = evdev_name(fd);
      close(fd);
      if (!nm.empty() && nm.find(name_sub) != std::string::npos) {
        target_dev = n;
        break;
      }
    }
  ...
  if (fd < 0) {
    ...
    if (errno == EACCES)
      std::cerr << "Hint: need permissions (try sudo or add user to input group)." << std::endl;
    return 1;
  }
