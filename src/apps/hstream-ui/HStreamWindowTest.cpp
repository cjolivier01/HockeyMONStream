...
bool test_pipeline_buttons(HStreamWindow* window) {
  ...
  if (!stop || !start || !pause || !restart || !mode || !control_points || !stitch_frame_time || !game_id || !rotate ||
      ...
      !program_controls || !program_controls_toggle || !stitched_controls || !program_control_tabs ||
  ...
  if (!expect(
          program_controls->isAncestorOf(max_speed_x) && program_controls->isAncestorOf(bring_up_shadows) &&
              ...
              program_control_tabs->count() == 4 && stitched_control_tabs->count() == 1,
  ...
  for (int i = 0;
       i < 100 && !window->logText().contains("stdin:@set-property stitched_gpu_preview_sink show-rink-mask=1");
       ++i) {
    QApplication::processEvents();
    QTest::qWait(10);
  }
  if (!expect(
          show_player_tracking->isChecked() && show_play_tracking->isChecked() && show_rink_mask->isChecked() &&
              window->logText().contains("stdin:@set-property program_gpu_preview_sink show-player-tracking=1") &&
              window->logText().contains("stdin:@set-property stitched_gpu_preview_sink show-play-tracking=1") &&
              window->logText().contains("stdin:@set-property stitched_gpu_preview_sink show-rink-mask=1") &&
              window->logText().contains("stdin:@set-property dsplaytracker0 draw=1"),
          "Live overlay checkboxes must update both GPU preview sinks and the play-tracker metadata producer")) {
    return false;
  }
  ...
  if (!expect(
          render_video->isChecked() && !preview_target->isHidden() && program_focus->isVisible() &&
              ...
              window->logText().count("stdin:@set-property program_gpu_preview_sink show-player-tracking=1") >= 2 &&
              window->logText().count("stdin:@set-property dsplaytracker0 draw=1") >= 2 &&
              window->logText().count("pipeline started pid=") == pipeline_start_count,
...
bool test_camera_controls(HStreamWindow* window) {
  if (!expect(window->cameraTabCount() == 5, "Native-effective controls should be grouped by associated stage")) {
    return false;
  }
  ...
  if (!expect(
          !lookup_yaml_path(after_direct_reset, {"rink", "camera", "stop_on_dir_change_delay"}, nullptr) &&
              ...
              !lookup_yaml_path(
                  after_direct_reset, {"pipeline", "hmplaycropper", "properties", "shadow-lift-black-point"}, nullptr),
  ...
  if (!expect(
          ...
          "Saved UI shadow controls should take precedence over canonical runtime values")) {
    return false;
  }
  ...
  activate(create);
  if (!expect(
          bring_up_shadows->value() == 45 && lift_shadow_black_point->isChecked() && !save->isEnabled(),
          "Reload should select companion shadow controls over stale canonical values before launch")) {
    return false;
  }
  mode->setCurrentIndex(mode->findData("program"));
  activate(start);
