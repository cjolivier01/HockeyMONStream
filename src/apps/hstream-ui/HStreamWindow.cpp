...
void HStreamWindow::handlePipelineFinished(int exit_code, QProcess::ExitStatus exit_status) {
  ...
  scheduled_playtracker_control_id_.clear();
...
QString HStreamWindow::writePlaytrackerRuntimeConfig() {
  ...
  try {
    ...
    YAML::Node play_tracker = play_tracker_config["play-tracker"];
    play_tracker["hstream-apply-to-fast-box"] = slider_value("Apply_To_Fast_Box") != 0;
    play_tracker["hstream-apply-to-follower-box"] = slider_value("Apply_To_Follower_Box") != 0;
    ...
    const QString changed_id = scheduled_playtracker_control_id_;
    auto set_changed_int = [&](const char* key, const char* control_id) {
      if (changed_id == control_id)
    ...
    if (changed_id == "Cancel_Stop_On_Opposite_Direction")
      runtime_tuning["cancel-stop-on-opposite-dir"] = slider_value(changed_id) != 0;
    ...
    if (changed_id == "Overshoot_Speed_Ratio_x100")
      runtime_tuning["overshoot-scale-speed-ratio"] = ratio_x100(slider_value(changed_id));
    if (changed_id == "Max_Speed_X_x10")
      runtime_tuning["max-speed-x"] = static_cast<double>(slider_value(changed_id)) / 10.0;
    if (changed_id == "Max_Speed_Y_x10")
      runtime_tuning["max-speed-y"] = static_cast<double>(slider_value(changed_id)) / 10.0;
    if (changed_id == "Max_Accel_X_x10")
      runtime_tuning["max-accel-x"] = static_cast<double>(slider_value(changed_id)) / 10.0;
    if (changed_id == "Max_Accel_Y_x10")
      runtime_tuning["max-accel-y"] = static_cast<double>(slider_value(changed_id)) / 10.0;
...
void HStreamWindow::handleRuntimeControlResponse(const QString& line) {
  auto acknowledge = [this](std::vector<PendingRuntimeControl>::iterator pending, const QString& result) {
    ...
    if (pending->property == "runtime-tuning-config-file" && result == "live") {
      if (!last_playtracker_runtime_snapshot_.isEmpty() &&
          last_playtracker_runtime_snapshot_ != pending->runtime_value) {
        QFile::remove(last_playtracker_runtime_snapshot_);
      }
      last_playtracker_runtime_snapshot_ = pending->runtime_value;
    } else if (pending->property == "runtime-tuning-config-file") {
...
void HStreamWindow::schedulePlaytrackerRuntimeControl(const QString& id, int value) {
  ...
  scheduled_playtracker_control_id_ = id;
  scheduled_playtracker_control_value_ = value;
  ...
  QTimer::singleShot(120, this, [this, generation]() {
    ...
    if (runtime_config_path.isEmpty()) {
      appendLog(QString("camera control %1=%2 apply=failed reason=runtime config publication")
                    .arg(scheduled_playtracker_control_id_)
                    .arg(scheduled_playtracker_control_value_));
      return;
    }
    ...
    pending_runtime_controls_.push_back(
        {"dsplaytracker0",
         ...
         runtime_config_path,
         scheduled_playtracker_control_id_,
         scheduled_playtracker_control_value_});
    appendLog(QString("camera control %1=%2 apply=pending")
                  .arg(scheduled_playtracker_control_id_)
                  .arg(scheduled_playtracker_control_value_));
  });
}
  ...
  connect(slider, &QSlider::valueChanged, this, [this, id, value_label](int new_value) {
    ...
    if (sent_live) {
      appendLog(QString("camera control %1=%2 apply=pending").arg(id).arg(new_value));
        ...
        !scheduled_playtracker_control_id_.isEmpty() && scheduled_playtracker_control_id_ == id &&
        scheduled_playtracker_control_value_ == new_value) {
