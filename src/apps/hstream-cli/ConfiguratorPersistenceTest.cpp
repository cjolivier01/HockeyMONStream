...
struct ConfiguratorTestAccess {
  ...
  static void set_config(Configurator* configurator, const YAML::Node& config) {
...
absl::StatusOr<YAML::Node> player_selection_fixture(
  ...
  plan.settings.frame_count = frame_count;
  ...
  for (size_t index = 1; index < frame_count; ++index) {
    PlayerFrameObservation frame = anchor;
    frame.pair.timeline_pts_ns += index * kPlayerFrameSecond;
    for (auto& camera : frame.pair.cameras)
      camera.source_pts_ns += index * kPlayerFrameSecond;
    frame.coverage = {0};
...
int main() {
  ...
  bool ok = true;
  const auto memory_profile_fixture = [] {
    return YAML::Load(
        "pipeline:\n"
        "  application: {global-gpu-id: 0}\n"
        "  source0: {num-extra-surfaces: 4, low-latency-mode: 0}\n"
        "  source1: {num-extra-surfaces: 4, low-latency-mode: 0}\n"
        "  hmstitcher:\n"
        "    num-output-buffers: 2\n"
        "    pre-converter-output-buffers: 0\n"
        "    properties: {output-pool-extra-buffers: 4}\n"
        "    private-properties: {compact-workspace: false}\n"
        "  hmplaycropper:\n"
        "    num-output-buffers: 3\n"
        "    properties: {output-pool-extra-buffers: 4}\n"
        "  primary-gie: {batch-size: 2, config-file: config_infer_yolov8_hockey.yaml}\n");
  };
  constexpr uint64_t kGiB = 1024ULL * 1024ULL * 1024ULL;
  hm::Configurator automatic_low_memory("", "", hm::Configurator::kUseConfigFileGpu);
  hm::ConfiguratorTestAccess::set_config(&automatic_low_memory, memory_profile_fixture());
  const bool automatic_low_memory_applied =
      hm::ConfiguratorTestAccess::apply_gpu_memory_profile(&automatic_low_memory, 8 * kGiB).ok();
  const YAML::Node automatic_low_pipeline = automatic_low_memory.config()["pipeline"];
  ok &= expect(
      automatic_low_memory_applied && automatic_low_pipeline["source0"]["num-extra-surfaces"].as<int>() == 0 &&
          automatic_low_pipeline["source1"]["low-latency-mode"].as<int>() == 1 &&
          automatic_low_pipeline["hmstitcher"]["num-output-buffers"].as<int>() == 1 &&
          automatic_low_pipeline["hmstitcher"]["pre-converter-output-buffers"].as<int>() == 1 &&
          automatic_low_pipeline["hmstitcher"]["properties"]["output-pool-extra-buffers"].as<int>() == 0 &&
  ...
  hm::Configurator automatic_standard_memory("", "", hm::Configurator::kUseConfigFileGpu);
  hm::ConfiguratorTestAccess::set_config(&automatic_standard_memory, memory_profile_fixture());
  const bool automatic_standard_memory_applied =
      hm::ConfiguratorTestAccess::apply_gpu_memory_profile(&automatic_standard_memory, 8 * kGiB + 1).ok();
  const YAML::Node automatic_standard_pipeline = automatic_standard_memory.config()["pipeline"];
  ok &= expect(
      automatic_standard_memory_applied &&
          automatic_standard_pipeline["source0"]["num-extra-surfaces"].as<int>() == 4 &&
          automatic_standard_pipeline["hmstitcher"]["num-output-buffers"].as<int>() == 2 &&
          !automatic_standard_pipeline["hmstitcher"]["private-properties"]["compact-workspace"].as<bool>() &&
          automatic_standard_pipeline["hmplaycropper"]["num-output-buffers"].as<int>() == 3 &&
          automatic_standard_pipeline["primary-gie"]["batch-size"].as<int>() == 2 &&
          automatic_standard_pipeline["primary-gie"]["config-file"].as<std::string>() ==
  ...
  ok &= expect(
      forced_standard_memory.apply_config_item("runtime.gpu_memory_profile", "standard").ok() &&
          hm::ConfiguratorTestAccess::apply_gpu_memory_profile(&forced_standard_memory, 8 * kGiB).ok() &&
          forced_standard_memory.config()["pipeline"]["hmstitcher"]["num-output-buffers"].as<int>() == 2,
      "An explicit standard profile must retain normal settings on an 8 GiB GPU");

  hm::Configurator forced_low_memory("", "", hm::Configurator::kUseConfigFileGpu);
  hm::ConfiguratorTestAccess::set_config(&forced_low_memory, memory_profile_fixture());
  ok &= expect(
      forced_low_memory.apply_config_item("runtime.gpu_memory_profile", "low").ok() &&
          hm::ConfiguratorTestAccess::apply_gpu_memory_profile(&forced_low_memory, 16 * kGiB).ok() &&
...
play-tracker:
  ...
  if (selected_fixture.ok()) {
    ...
    for (const bool changed : {false, true}) {
      ...
      const auto before_override = YAML::LoadFile((input_override_dir / "config.yaml").string());
      ...
      const auto persisted = overridden.persist_stitch_frame_time_override(changed ? "00:00:09" : "00:00:08");
      const auto after_persist = YAML::LoadFile((input_override_dir / "config.yaml").string());
      ok &= expect(
          loaded.ok() &&
          ...
          "preserve the plan's stored anchor even when inherited settings make it redundant");
    }
    ...
    const fs::path no_count_dir = games / "selected-frame-missing-count";
    fs::create_directories(no_count_dir);
    const auto no_count_fixture = player_selection_fixture(no_count_dir, 4);
    ok &= expect(no_count_fixture.ok(), "selected-frame count fallback fixture must validate");
    if (no_count_fixture.ok()) {
      YAML::Node no_count_config = YAML::Clone(*no_count_fixture);
      no_count_config["hstream_ui"]["stitching_calibration"]["status"] = "complete";
      const std::string no_count_contents = YAML::Dump(no_count_config);
      fs::remove(no_count_dir / "source-left.bin");
      fs::remove(no_count_dir / "source-right.bin");
      for (const char* path :
        ...
        ok &= expect(
            hm::stitching::publish_game_config(no_count_dir, no_count_contents + "\n").ok(),
            "selected-frame count fallback fixture must publish");
        hm::Configurator same_count(
            "selected-frame-missing-count", baseline_root.string(), hm::Configurator::kUseConfigFileGpu);
        const auto loaded = same_count.configure();
        const auto before_override = YAML::LoadFile((no_count_dir / "config.yaml").string());
        const auto applied = same_count.apply_config_item(path, "4");
        const auto reconciled = hm::ConfiguratorTestAccess::reconcile_selected_frame_count_override(&same_count);
        const auto saved = YAML::LoadFile((no_count_dir / "config.yaml").string());
        const auto saved_fingerprint = hm::stitching::player_frame_selection_fingerprint(saved);
        const auto expected_fingerprint = hm::stitching::player_frame_selection_fingerprint(no_count_config);
        ok &= expect(
            loaded.ok() && applied.ok() && reconciled.ok() && YAML::Dump(saved) == YAML::Dump(before_override) &&
                saved_fingerprint.ok() && expected_fingerprint.ok() && *saved_fingerprint == *expected_fingerprint &&
                hm::get_node(same_count.config(), "stitching.calibration_frame_selection").has_value() &&
                same_count.config()["stitching"]["calibration_frame_count"].as<int>() == 4 &&
                same_count.config()["hstream_ui"]["stitching_calibration"]["frame_count"].as<int>() == 4 &&
                same_count.config()["pipeline"]["hmstitcher"]["calibration-frame-count"].as<int>() == 4 &&
                !hm::get_node(same_count.config(), "pipeline.hmstitcher.calibration_frame_count").has_value(),
            "An explicit count matching the plan must retain it and normalize runtime aliases without saved count "
            "bookkeeping or source media");
      }
    }
  ...
  const fs::path generated_backend_alias_dir = games / "generated-backend-alias";
  fs::create_directories(generated_backend_alias_dir);
  YAML::Node generated_backend_alias_private(YAML::NodeType::Map);
  generated_backend_alias_private["stitching"]["control_point_matcher"] = "native-aliked-lightglue";
  generated_backend_alias_private["stitching"]["mapping_backend"] = "MAGSAC++";
  generated_backend_alias_private["stitching"]["projection"] = "rectilinear";
  generated_backend_alias_private["stitching"]["run_autooptimizer"] = false;
  generated_backend_alias_private["stitching"]["projection_parameters"]["general-panini"] = YAML::Load("[111, 1, -1]");
  YAML::Node generated_backend_alias_choices =
      generated_backend_alias_private["hstream_ui"]["generated_stitching_backend_choices"];
  ...
  generated_backend_alias_choices["previous_projection_parameters"] = YAML::Load("[111, 1, -1]");
  const absl::Status generated_backend_alias_published = hm::stitching::publish_game_config(
      generated_backend_alias_dir, YAML::Dump(generated_backend_alias_private) + "\n");
  hm::Configurator generated_backend_alias_configurator(
      "generated-backend-alias", backend_partial_baseline_root.string(), hm::Configurator::kUseConfigFileGpu);
  const absl::Status generated_backend_alias_configured = generated_backend_alias_configurator.configure();
  const absl::Status generated_backend_alias_persisted =
      generated_backend_alias_configurator.persist_effective_stitching_backend_choices();
  auto generated_backend_alias_final =
      hm::stitching::load_game_config_file(generated_backend_alias_dir / "config.yaml");
  ok &= expect(
      generated_backend_alias_published.ok() && generated_backend_alias_configured.ok() &&
          generated_backend_alias_persisted.ok() && generated_backend_alias_final.ok() &&
          generated_backend_alias_final->has_value() &&
          (**generated_backend_alias_final)["stitching"]["mapping_backend"].as<std::string>() == "nona" &&
          (**generated_backend_alias_final)["stitching"]["projection"].as<std::string>() == "general-panini" &&
          (**generated_backend_alias_final)["stitching"]["projection_parameters"]["general-panini"][0].as<double>() ==
              111.0 &&
          !hm::get_node(**generated_backend_alias_final, "hstream_ui.generated_stitching_backend_choices").has_value(),
