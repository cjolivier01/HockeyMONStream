...
int main(int argc, char** argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (argument == "--print-targets")
      ...
      std::cout << "usage: hmstream-assets [--print-targets] CONFIG.yaml...\n";
      return 0;
    } else
  ...
  if (print_targets) {
    auto assets = hm::assets::AssetManager::Discover(configs);
    if (!assets.ok()) {
      std::cerr << assets.status() << '\n';
      return assets.status().raw_code();
    }
    for (const auto& asset : *assets) std::cout << asset.target.string() << '\n';
    return 0;
  }
