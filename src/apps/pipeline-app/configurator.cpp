...
void set_all_field_values(const YAML::Node& parent,
                          const std::string& field_name,
                          const std::string& field_value, bool only_if_exists) {
...
std::optional<std::tuple<int, int>> get_canvas_size(
    const std::string& game_dir) {
  hm::pano::ControlMasks control_masks(game_dir);
  if (!control_masks.is_valid()) {
    return std::nullopt;
  }
  return std::make_tuple(control_masks.canvas_width(),
                         control_masks.canvas_height());
}

std::optional<YAML::Node> get_node_if_enabled(const YAML::Node& pipeline,
...
std::optional<YAML::Node> get_enabled_audio_uri(const YAML::Node& pipeline) {
  ...
  do {
    ...
    if (audio["enable"].IsDefined() && audio["enable"].as<int>() &&
  ...
  for (auto kv : pipeline) {
    ...
    if (absl::StartsWith(key, "source")) {
      ...
      if (!has_node(src_node, "source-id", /*non_null=*/true)) {
        std::cerr << "No source-id in enabled source section: " << key
                  << std::endl;
        continue;
      }
...
std::map<int, YAML::Node> replace_sink_source_id(const YAML::Node& pipeline,
                                                 int from_source_id,
                                                 int to_source_id) {
...
absl::StatusOr<std::map<int, YAML::Node>> get_camera_sources(
  ...
  for (const auto& src_iter_item : sources) {
    NvDsSourceType type =
        (NvDsSourceType)(src_iter_item.second["type"].as<int>());
    if (type == NvDsSourceType::NV_DS_SOURCE_CAMERA_CSI ||
        type == NvDsSourceType::NV_DS_SOURCE_CAMERA_V4L2) {
      if (type == NvDsSourceType::NV_DS_SOURCE_CAMERA_V4L2) {
        HM_RETURN_IF_ERROR(
            ready_camera(TO_STRING(
                "/dev/video"
                << src_iter_item.second["camera-v4l2-dev-node"].as<int>())));
      }
...
std::optional<YAML::Node> maybe_get_config_file(const YAML::Node& yaml_node,
...
Configurator::Configurator(const std::string& game_id,
                           const std::string& config_root_dir,
                           int override_gpu_id)
    : game_id_(game_id),
      config_root_dir_(config_root_dir),
...
std::vector<size_t> Configurator::enable_source_types(
    const std::set<NvDsSourceType>& source_enums, bool disable_others) {
  ...
  for (auto kv : pipeline) {
    ...
    if (absl::StartsWith(key, "source")) {
      YAML::Node src_node = kv.second;
      const NvDsSourceType type =
          static_cast<NvDsSourceType>(get_node_value(src_node, "type", 0));
      if (!type) {
...
size_t Configurator::disable_source_types(
...
std::filesystem::path Configurator::get_private_config_file_name(
...
absl::Status Configurator::save_private_config(
  ...
  if (!fout.is_open()) {
    return absl::InternalError(
        TO_STRING("Failed to open private config file for writing: \""
                  << private_config_file << "\", reason: " << strerror(errno)));
  }
  fout << private_config << "\n";
  return absl::OkStatus();
}
...
absl::StatusOr<YAML::Node> Configurator::load_config() {
  ...
  if (!config_root_dir_.empty()) {
    std::filesystem::path baseline_path =
        std::filesystem::path(config_root_dir_) / "baseline.yaml";
  ...
  if (private_config.has_value()) {
    ...
    config = merge_nodes(config, private_config_,
...
bool set_if_not_set(YAML::Node node, const std::string& dest_key,
...
bool Configurator::underlay_config(const std::string& node_name,
  ...
  if (node_name.empty()) {
    config_ = merge_nodes(underlaid_config, config_,
                          /*warn_if_key_not_in_dest=*/false);
  } else {
    // std::cout << config_ << std::endl;
    config_[node_name] = merge_nodes(underlaid_config, config_[node_name],
                                     /*warn_if_key_not_in_dest=*/false);
    // std::cout << config_ << std::endl;
  }
  return true;
...
bool Configurator::overlay_config(const std::string& node_name,
                                  const std::string& filename) {
  if (!std::filesystem::exists(filename)) {
    return false;
  }
  ...
  if (node_name.empty()) {
    config_ = merge_nodes(config_, overlaid_config,
                          /*warn_if_key_not_in_dest=*/false);
  } else {
    config_[node_name] = merge_nodes(config_[node_name], overlaid_config,
                                     /*warn_if_key_not_in_dest=*/false);
  }
  // std::cout << config_ << std::endl;
  return true;
...
YAML::Node Configurator::merge_nodes(const YAML::Node& base,
                                     const YAML::Node& overlay,
                                     bool warn_if_key_not_in_dest) {
  if (!overlay.IsMap()) {
    return base;
  }
  if (!base.IsMap()) {
  ...
  for (const auto& pair : overlay) {
    ...
    if (warn_if_key_not_in_dest && !base[key]) {
      std::cerr << "Warning: Key '" << key
                << "' in overlay does not exist in base config\n";
    }

    // If both are maps, recursively merge
    if (pair.second.IsMap() && base[key].IsDefined() && base[key].IsMap()) {
      result[key] =
          merge_nodes(base[key], pair.second, warn_if_key_not_in_dest);
    } else {
...
absl::StatusOr<bool> Configurator::does_need_stitching(
...
absl::Status Configurator::complete_configuration(bool force) {
  ...
  if (override_gpu_id_ != kUseConfigFileGpu) {
    ...
    set_all_field_values(pipeline, "gpu-id", std::to_string(override_gpu_id_),
                         /*only_if_exists=*/true);
  }
  ...
  // If no 'hmstitcher' in pipeline config, then don;t even consider it
  const bool pipeline_has_hmstitcher =
      get_node(pipeline, "hmstitcher")->IsDefined();
  ...
  if (pipeline_has_hmstitcher) {
    ...
    if (get_node_value(pipeline, "hmstitcher.enable", FALSE) &&
      ...
      bool is_configured;
      HM_ASSIGN_OR_RETURN(is_configured,
                          stitching::is_stitching_configured(game_dir));
  ...
  pipeline["ds-fieldmask"]["detection-mask"] =
      std::string(game_dir / kRinkMaskFilename);

  const std::map<std::string, std::string> map_dest_from_src{
      {"pipeline.hmplaycropper.fixed-edge-rotation-angle",
       "rink.camera.fixed_edge_rotation_angle"},
      {"pipeline.ds-playtracker.fixed-edge-rotation-angle",
       "rink.camera.fixed_edge_rotation_angle"},
  };
  ...
  const std::map<std::string, std::string> live_box_map_dest_from_src{
      {"pipeline.ds-playtracker.play-tracker.live-boxes:1",
  ...
  if (has_node(config_, "rink.scoreboard.perspective_polygon",
               /*non_null=*/true)) {
    auto points = config_["rink"]["scoreboard"]["perspective_polygon"]
  ...
  if (!is_camera_source) {
    ...
    if (videos.count("stitched")) {
      ...
      if (all_exist) {
        ...
        num_video_sources = 1;
        Videoinfo stitched_info =
            getVideoInfo(file_maybe_in_game_dir(stitched_file));
    ...
    if (stitched_file.empty()) {
      ...
      if (!force) {
        if (has_node(config_, "game.videos.left", /*non_null=*/true)) {
          left_files =
              config_["game"]["videos"]["left"].as<std::vector<std::string>>();
        }
        if (has_node(config_, "game.videos.right", /*non_null=*/true)) {
          right_files =
              config_["game"]["videos"]["right"].as<std::vector<std::string>>();
        }
      }

      if (left_files.empty() && right_files.empty() && !videos.count("left") &&
          !videos.count("right")) {
      ...
      if (left_files.empty() && right_files.empty() && videos.count("left") &&
          ...
          for (const auto& item : left_chapter) {
            if (!right_chapter.empty()) {
              ...
              if (!right_chapter.count(chapter)) {
                std::cerr << "Right vids are missing chapter " << chapter
                          << ", skipping..." << std::endl;
                continue;
              }
            }
          ...
          for (const auto& item : right_chapter) {
            if (!left_chapter.empty()) {
              ...
              if (!left_chapter.count(chapter)) {
                std::cerr << "Left vids are missing chapter " << chapter
                          << ", skipping..." << std::endl;
                continue;
              }
            }
        ...
        if (!left_files.empty() && !right_files.empty()) {
          ...
          if (!spp_status.ok()) {
            ...
            std::cerr << "Warnings: failed to save private config: "
                      << spp_status << std::endl;
          }
        }
      }
      ...
      if (!left_files.empty() && !right_files.empty()) {
        if (!has_node(config_, "game.stitching.frame_offsets.left",
                      /*non_null=*/true) ||
            force) {
          stitching::Synchronization sync;
          HM_ASSIGN_OR_RETURN(
              sync, stitching::calculate_stitching_synchronization(
                        game_dir / left_files[0], game_dir / right_files[0]));
          offsets["left"] = std::to_string(sync.video1_frame_offset);
          offsets["right"] = std::to_string(sync.video2_frame_offset);
          private_config_["game"]["stitching"]["frame_offsets"]["left"] =
              std::to_string(sync.video1_frame_offset);
          private_config_["game"]["stitching"]["frame_offsets"]["right"] =
              std::to_string(sync.video2_frame_offset);
          auto spp_status = save_private_config(private_config_);
          if (!spp_status.ok()) {
            // We can continue, so just warn
            std::cerr << "Warnings: failed to save private config: "
      ...
      if (!left_files.empty()) {
        Videoinfo left_info =
            getVideoInfo(file_maybe_in_game_dir(left_files[0]));
        ...
        pipeline["hmstitcher"]["left-frame-offset-ns"] =
            std::to_string(size_t(lfo / left_info.fps * GST_SECOND));
        area = left_info.width * left_info.height;
        ww = left_info.width;
        hh = left_info.height;
      }
      if (!right_files.empty()) {
        Videoinfo right_info =
            getVideoInfo(file_maybe_in_game_dir(right_files[0]));
        ...
        pipeline["hmstitcher"]["right-frame-offset-ns"] =
            std::to_string(size_t(rfo / right_info.fps * GST_SECOND));
        ...
        if (rfo == 0.0) {
          ...
          // Change the source id of sinks that reference the left video to now
          // reference the right video
          if (lfo != 0.0) {
  ...
  auto maybe_scale_down = [is_udp_output](long width,
                                          long height) -> std::tuple<int, int> {
    if (!is_udp_output) {
      return std::make_tuple(width, height);
    }
    // 4k @ 16:9
    return resize_to_fit(width, height, kMaxUdpStreamingWidth,
  ...
  if (is_camera_source) {
    ...
    pipeline["hmplaycropper"]["output-width"] =
        std::to_string(std::get<0>(wh_tuple));
    pipeline["hmplaycropper"]["output-height"] =
        std::to_string(std::get<1>(wh_tuple));
  } else if (!left_files.empty() && !right_files.empty() && pipeline_has_hmstitcher) {
    ...
    if (canvas_size_result) {
      ...
      auto wh_tuple = maybe_scale_down(static_cast<long>(ar * canvas_height),
                                       canvas_height);
      pipeline["hmplaycropper"]["output-width"] =
          std::to_string(std::get<0>(wh_tuple));
      pipeline["hmplaycropper"]["output-height"] =
      ...
      // We don't know the canvas size since the mapping hasn't been generatewd
      // yet, so we'll have to
      bool configure_only =
          get_node_value(pipeline, "hmstitcher.configure-only", false);
      if (!configure_only) {
        return absl::FailedPreconditionError(
            "Unable to determine the canvas size and stitcher is not set to "
            "configure-only");
      }
      std::cout << "The stitched canvas size is not yet known, will determine "
                   "in the ensuing pipeline run"
                << std::endl;
    }
  } else {
    auto wh_tuple = maybe_scale_down(ww, hh);
  ...
  if (!left_files.empty() && !right_files.empty()) {
    ...
    if (src0.IsDefined() && as_int(src0["enable"]) &&
        as_int(src0["type"]) == NV_DS_SOURCE_URI_MULTIPLE && src1.IsDefined() &&
        as_int(src1["enable"]) &&
        as_int(src1["type"]) == NV_DS_SOURCE_URI_MULTIPLE) {
    ...
    if (src0.IsDefined() && as_int(src0["enable"]) &&
      ...
      if (!src0["uri"].IsDefined() || src0["uri"].IsNull() ||
          src0["uri"].as<std::string>().empty()) {
        std::string stiched_output =
            file_maybe_in_game_dir("stitched_output-with-audio.mp4");
        if (std::filesystem::exists(stiched_output)) {
          src0["uri"] = ff + stiched_output;
          disable_source_types({NvDsSourceType::NV_DS_SOURCE_URI,
                                NvDsSourceType::NV_DS_SOURCE_URI_MULTIPLE});
          src0["enabled"] = "1";
        }
      }
      if (src0["uri"].IsDefined() && !src0["uri"].IsNull()) {
  ...
  if (!possible_audio_uri.empty() ||
      audio_source_id != std::numeric_limits<size_t>::max()) {
    ...
    if (audio_uri_opt.has_value()) {
      ...
      if (!audio_uri["uri"].IsDefined() ||
          !is_valid_yaml_value_string(audio_uri["uri"].as<std::string>())) {
    ...
    for (size_t hmaudio_index = 0; hmaudio_index < INT_MAX; ++hmaudio_index) {
      ...
      if (audio_source_id != std::numeric_limits<size_t>::max() &&
        ...
        if (!(*audio_uri_opt)["source-id"].IsDefined() ||
            !is_valid_yaml_value_string(
                (*audio_uri_opt)["source-id"].as<std::string>())) {
        ...
        if (!is_defined ||
            !is_valid_yaml_value_string(audio_uri[key].as<std::string>())) {
...
absl::Status Configurator::apply_config_item(const std::string& key,
  ...
  //  std::cout << config_ << std::endl;
  config_ =
      merge_nodes(config_, overlaid_config, /*warn_if_key_not_in_dest=*/false);
  // std::cout << config_ << std::endl;
  return absl::OkStatus();
}

absl::Status Configurator::post_config_pipeline(NvDsPipeline& pipeline,
                                                const NvDsConfig& config,
                                                uint64_t start_time_ns) {
  // We need to do this get state for some reason
  GstState state, pending;
  gst_element_get_state(pipeline.pipeline, &state, &pending,
                        GST_CLOCK_TIME_NONE);
  if (state == GST_STATE_READY) {
    ...
    return absl::InternalError(
        TO_STRING("Pipeline in unexpected state: " << gstStateToString(state)));
  }
  ...
  if (src_bins.size() == 2) {
    if (config.hmsticher_config.left_frame_offset_ns || start_time_ns) {
      bool result = seek_element(
          src_bins[0],
          config.hmsticher_config.left_frame_offset_ns + start_time_ns);
      if (!result) {
    ...
    if (config.hmsticher_config.right_frame_offset_ns || start_time_ns) {
      ...
      bool result = seek_element(
          src_bins[1],
          config.hmsticher_config.right_frame_offset_ns + start_time_ns);
      if (!result) {
  ...
  for (size_t i = 0; i < MAX_SOURCE_BINS; ++i) {
    if (pipeline.instance_bins[i].hmaudio_bin.bin && start_time_ns) {
      bool result = seek_element(pipeline.instance_bins[i].hmaudio_bin.bin,
                                 start_time_ns);
      if (!result) {
...
std::string get_section_prefix(const std::string& section_name) {
  auto found_digit =
      std::find_if(section_name.begin(), section_name.end(), ::isdigit);
...
absl::Status Configurator::load_sub_configs(
  ...
  std::map<std::string, std::vector<std::pair<std::string, YAML::Node>>>
      subconfigs;
  ...
  for (const auto& node : parent_node) {
    ...
    if (!all_existing_fields.emplace(key).second) {
      return absl::InvalidArgumentError(
          TO_STRING("Found duplicate YAML section: " << key));
    }
    for (const std::string& prefix : allowed_prefixes) {
      if (!strncmp(key.c_str(), prefix.c_str(), prefix.size())) {
        // Have a prefix match
        std::optional<YAML::Node> subconfig_node =
            maybe_get_config_file(node.second, config_path);
  ...
  auto get_next_available_name =
      [&all_existing_fields](const std::string& prefix) {
        size_t c = 0;
        std::string sn;
        do {
  ...
  for (const auto& sc_item : subconfigs) {
    for (const auto& subitem : sc_item.second) {
      ...
      for (const auto& subsection_section : section_config) {
        ...
        std::string new_subsection_name =
            get_next_available_name(subsection_prefix);
        all_existing_fields.emplace(new_subsection_name);
        YAML::Node subsection_config = subsection_section.second;
        merge_nodes(subsection_config, YAML::Clone(parent_node[section_name]),
                    /*warn_if_key_not_in_dest=*/false);
