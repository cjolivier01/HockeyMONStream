  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_LIST_NUM_SOURCE_BINS)) {
      config->total_num_sources =
          g_key_file_get_integer(key_file, CONFIG_GROUP_SOURCE_LIST, CONFIG_GROUP_SOURCE_LIST_NUM_SOURCE_BINS, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_LIST_URI_LIST)) {
      config->uri_list = g_key_file_get_string_list(
          key_file, CONFIG_GROUP_SOURCE_LIST, CONFIG_GROUP_SOURCE_LIST_URI_LIST, &num_strings_uri, &error);
      ...
      config->sensor_id_list = g_key_file_get_string_list(
          key_file, CONFIG_GROUP_SOURCE_LIST, CONFIG_GROUP_SOURCE_LIST_SENSOR_ID_LIST, &num_strings_sensor_id, &error);
      ...
      config->stream_name_display = g_key_file_get_boolean(
          key_file, CONFIG_GROUP_SOURCE_LIST, CONFIG_GROUP_SOURCE_LIST_STREAM_NAME_DISPLAY, &error);
      CHECK_ERROR(error);
      ...
      config->max_batch_size =
          g_key_file_get_integer(key_file, CONFIG_GROUP_SOURCE_LIST, CONFIG_GROUP_SOURCE_LIST_MAX_BATCH_SIZE, &error);
      CHECK_ERROR(error);
      ...
      config->extract_sei_type5_data = g_key_file_get_integer(
          key_file, CONFIG_GROUP_SOURCE_LIST, CONFIG_GROUP_SOURCE_EXTRACT_SEI_TYPE5_DATA, &error);
      CHECK_ERROR(error);
    } else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_LIST_LOW_LATENCY_MODE)) {
      config->low_latency_mode =
          g_key_file_get_integer(key_file, CONFIG_GROUP_SOURCE_LIST, CONFIG_GROUP_SOURCE_LIST_LOW_LATENCY_MODE, &error);
      CHECK_ERROR(error);
...
static gboolean set_source_all_configs(NvDsConfig* config, const gchar* cfg_file_path) {
  ...
  for (i = 0; i < config->total_num_sources; i++) {
    ...
    if (config->uri_list) {
      ...
      if (!uri) {
        NVGSTDS_ERR_MSG_V(
            "uri %d entry of list is NULL, use valid uri separated by ';' with the source-list section", (i + 1));
        return FALSE;
      }
      if (g_str_has_prefix(config->uri_list[i], "file://")) {
        ...
        config->multi_source_config[i].uri =
            g_strdup_printf("file://%s", get_absolute_file_path(cfg_file_path, config->multi_source_config[i].uri));
        ...
        if (source_id_start_ptr == source_id_end_ptr || *source_id_end_ptr != '\0') {
          NVGSTDS_ERR_MSG_V("Incorrect URI for camera source %s. FORMAT: <usb/csi>:<dev_node/sensor_id>", uri);
          return FALSE;
        }
...
static gboolean parse_tests(NvDsConfig* config, GKeyFile* key_file) {
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_TESTS_FILE_LOOP)) {
      config->file_loop = g_key_file_get_integer(key_file, CONFIG_GROUP_TESTS, CONFIG_GROUP_TESTS_FILE_LOOP, &error);
      CHECK_ERROR(error);
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_APP_ENABLE_PERF_MEASUREMENT)) {
      ...
      config->bbox_dir_path = get_absolute_file_path(
          cfg_file_path, g_key_file_get_string(key_file, CONFIG_GROUP_APP, CONFIG_GROUP_APP_GIE_OUTPUT_DIR, &error));
      CHECK_ERROR(error);
      ...
      config->terminated_track_output_path = get_absolute_file_path(
          cfg_file_path,
          g_key_file_get_string(key_file, CONFIG_GROUP_APP, CONFIG_GROUP_APP_TERMINATED_TRACK_OUTPUT_DIR, &error));
      CHECK_ERROR(error);
...
extern "C" gboolean parse_config_file(NvDsConfig* config, const gchar* cfg_file_path) {
  ...
  for (group = groups; *group; group++) {
    ...
    if (!strncmp(*group, CONFIG_GROUP_SOURCE, sizeof(CONFIG_GROUP_SOURCE) - 1)) {
      ...
      if (config->source_list_enabled) {
        if (index >= config->total_num_sources) {
          NVGSTDS_ERR_MSG_V("Invalid source group index %d, index cannot exceed %d", index, config->total_num_sources);
          ret = FALSE;
          goto done;
        }
        source_id = index;
      ...
      if (config->source_list_enabled && config->multi_source_config[source_id].type == NV_DS_SOURCE_URI_MULTIPLE) {
    ...
    if (!g_strcmp0(*group, CONFIG_GROUP_PRIMARY_GIE)) {
      ...
      parse_err = !parse_gie(&config->primary_gie_config, cfg_file, (gchar*)CONFIG_GROUP_PRIMARY_GIE, cfg_file_path);
    }
    ...
    if (!strncmp(*group, CONFIG_GROUP_SECONDARY_GIE, sizeof(CONFIG_GROUP_SECONDARY_GIE) - 1)) {
       ...
       * it will override the value set using global_gpu_id in parse_gie function */
      parse_err = !parse_gie(
          &config->secondary_gie_sub_bin_config[config->num_secondary_gie_sub_bins], cfg_file, *group, cfg_file_path);
    ...
    if (!strncmp(*group, CONFIG_GROUP_SECONDARY_PREPROCESS, sizeof(CONFIG_GROUP_SECONDARY_PREPROCESS) - 1)) {
      ...
      parse_err = !parse_preprocess(
          &config->secondary_preprocess_sub_bin_config[config->num_secondary_preprocess_sub_bins],
          cfg_file,
          *group,
          cfg_file_path);
    ...
    if (!strncmp(*group, CONFIG_GROUP_MSG_CONSUMER, sizeof(CONFIG_GROUP_MSG_CONSUMER) - 1)) {
      ...
      parse_err = !parse_msgconsumer(
          &config->message_consumer_config[config->num_message_consumers], cfg_file, *group, cfg_file_path);
    ...
    if (!strncmp(*group, CONFIG_GROUP_DEWARPER, strlen(*group) - 1)) {
      ...
      parse_err =
          !parse_dewarper(&config->multi_source_config[source_id].dewarper_config, cfg_file, cfg_file_path, *group);
