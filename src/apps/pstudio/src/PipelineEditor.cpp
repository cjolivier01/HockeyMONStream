...
void PipelineEditor::setPipelineModel(GstPipelineModel *model)
{
    ...
    if (m_pipelineModel) {
        disconnect(m_pipelineModel, &GstPipelineModel::pipelineModified,
                  this, &PipelineEditor::onPipelineModified);
    }
    ...
    if (m_pipelineModel) {
        connect(m_pipelineModel, &GstPipelineModel::pipelineModified,
                this, &PipelineEditor::onPipelineModified);
    }
...
void PipelineEditor::addElement(const QString &factoryName)
{
    ...
    if (!m_pipelineModel->addElement(factoryName)) {
        QMessageBox::warning(this, "Error", 
                            "Failed to add element: " + m_pipelineModel->lastError());
        return;
    }
...
void PipelineEditor::onConnectionRequest(const QString &srcElement, const QString &dstElement)
{
    ...
    if (!m_pipelineModel->connectElements(srcElement, "src", dstElement, "sink")) {
        QMessageBox::warning(this, "Error", 
                            "Failed to connect elements: " + m_pipelineModel->lastError());
        return;
    }
...
void PipelineEditor::onConnectionRemoveRequest(const QString &srcElement, const QString &dstElement)
{
    ...
    if (!m_pipelineModel->disconnectElements(srcElement, "src", dstElement, "sink")) {
        QMessageBox::warning(this, "Error", 
                            "Failed to disconnect elements: " + m_pipelineModel->lastError());
        return;
    }
...
void PipelineEditor::updateScene()
{
    ...
    for (int i = 0; i < elements.size(); ++i) {
        ...
        // Connect signals
        connect(widget, &GstElementWidget::moved, 
                this, &PipelineEditor::onElementMoved);
        connect(widget, &GstElementWidget::selected, 
                this, &PipelineEditor::onElementSelected);
        connect(widget, &GstElementWidget::connectionRequested, 
                this, &PipelineEditor::onConnectionRequest);
        connect(widget, &GstElementWidget::connectionRemoveRequested, 
                this, &PipelineEditor::onConnectionRemoveRequest);
