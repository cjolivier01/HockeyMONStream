...
bool MainWindow::loadPipelineFromString(const QString &pipelineString)
{
    ...
    if (success) {
        ...
        m_logViewer->append("Failed to load pipeline from string: " + 
                            m_pipelineModel->lastError());
    }
    return success;
}
...
void MainWindow::runPipeline()
{
    ...
    if (!m_pipelineModel->isBuilt()) {
        if (!m_pipelineModel->buildPipeline()) {
            m_logViewer->append("Failed to build pipeline: " + 
                                m_pipelineModel->lastError());
            return;
        }
    }
    ...
    if (m_pipelineModel->setState(GST_STATE_PLAYING)) {
        ...
        m_logViewer->append("Failed to start pipeline: " + 
                            m_pipelineModel->lastError());
    }
}
...
void MainWindow::stopPipeline()
{
    ...
    if (m_pipelineModel->setState(GST_STATE_NULL)) {
        ...
        m_logViewer->append("Failed to stop pipeline: " + 
                            m_pipelineModel->lastError());
    }
}
...
void MainWindow::pausePipeline()
{
    ...
    if (m_pipelineModel->setState(GST_STATE_PAUSED)) {
        ...
        m_logViewer->append("Failed to pause pipeline: " + 
                            m_pipelineModel->lastError());
    }
}
...
void MainWindow::showPipelineDot()
{
    if (!m_pipelineModel->isBuilt()) {
        if (!m_pipelineModel->buildPipeline()) {
            m_logViewer->append("Failed to build pipeline for visualization: " + 
                                m_pipelineModel->lastError());
            return;
        }
    }
...
void MainWindow::updatePipelineStatusMessage()
{
    ...
    QString posStr = "Position: " + 
                     QString::number(position / GST_SECOND) + "." +
                     QString::number((position % GST_SECOND) / (GST_SECOND / 100), 10, 2).rightJustified(2, '0') +
                     " s";
                     
    if (duration > 0) {
        posStr += " / " +
                 QString::number(duration / GST_SECOND) + "." +
                 QString::number((duration % GST_SECOND) / (GST_SECOND / 100), 10, 2).rightJustified(2, '0') +
                 " s";
    }
...
void MainWindow::setupConnections()
{
    ...
    connect(m_pipelineEditor, &PipelineEditor::elementSelected, 
            this, &MainWindow::showElementProperties);
    
    // Connect ElementLibrary signals to PipelineEditor slots
    connect(m_elementLibrary, &ElementLibrary::elementAdded,
            m_pipelineEditor, &PipelineEditor::addElement);
    
    // Connect PipelineModel signals to slots
    connect(m_pipelineModel, &GstPipelineModel::stateChanged,
            this, &MainWindow::onPipelineStateChanged);
    connect(m_pipelineModel, &GstPipelineModel::errorOccurred,
            this, &MainWindow::onPipelineError);
}
