...
ElementLibrary::ElementLibrary(QWidget* parent)
  ...
  // Connect signals
  connect(m_searchEdit, &QLineEdit::textChanged, this, &ElementLibrary::onSearchTextChanged);
  connect(m_elementList, &QListWidget::itemDoubleClicked, this, &ElementLibrary::onElementDoubleClicked);
}
...
QString determineElementCategory(GstElementFactory* factory) {
  ...
  if (klassStr.contains("Source") || klassStr.contains("Producer")) {
    ...
    category = "Sink";
  } else if (klassStr.contains("Filter") || klassStr.contains("Effect") || klassStr.contains("Transform")) {
    category = "Filter";
  } else if (klassStr.contains("Codec") || klassStr.contains("Converter") || klassStr.contains("Formatter")) {
    category = "Converter";
...
void ElementLibrary::loadElements() {
  ...
  QStringList categories = {"Source", "Sink", "Filter", "Converter", "Encoder", "Decoder", "Other"};
...
void ElementLibrary::updateElementList(const QString& filter) {
  ...
  for (GList* f = factories; f; f = f->next) {
    ...
    if (!filter.isEmpty() && !factoryName.contains(filter, Qt::CaseInsensitive) &&
