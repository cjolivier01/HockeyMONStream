...
bool GstPipelineModel::setElementProperty(const QString &elementName, const QString &property, const QVariant &value)
{
    ...
    if (!param) {
        setLastError(QString("Property '%1' not found on element '%2'")
                        .arg(property, elementName));
        return false;
    }
    
    // Skip properties that can't be written
    if (!(param->flags & G_PARAM_WRITABLE)) {
        setLastError(QString("Property '%1' on element '%2' is not writable")
                        .arg(property, elementName));
        return false;
    }
    ...
    if (!success) {
        ...
        setLastError(QString("Failed to convert value for property '%1' on element '%2'")
                        .arg(property, elementName));
        return false;
    }
...
bool GstPipelineModel::connectElements(const QString &srcElement, const QString &srcPad,
                                     const QString &sinkElement, const QString &sinkPad)
{
...
bool GstPipelineModel::disconnectElements(const QString &srcElement, const QString &srcPad,
                                        const QString &sinkElement, const QString &sinkPad)
{
    // Find the connection
    QPair<QString, QString> sink(sinkElement, sinkPad);
    ...
    if (index == -1) {
        setLastError(QString("Connection not found: %1:%2 -> %3:%4")
                        .arg(srcElement, srcPad, sinkElement, sinkPad));
        return false;
    }
...
QString GstPipelineModel::toDotFormat() const
{
    ...
    gchar *dotString = gst_debug_bin_to_dot_data(GST_BIN(m_pipeline), 
...
void GstPipelineModel::handleBusMessage(GstMessage *message)
{
    ...
    case GST_MESSAGE_ERROR: {
        ...
        QString errorMsg = QString("Error from element %1: %2")
                            .arg(GST_OBJECT_NAME(message->src), err->message);
    ...
    case GST_MESSAGE_WARNING: {
        ...
        QString warningMsg = QString("Warning from element %1: %2")
                            .arg(GST_OBJECT_NAME(message->src), err->message);
