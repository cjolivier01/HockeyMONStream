...
static GstFlowReturn gst_dsexample_submit_input_buffer(
    GstBaseTransform* btrans,
    gboolean discont,
    GstBuffer* inbuf);
static GstFlowReturn gst_dsexample_generate_output(
    GstBaseTransform* btrans,
    GstBuffer** outbuf);
...
static gboolean gst_dsexample_start(GstBaseTransform* btrans) {
  ...
  if (NvBufSurfaceCreate(
          &dsexample->inter_buf, dsexample->max_batch_size, &create_params) !=
      0) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    for (guint j = 0; j < dsexample->max_batch_size; j++) {
      cvmat[j] = cv::Mat(
          dsexample->processing_height, dsexample->processing_width, CV_8UC3);
    }
  ...
  for (int i = 0; i < 2; i++) {
    if (NvBufSurfaceCreate(
            &inter_buf, dsexample->max_batch_size, &create_params) != 0) {
      GST_ERROR("Error: Could not allocate internal buffer for dsexample");
      goto error;
    }
  ...
  dsexample->transform_params.dst_rect =
      new NvBufSurfTransformRect[dsexample->max_batch_size];
  dsexample->transform_params.transform_flag = NVBUFSURF_TRANSFORM_FILTER |
  ...
  dsexample->process_thread = g_thread_new(
      "dsexample-process-thread", gst_dsexample_output_loop, dsexample);
...
static GstFlowReturn scale_and_fill_data(
  ...
  dsexample->batch_insurf.surfaceList[dsexample->batch_insurf.numFilled] =
      *src_frame;
...
static gboolean convert_batch_and_push_to_process_thread(
  ...
  nvtx_str =
      "convert_buf batch_num=" + std::to_string(dsexample->current_batch_num);
...
static GstFlowReturn gst_dsexample_submit_input_buffer(
  ...
  nvtx_str = "buffer_process batch_num=" +
      std::to_string(dsexample->current_batch_num);
  eventAttrib.message.ascii = nvtx_str.c_str();
  nvtxRangeId_t buf_process_range =
      nvtxDomainRangeStartEx(dsexample->nvtx_domain, &eventAttrib);
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
      ...
      for (l_obj = frame_meta->obj_meta_list; l_obj != NULL;
        ...
        if (batch->frames.size() == dsexample->max_batch_size ||
           ...
           * GstDsExampleBatch structure can be allocated if required. */
          i = 0;
          batch.release();
          dsexample->batch_insurf.numFilled = 0;
        }
...
static gpointer gst_dsexample_output_loop(gpointer data) {
  ...
  while (!dsexample->stop) {
    ...
    if (batch->push_buffer) {
      nvtxDomainRangeEnd(
          dsexample->nvtx_domain, batch->nvtx_complete_buf_range);

      nvds_set_output_system_timestamp(
          batch->inbuf, GST_ELEMENT_NAME(dsexample));
    ...
    for (guint i = 0; i < batch->frames.size(); i++) {
      if (dsexample->process_full_frame) {
...
#ifdef WITH_OPENCV
        output =
            DsExampleProcess(dsexample->dsexamplelib_ctx, batch->cvmat[i].data);
#else
        ...
        attach_metadata_full_frame(
            dsexample, batch->frames[i].frame_meta, scale_ratio, output, i);
        free(output);
      } else {
