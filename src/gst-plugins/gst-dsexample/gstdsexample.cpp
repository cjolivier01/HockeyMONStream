...
static GstFlowReturn get_converted_mat(
  ...
  if (dsexample->is_integrated) {
    ...
    if (USE_EGLIMAGE) {
      ...
      auto egl_image = dsexample->inter_buf->surfaceList[0].mappedAddr.eglImage;

      cudaGraphicsResource* cuResource{nullptr};
      cudaError_t result = cudaGraphicsEGLRegisterImage(&cuResource, egl_image, CU_GRAPHICS_MAP_RESOURCE_FLAGS_NONE);
      ...
      result = cudaGraphicsResourceGetMappedEglFrame(&eglFrame, cuResource, 0, 0);
      ...
      result = cudaGraphicsUnregisterResource(cuResource);
      if (result != cudaError_t::cudaSuccess) {
        std::cerr << "FAILED!!!" << std::endl;
        assert(false);
      }
      ...
      auto nvresult = NvBufSurfaceUnMapEglImage(dsexample->inter_buf, 0);
      if (nvresult != 0) {
        std::cerr << "FAILED!!!" << std::endl;
        assert(false);
      }
  ...
  if (dsexample->process_full_frame) {
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next) {
      ...
      output = DsExampleProcess(
          dsexample->dsexamplelib_ctx, (unsigned char*)dsexample->inter_buf->surfaceList[0].mappedAddr.addr[0]);
#endif
