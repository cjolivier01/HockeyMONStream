...
static GstFlowReturn gst_ds_example_submit_input_buffer(
  ...
  if (dsexample->batch_size &&
      input.pBatchMeta->num_frames_in_batch > dsexample->batch_size) {
    GST_ELEMENT_ERROR(
        dsexample,
        STREAM,
        FAILED,
...
//     dsexample->trackerIface->waitForCompletion(inputParams); if (status ==
//     CompletionStatus_OK && dsexample->running)
//     {
