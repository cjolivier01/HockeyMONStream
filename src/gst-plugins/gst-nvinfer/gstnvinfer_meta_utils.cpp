...
void attach_metadata_detector(
  ...
  for (guint i = 0; i < detection_output.numObjects; i++) {
     ...
     * scaled during input. */
    obj.left =
        (obj.left - frame.offset_left) / frame.scale_ratio_x + frame.roi_left;
    obj.top =
        (obj.top - frame.offset_top) / frame.scale_ratio_y + frame.roi_top;
    ...
    if (nvinfer->input_tensor_from_meta && nvinfer->clip_object_outside_roi) {
      if (obj.left + obj.width >=
          frame.roi_meta->roi.left + frame.roi_meta->roi.width) {
        obj.width =
            frame.roi_meta->roi.left + frame.roi_meta->roi.width - obj.left;
      }
      if (obj.top + obj.height >=
          frame.roi_meta->roi.top + frame.roi_meta->roi.height) {
        obj.height =
            frame.roi_meta->roi.top + frame.roi_meta->roi.height - obj.top;
      }
    ...
    if (nvinfer->crop_objects_to_roi_boundary) {
      ...
      if (obj.top + obj.height >
        ...
        obj.height = frame.input_surf_params->height -
            filter_params.roiBottomOffset - obj.top;
    } else {
      if (obj.top < filter_params.roiTopOffset)
        continue;
      if (obj.top + obj.height >
 ...
 * frames, need to attach a new metadata. Assume only one label per object is
 * generated.
 */
void attach_metadata_classifier(
    GstNvInfer* nvinfer,
    GstMiniObject* tensor_out_object,
    GstNvInferFrame& frame,
 ...
 */
void merge_classification_output(
  ...
  history.cached_info.attributes.assign(
      new_result.attributes.begin(), new_result.attributes.end());
  for (auto& attr : history.cached_info.attributes) {
    attr.attributeLabel =
...
static gpointer copy_segmentation_meta(gpointer data, gpointer user_data) {
  ...
  meta->class_probabilities_map = (gfloat*)g_memdup2(
      src_meta->class_probabilities_map,
...
void attach_metadata_segmentation(
    GstNvInfer* nvinfer,
    GstMiniObject* tensor_out_object,
    GstNvInferFrame& frame,
...
static void release_user_meta_at_frame_level(
    gpointer data,
    gpointer user_data) {
  NvDsUserMeta* user_meta = (NvDsUserMeta*)data;
...
void attach_tensor_output_meta(
    GstNvInfer* nvinfer,
    GstMiniObject* tensor_out_object,
  ...
  for (size_t j = 0; j < batch->frames.size(); j++) {
    ...
    if (nvinfer->input_tensor_from_meta) {
      ...
      user_meta->base_meta.meta_type = (NvDsMetaType)NVDS_ROI_META;
