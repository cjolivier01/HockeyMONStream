...
#define DS_NVINFER_IMPL(gst_nvinfer) \
  reinterpret_cast<DsNvInferImpl*>((gst_nvinfer)->impl)
...
static GstStaticPadTemplate gst_nvinfer_sink_template = GST_STATIC_PAD_TEMPLATE(
    "sink",
    GST_PAD_SINK,
    GST_PAD_ALWAYS,
...
static GstStaticPadTemplate gst_nvinfer_src_template = GST_STATIC_PAD_TEMPLATE(
    "src",
    GST_PAD_SRC,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS(GST_VIDEO_CAPS_MAKE_WITH_FEATURES(
...
static void gst_nvinfer_set_property(
    GObject* object,
    guint prop_id,
    const GValue* value,
    GParamSpec* pspec);
static void gst_nvinfer_get_property(
    GObject* object,
    guint prop_id,
    GValue* value,
    GParamSpec* pspec);
...
static gboolean gst_nvinfer_sink_event(
    GstBaseTransform* trans,
    GstEvent* event);

static GstFlowReturn gst_nvinfer_submit_input_buffer(
    GstBaseTransform* btrans,
    gboolean discont,
    GstBuffer* inbuf);
static GstFlowReturn gst_nvinfer_generate_output(
    GstBaseTransform* btrans,
    GstBuffer** outbuf);
...
static GType gst_nvinfer_process_mode_get_type(void) {
  ...
  if (g_once_init_enter(&process_mode_type)) {
    GType tmp =
        g_enum_register_static("GstNvInferProcessModeType", process_mode);
 ...
 * element.
 */
static void gst_nvinfer_class_init(GstNvInferClass* klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum(
          ...
          GST_TYPE_NVDSINFER_PROCESS_MODE,
  ...
  gst_nvinfer_signals[SIGNAL_MODEL_UPDATED] = g_signal_new(
      "model-updated",
  ...
  gst_element_class_add_pad_template(
      gstelement_class, gst_static_pad_template_get(&gst_nvinfer_src_template));
  gst_element_class_add_pad_template(
      gstelement_class,
...
static void gst_nvinfer_init(GstNvInfer* nvinfer) {
...
static void gst_nvinfer_set_property(
  ...
  switch (prop_id) {
    ...
    case PROP_MODEL_ENGINEFILE: {
      ...
      g_strlcpy(
          impl->m_InitParams->modelEngineFilePath,
...
static void gst_nvinfer_get_property(
  ...
  switch (prop_id) {
    ...
    case PROP_OUTPUT_WRITE_TO_FILE:
      ...
      break;
    case PROP_OUTPUT_CALLBACK:
      g_value_set_pointer(value, (gpointer)nvinfer->output_generated_callback);
      break;
    case PROP_OUTPUT_CALLBACK_USERDATA:
...
void gst_nvinfer_logger(
  ...
  switch (log_level) {
    case NVDSINFER_LOG_ERROR:
      GST_ERROR_OBJECT(
          nvinfer, "NvDsInferContext[UID %d]: %s", unique_id, log_message);
      return;
...
static void gst_nvinfer_reset_init_params(GstNvInfer* nvinfer) {
  ...
  if (nvinfer->is_prop_set->at(PROP_MODEL_ENGINEFILE))
    g_strlcpy(
        impl->m_InitParams->modelEngineFilePath,
        prev_params->modelEngineFilePath,
        _PATH_MAX);
...
static gboolean gst_nvinfer_sink_event(
  ...
  if ((GstNvDsCustomEventType)GST_EVENT_TYPE(event) ==
    ...
    gst_nvevent_parse_infer_interval_update(
        event, &stream_id, &nvinfer->interval);
    g_mutex_unlock(&nvinfer->process_lock);
  }
...
static gboolean gst_nvinfer_start(GstBaseTransform* btrans) {
  ...
  /* Should not infer on objects smaller than MIN_INPUT_OBJECT_WIDTH x
  ...
  if (prop.integrated) {
    if (nvinfer->transform_config_params.compute_mode ==
  ...
  /* Set the number of output buffers that should be allocated by
   * NvDsInferContext. Should allocate more buffers if the output tensor buffers
   * will be attached as meta to GstBuffers and pushed downstream. */

  init_params->outputBufferPoolSize = std::max<uint>(
  ...
  status = createNvDsInferContext(
      &infer_context, *init_params, nvinfer, gst_nvinfer_logger);
  if (status != NVDSINFER_SUCCESS) {
    GST_ELEMENT_ERROR(
        ...
        ("Config file path: %s, NvDsInfer Error: %s",
  ...
  if (!nvinfer->input_tensor_from_meta) {
    ...
    gst_buffer_pool_config_set_params(
        config_ptr.get(),
        nullptr,
        sizeof(GstNvInferMemory),
        INTERNAL_BUF_POOL_SIZE,
        INTERNAL_BUF_POOL_SIZE);

    /* Create a new GstNvInferAllocator instance. Allocator has methods to
    ...
    std::unique_ptr<GstAllocator, decltype(allocator_deleter)> allocator_ptr(
        gst_nvinfer_allocator_new(
            nvinfer->network_width,
            nvinfer->network_height,
            color_format,
    ...
    gst_buffer_pool_config_set_allocator(
        config_ptr.get(), allocator_ptr.get(), &allocation_params);
    ...
    if (!gst_buffer_pool_set_config(pool_ptr.get(), config_ptr.release())) {
      GST_ELEMENT_ERROR(
          ...
          RESOURCE,
          FAILED,
          ("Failed to set config on buffer pool"),
          (nullptr));
      return FALSE;
    }
  ...
  cudaReturn =
      cudaStreamCreateWithFlags(&nvinfer->convertStream, cudaStreamNonBlocking);
  if (cudaReturn != cudaSuccess) {
    GST_ELEMENT_ERROR(
        nvinfer,
        RESOURCE,
  ...
  nvinfer->tmp_surf.surfaceList =
      new NvBufSurfaceParams[nvinfer->max_batch_size];
  ...
  nvinfer->transform_params.dst_rect =
      new NvBufSurfTransformRect[nvinfer->max_batch_size];
  nvinfer->transform_params.transform_flag = NVBUFSURF_TRANSFORM_FILTER |
  ...
  nvinfer->source_info->emplace(
      0, GstNvInferSourceInfo{GstNvInferObjectHistoryMap(), 0});
  ...
  if (!nvinfer->input_tensor_from_meta) {
    nvinfer->input_queue_thread = g_thread_new(
        "nvinfer-input-queue-thread", gst_nvinfer_input_queue_loop, nvinfer);
  }
  ...
  if (impl->start() != NVDSINFER_SUCCESS) {
    GST_ELEMENT_WARNING(
        ...
        RESOURCE,
        FAILED,
        ("NvInfer start loading model thread failed."),
        (nullptr));
    return FALSE;
  }
  ...
  impl->notifyLoadModelStatus(ModelStatus{
      NVDSINFER_SUCCESS,
      nvinfer->config_file_path,
 ...
 * input format.
 */
static GstFlowReturn get_converted_buffer(
    GstNvInfer* nvinfer,
...
static gpointer gst_nvinfer_input_queue_loop(gpointer data) {
  ...
  while (nvinfer->stop == FALSE) {
    ...
    nvtx_str =
        "queueInput batch_num=" + std::to_string(nvinfer->current_batch_num);
...
static gboolean convert_batch_and_push_to_input_thread(
    GstNvInfer* nvinfer,
    GstNvInferBatch* batch,
    GstNvInferMemory* mem) {
  ...
  eventAttrib.messageType = NVTX_MESSAGE_TYPE_ASCII;
  nvtx_str =
      "convert_buf batch_num=" + std::to_string(nvinfer->current_batch_num);
  eventAttrib.message.ascii = nvtx_str.c_str();
...
static GstFlowReturn gst_nvinfer_process_full_frame(
    GstNvInfer* nvinfer,
    GstBuffer* inbuf,
  ...
  for (guint i = 0; i < num_filled; i++) {
    ...
    if (batch == nullptr) {
      ...
      flow_ret =
          gst_buffer_pool_acquire_buffer(nvinfer->pool, &conv_gst_buf, nullptr);
    ...
    if (get_converted_buffer(
      ...
      GST_ELEMENT_ERROR(
          nvinfer, STREAM, FAILED, ("Buffer conversion failed"), (NULL));
      return GST_FLOW_ERROR;
    }
    ...
    if (batch->frames.size() == nvinfer->max_batch_size ||
        i == num_filled - 1) {
      if (!convert_batch_and_push_to_input_thread(
              nvinfer, batch.release(), memory)) {
        return GST_FLOW_ERROR;
      }
 ...
 * in check. */
static void cleanup_history_map(GstNvInfer* nvinfer, GstBuffer* inbuf) {
  LockGMutex locker(nvinfer->process_lock);
...
static inline gboolean should_infer_object(
    GstNvInfer* nvinfer,
    GstBuffer* inbuf,
 ...
 * is pushed downstream (from the input thread itself) without waiting for
 * results. When the infer results are available they are stored in the object
 * history map in the output loop. After the results are available the
 * new/updated results are attached (in the input thread) to the object whenever
 * it is found in the frame again. */
static GstFlowReturn gst_nvinfer_process_objects(
    GstNvInfer* nvinfer,
    GstBuffer* inbuf,
    NvBufSurface* in_surf) {
  std::unique_ptr<GstNvInferBatch> batch(nullptr);
  ...
  for (NvDsMetaList* l_frame = batch_meta->frame_meta_list; l_frame != NULL;
    ...
    for (NvDsMetaList* l_obj = frame_meta->obj_meta_list; l_obj != NULL;
      ...
      bool needs_infer = should_infer_object(
          nvinfer, inbuf, object_meta, frame_num, obj_history.get());
      if (!needs_infer) {
        /* Should not infer again. */

        if (IS_CLASSIFIER_INSTANCE(nvinfer) && obj_history != nullptr) {
          /* Working in synchronous mode. Defer attachment of classifier
           * metadata in the object history to the output thread. */
          if (!nvinfer->classifier_async_mode) {
            /* No existing GstNvInferBatch structure. Allocate a new structure,
             * acquire a buffer from our internal pool for conversions. */
            if (batch == nullptr) {
              ...
              flow_ret = gst_buffer_pool_acquire_buffer(
                  nvinfer->pool, &conv_gst_buf, nullptr);
              locker.lock();
              if (flow_ret != GST_FLOW_OK) {
                return flow_ret;
              }
            ...
            batch->objs_pending_meta_attach.emplace_back(
                obj_history, object_meta);
            continue;
          }
        }
      }
      ...
      if (obj_history && nvinfer->classifier_async_mode) {
        ...
        attach_metadata_classifier(
            nvinfer, nullptr, frame, obj_history->cached_info);
        obj_history->last_accessed_frame_num = frame_meta->frame_num;
      }
      ...
      if (source_info != nullptr &&
          object_meta->object_id != UNTRACKED_OBJECT_ID &&
          obj_history == nullptr) {
        auto ret_iter = source_info->object_history_map.emplace(
  ...
  if (nvinfer->current_batch_num - nvinfer->last_map_cleanup_frame_num >
      MAP_CLEANUP_INTERVAL) {
...
static GstFlowReturn gst_nvinfer_process_tensor_input(
    GstNvInfer* nvinfer,
    GstBuffer* inbuf,
    NvBufSurface* in_surf) {
  typedef struct {
  ...
  for (NvDsMetaList* l_user = batch_meta->batch_user_meta_list; l_user != NULL;
    ...
    nvinfer->maintain_aspect_ratio =
        preproc_meta->tensor_meta->maintain_aspect_ratio;
    ...
    if (preproc_meta->tensor_meta->gpu_id != nvinfer->gpu_id) {
      GST_ELEMENT_ERROR(
          ...
          ("nvinfer configured with gpu-id %d but received input tensor allocated on gpu %d",
           nvinfer->gpu_id,
           preproc_meta->tensor_meta->gpu_id),
          (NULL));
      return GST_FLOW_ERROR;
    }
    ...
    for (auto& layer : *nvinfer->layers_info) {
      if (layer.isInput &&
          preproc_meta->tensor_meta->tensor_name == layer.layerName) {
        layerExists = true;
        break;
      }
    }
    ...
    if (tensormeta_map[meta_id].batch_size == 0) {
      ...
      GST_ELEMENT_ERROR(
          ...
          ("Mismatch in input tensor batch sizes %d vs %d",
           tensormeta_map[meta_id].batch_size,
  ...
  for (auto& it : tensormeta_map) {
    ...
    for (size_t i = 0; i < tensor_input_batch.frames.size(); i++) {
      ...
      if (i == tensor_input_batch.frames.size() - 1 ||
        ...
        NvDsInferStatus status =
            DS_NVINFER_IMPL(nvinfer)->m_InferCtx->queueInputBatchPreprocessed(
                input_batch);
        ...
        g_mutex_lock(&nvinfer->process_lock);
        g_queue_push_tail(
            nvinfer->process_queue, tensor_input_batch.batch.release());
        g_cond_broadcast(&nvinfer->process_cond);
        g_mutex_unlock(&nvinfer->process_lock);
...
static GstFlowReturn gst_nvinfer_submit_input_buffer(
  ...
  eventAttrib.messageType = NVTX_MESSAGE_TYPE_ASCII;
  nvtx_str =
      "buffer_process batch_num=" + std::to_string(nvinfer->current_batch_num);
  eventAttrib.message.ascii = nvtx_str.c_str();
  nvtxRangeId_t buf_process_range =
      nvtxDomainRangeStartEx(nvinfer->nvtx_domain, &eventAttrib);
...
static void gst_nvinfer_output_generated_file_write(
    GstBuffer* buf,
...
static void gst_nvinfer_tensoroutput_free(GstMiniObject* obj) {
  GstNvInferTensorOutputObject* output_obj = (GstNvInferTensorOutputObject*)obj;
...
static gpointer gst_nvinfer_output_loop(gpointer data) {
  ...
  nvtx_str =
      "gst-nvinfer_output-loop_uid=" + std::to_string(nvinfer->unique_id);
  ...
  while (!nvinfer->stop) {
    ...
    if (batch->frames.size() == 0 && !batch->push_buffer) {
      for (auto& hist : batch->objs_pending_meta_attach) {
        ...
        attach_metadata_classifier(
            nvinfer, nullptr, frame, obj_history.lock()->cached_info);
      }
      continue;
    }
    ...
    gst_mini_object_init(
        GST_MINI_OBJECT(tensor_out_object.get()),
        0,
        G_TYPE_POINTER,
        NULL,
        NULL,
        gst_nvinfer_tensoroutput_free);
    ...
    if (nvinfer->output_generated_callback) {
      nvinfer->output_generated_callback(
          ...
          nvinfer->layers_info->size(),
          batch->frames.size(),
          nvinfer->output_generated_userdata);
    }
    ...
    for (guint i = 0; i < batch->frames.size(); i++) {
      ...
      if (IS_DETECTOR_INSTANCE(nvinfer) ||
          IS_INSTANCE_SEGMENTATION_INSTANCE(nvinfer)) {
        ...
        NvDsInferClassificationOutput& classification_output =
            frame_output.classificationOutput;
        GstNvInferObjectInfo new_info;
        new_info.attributes.assign(
            classification_output.attributes,
            classification_output.attributes +
                classification_output.numAttributes);
        ...
        if (nvinfer->classifier_async_mode == FALSE) {
          attach_metadata_classifier(
              nvinfer, GST_MINI_OBJECT(tensor_out_object.get()), frame, info);
        }
        ...
        attach_metadata_segmentation(
            ...
            GST_MINI_OBJECT(tensor_out_object.get()),
            frame,
            frame_output.segmentationOutput);
      }
    }
 ...
 */
static gboolean nvinfer_plugin_init(GstPlugin* plugin) {
  ...
  return gst_element_register(
      plugin, "nvinfer", GST_RANK_PRIMARY, GST_TYPE_NVINFER);
}
