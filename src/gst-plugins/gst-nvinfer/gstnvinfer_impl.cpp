...
DsNvInferImpl::DsNvInferImpl(GstNvInfer* infer)
    : m_InitParams(new NvDsInferContextInitParams), m_GstInfer(infer) {
...
NvDsInferStatus DsNvInferImpl::start() {
...
bool DsNvInferImpl::triggerNewModel(
...
void DsNvInferImpl::loadModel(
  ...
  if (!needUpdate) {
    notifyLoadModelStatus(ModelStatus{
        ...
        modelPath,
        "Trying to update model too frequently"});
    return;
  }
  ...
  NvDsInferStatus err = createNvDsInferContext(
      &newContext, newParams, m_GstInfer, gst_nvinfer_logger);

  if (err != NVDSINFER_SUCCESS) {
    /* Notify application if the model load failed. */
    notifyLoadModelStatus(
        ModelStatus{err, modelPath, "Creation new model context failed"});
    return;
  }
  ...
  if (!triggerContextReplace(newCtxPtr, std::move(newParamsPtr), modelPath)) {
    notifyLoadModelStatus(ModelStatus{
        NVDSINFER_UNKNOWN_ERROR,
        modelPath,
        "trigger new model replace failed"});
    return;
  }
}
...
bool DsNvInferImpl::initNewInferModelParams(
  ...
  switch (loadType) {
    case MODEL_LOAD_FROM_CONFIG:
      if (!newModelPath.compare(newModelPath.length() - 4, 4, string_yml) ||
        ...
        if (!gst_nvinfer_parse_context_params_yaml(
                &newParams, newModelPath.c_str())) {
          GST_WARNING_OBJECT(
              m_GstInfer,
              "[UID %d]: parse new model config file: %s failed.",
              m_GstInfer->unique_id,
              newModelPath.c_str());
          return false;
        }
      } else if (!newModelPath.compare(
                     newModelPath.length() - 4, 4, string_txt)) {
        if (!gst_nvinfer_parse_context_params(
                &newParams, newModelPath.c_str())) {
          GST_WARNING_OBJECT(
              m_GstInfer,
              "[UID %d]: parse new model config file: %s failed.",
    ...
    case MODEL_LOAD_FROM_ENGINE:
      g_strlcpy(
          newParams.modelEngineFilePath,
          newModelPath.c_str(),
          sizeof(newParams.modelEngineFilePath));
      newParams.networkMode = oldParams.networkMode;
      memcpy(
          newParams.meanImageFilePath,
          oldParams.meanImageFilePath,
  ...
  if (string_empty(newParams.labelsFilePath) &&
    ...
    g_strlcpy(
        newParams.labelsFilePath,
        oldParams.labelsFilePath,
  ...
  if (string_empty(newParams.customLibPath) &&
    ...
    g_strlcpy(
        newParams.customLibPath,
        oldParams.customLibPath,
  ...
  if (string_empty(newParams.customBBoxParseFuncName) &&
    ...
    g_strlcpy(
        newParams.customBBoxParseFuncName,
        oldParams.customBBoxParseFuncName,
...
bool DsNvInferImpl::isNewContextValid(
  ...
  if (newParam.maxBatchSize < m_GstInfer->max_batch_size) {
    GST_WARNING_OBJECT(
        ...
        m_GstInfer->unique_id,
        newParam.maxBatchSize,
        m_GstInfer->max_batch_size);
    return false;
  }
...
void DsNvInferImpl::notifyLoadModelStatus(const ModelStatus& res) {
  ...
  if (res.status == NVDSINFER_SUCCESS) {
    GST_INFO_OBJECT(
        m_GstInfer,
        "[UID %d]: Load new model:%s sucessfully",
        m_GstInfer->unique_id,
        res.cfg_file.c_str());
  } else {
    GST_ELEMENT_WARNING(
...
bool DsNvInferImpl::triggerContextReplace(
    NvDsInferContextPtr ctx,
    NvDsInferContextInitParamsPtr params,
    const std::string& path) {
  std::string lastConfig;
  LockGMutex lock(m_GstInfer->process_lock);
  m_NextContextReplacement.reset(
      new ContextReplacementPtr::element_type(ctx, std::move(params), path));
  return true;
}

DsNvInferImpl::ContextReplacementPtr DsNvInferImpl::getNextReplacementUnlock() {
 ...
 * pushed downstream. */
NvDsInferStatus DsNvInferImpl::flushDataUnlock(LockGMutex& lock) {
 ...
 * the model. */
NvDsInferStatus DsNvInferImpl::resetContextUnlock(
    NvDsInferContextPtr ctx,
    NvDsInferContextInitParamsPtr params,
    const std::string& path) {
 ...
 * model replacement. */
NvDsInferStatus DsNvInferImpl::ensureReplaceNextContext() {
  ...
  if (err != NVDSINFER_SUCCESS) {
    notifyLoadModelStatus(
        ModelStatus{err, path, "Model update failed while flushing data"});
    return err;
  }
  ...
  notifyLoadModelStatus(
      ModelStatus{NVDSINFER_SUCCESS, path, "New Model updated succefully"});
  return NVDSINFER_SUCCESS;
}
