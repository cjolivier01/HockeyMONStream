...
#define GST_TYPE_NVINFER_ALLOCATOR (gst_nvinfer_allocator_get_type())
#define GST_NVINFER_ALLOCATOR(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST(     \
      (obj), GST_TYPE_NVINFER_ALLOCATOR, GstNvInferAllocator))
#define GST_NVINFER_ALLOCATOR_CLASS(klass) \
  (G_TYPE_CHECK_CLASS_CAST(                \
      (klass), GST_TYPE_NVINFER_ALLOCATOR, GstNvInferAllocatorClass))
 ...
 * allocator. */
static GstMemory* gst_nvinfer_allocator_alloc(
    GstAllocator* allocator,
    gsize size,
  ...
  if (NvBufSurfaceCreate(
          &tmem->surf, inferallocator->batch_size, &create_params) != 0) {
...
static void gst_nvinfer_allocator_free(
    GstAllocator* allocator,
    GstMemory* memory) {
...
static gpointer gst_nvinfer_memory_map(
    GstMemory* mem,
    gsize maxsize,
    GstMapFlags flags) {
...
static void gst_nvinfer_memory_unmap(GstMemory* mem) {}

/* Standard boiler plate. Assigning implemented function pointers. */
static void gst_nvinfer_allocator_class_init(GstNvInferAllocatorClass* klass) {
 ...
 * the memory type. */
static void gst_nvinfer_allocator_init(GstNvInferAllocator* allocator) {
