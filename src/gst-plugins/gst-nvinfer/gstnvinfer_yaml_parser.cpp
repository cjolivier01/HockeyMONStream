...
static gboolean gst_nvinfer_parse_class_attrs(
  ...
  for (YAML::const_iterator itr = configyml[group_str].begin();
       itr != configyml[group_str].end();
       ++itr) {
    std::string paramKey = itr->first.as<std::string>();
    if (paramKey == "pre-cluster-threshold") {
      detection_params.preClusterThreshold = itr->second.as<double>();
      ...
      detection_params.postClusterThreshold = itr->second.as<double>();
      ...
      detection_params.eps = itr->second.as<double>();
      ...
      detection_params.groupThreshold = itr->second.as<int>();
      ...
      detection_params.minBoxes = itr->second.as<int>();
      ...
      detection_params.minScore = itr->second.as<double>();
      ...
      detection_filter_params.roiTopOffset = itr->second.as<int>();
      ...
      detection_filter_params.roiBottomOffset = itr->second.as<int>();
      ...
      detection_filter_params.detectionMinWidth = itr->second.as<int>();
      ...
      detection_filter_params.detectionMinHeight = itr->second.as<int>();
      ...
      detection_filter_params.detectionMaxWidth = itr->second.as<int>();
      ...
      detection_filter_params.detectionMaxHeight = itr->second.as<int>();
...
static gboolean gst_nvinfer_parse_other_attribute_yaml(
    GstNvInfer* nvinfer,
    const gchar* cfg_file_path,
  ...
  if (pair[0] == "process-mode") {
    ...
    if ((gint)nvinfer->min_input_object_width < 0) {
      g_printerr(
          "Error: Negative value specified for input-object-min-width (%d)\n",
    ...
    if ((gint)nvinfer->min_input_object_height < 0) {
      g_printerr(
          "Error: Negative value specified for input-object-min-height (%d)\n",
    ...
    if ((gint)nvinfer->max_input_object_width < 0) {
      g_printerr(
          "Error: Negative value specified for input-object-max-width (%d)\n",
    ...
    if ((gint)nvinfer->max_input_object_height < 0) {
      g_printerr(
          "Error: Negative value specified for input-object-max-height (%d)\n",
    ...
    g_printerr(
        "Unknown or legacy key specified '%s' for group property\n",
        pair[0].c_str());
  }
...
static gboolean gst_nvinfer_parse_props_yaml(
    GstNvInfer* nvinfer,
  ...
  for (YAML::const_iterator itr = configyml["property"].begin();
    ...
    if (paramKey == "gie-unique-id") {
      ...
      if (init_params->uniqueID <= 0) {
        g_printerr(
            "Error: gie-unique-id (%d) should be > 0\n", nvinfer->unique_id);
        goto done;
      }
      if (nvinfer)
      ...
      if (!get_absolute_file_path(
              cfg_file_path, temp.c_str(), init_params->labelsFilePath)) {
      ...
      if (init_params->maxBatchSize <= 0 ||
        ...
        g_printerr(
            "Error: batch-size(%d) should be in the range [%d,%d]\n",
      ...
      if (vec.size() != 3) {
        printf(
            "Error. infer-dims array length is %lu. Should be 3 as [c;h;w] order.\n",
            vec.size());
        goto done;
      }
      init_params->inferInputDims = NvDsInferDimsCHW{
          (unsigned int)std::stoul(vec[0]),
          (unsigned int)std::stoul(vec[1]),
      ...
      if (!get_absolute_file_path(
              cfg_file_path, temp.c_str(), init_params->modelEngineFilePath)) {
      ...
      if (!get_absolute_file_path(
              cfg_file_path, temp.c_str(), init_params->ipTensorFilePath)) {
      ...
      if (vec.size() > _MAX_CHANNELS) {
        g_printerr(
            "Error. Maximum  length of %d is allowed for offsets\n",
            _MAX_CHANNELS);
        goto done;
      }
      ...
      if (!get_absolute_file_path(
              cfg_file_path, temp.c_str(), init_params->meanImageFilePath)) {
      ...
      if (!get_absolute_file_path(
              cfg_file_path, temp.c_str(), init_params->customLibPath)) {
      ...
      std::string temp = itr->second.as<std::string>();
      std::strncpy(
          init_params->customBBoxInstanceMaskParseFuncName, temp.c_str(), 1023);
      ...
      std::string temp = itr->second.as<std::string>();
      std::strncpy(
          init_params->customClassifierParseFuncName, temp.c_str(), 1023);
    } else if (paramKey == "custom-network-config") {
      std::string temp = itr->second.as<std::string>();
      std::strncpy(
          init_params->customNetworkConfigFilePath, temp.c_str(), 4095);
    } else if (paramKey == "model-file") {
      std::string temp = itr->second.as<std::string>();

      if (!get_absolute_file_path(
              cfg_file_path, temp.c_str(), init_params->modelFilePath)) {
      ...
      if (!get_absolute_file_path(
              cfg_file_path, temp.c_str(), init_params->protoFilePath)) {
      ...
      if (!get_absolute_file_path(
              cfg_file_path, temp.c_str(), init_params->uffFilePath)) {
      ...
      switch (val) {
        ...
        default:
          g_printerr(
              "Error. Invalid value for network-input-order, network input order :%d\n",
              val);
          goto done;
          break;
      }
      ...
      switch (val) {
        ...
        default:
          g_printerr(
              "Error. Invalid value for uff-input-order, UFF input order:%d\n",
              val);
          goto done;
          break;
      }
      ...
      if (!get_absolute_file_path(
              cfg_file_path, temp.c_str(), init_params->onnxFilePath)) {
      ...
      if (init_params->numDetectedClasses < 0) {
        g_printerr(
            "Error: Negative value specified for num-detected-classes(%d)\n",
      ...
      if (val < 0) {
        g_printerr(
            "Error: Negative value specified for cluster-mode(%d)\n", val);
        goto done;
      }
      switch (val) {
        case 0:
      ...
      if (init_params->classifierThreshold < 0) {
        g_printerr(
            "Error: Negative value specified for classifier-threshold(%.2f)\n",
      ...
      if (init_params->segmentationThreshold < 0) {
        g_printerr(
            "Error: Negative value specified for segmentation-threshold(%.2f)\n",
      ...
      switch (val) {
        ...
        default:
          g_printerr(
              "Error. Invalid value for segmentation-output-order, Segmentation output order :'%d'\n",
              val);
          goto done;
          break;
      }
...
gboolean gst_nvinfer_parse_config_file_yaml(
  ...
  if (init_params->networkType == NvDsInferNetworkType_Detector ||
    ...
    for (YAML::const_iterator itr = configyml.begin(); itr != configyml.end();
         ++itr) {
      std::string paramKey = itr->first.as<std::string>();
      ...
      if (paramKey != "class-attrs-all") {
        if (paramKey.compare(0, class_str.size(), class_str) == 0) {
          ...
          /* Check that class_index has been parsed successfully and that it
           * lies within the valid range of class_ids [0, numDetectedClasses -
           * 1]. */
          if ((gint)class_index < 0) {
            g_printerr(
                "Invalid group [%s]. class-id should be >= 0\n",
                paramKey.c_str());
            goto done;
          }
