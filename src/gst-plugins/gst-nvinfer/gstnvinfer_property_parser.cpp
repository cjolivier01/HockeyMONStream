...
static gboolean get_absolute_file_path(
...
static gboolean gst_nvinfer_parse_class_attrs(
    GKeyFile* key_file,
    gchar* group,
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_INFER_CLASS_ATTRS_THRESHOLD)) {
      ...
      detection_params.preClusterThreshold = g_key_file_get_double(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_THRESHOLD, &error);
      CHECK_ERROR(error);
      if (detection_params.preClusterThreshold < 0) {
        g_printerr(
            "Error: Negative threshold (%.5f) specified for group %s\n",
      ...
      if (detection_params.preClusterThreshold < 0) {
        g_printerr(
            "Error: Negative pre cluster threshold (%.5f) specified for group %s\n",
            detection_params.preClusterThreshold,
            group);
        goto done;
      }
      ...
      detection_params.postClusterThreshold = g_key_file_get_double(
          key_file,
          group,
          CONFIG_GROUP_INFER_CLASS_ATTRS_POST_CLUSTER_THRESHOLD,
          &error);
      ...
      if (detection_params.postClusterThreshold < 0) {
        g_printerr(
            "Error: Negative post cluster threshold (%.5f) specified for group %s\n",
            detection_params.postClusterThreshold,
            group);
        goto done;
      }
    } else if (!g_strcmp0(*key, CONFIG_GROUP_INFER_CLASS_ATTRS_EPS)) {
      detection_params.eps = g_key_file_get_double(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_EPS, &error);
      CHECK_ERROR(error);
      if (detection_params.eps < 0) {
        g_printerr(
            "Error: Negative eps (%.5f) specified for group %s\n",
            detection_params.eps,
            group);
        goto done;
      }
    } else if (!g_strcmp0(
                   *key, CONFIG_GROUP_INFER_CLASS_ATTRS_GROUP_THRESHOLD)) {
      detection_params.groupThreshold = g_key_file_get_integer(
          key_file,
          group,
      ...
      detection_params.minBoxes = g_key_file_get_integer(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_MIN_BOXES, &error);
      CHECK_ERROR(error);
      if (detection_params.minBoxes < 0) {
        ...
        goto done;
                   ...
                   *key, CONFIG_GROUP_INFER_CLASS_ATTRS_DBSCAN_MIN_SCORE)) {
      detection_params.minScore = g_key_file_get_double(
          key_file,
          group,
      ...
      if (detection_params.minScore < 0) {
        ...
        goto done;
                   ...
                   *key, CONFIG_GROUP_INFER_CLASS_ATTRS_ROI_TOP_OFFSET)) {
      detection_filter_params.roiTopOffset = g_key_file_get_integer(
          key_file,
          group,
      ...
      if ((gint)detection_filter_params.roiTopOffset < 0) {
        ...
        goto done;
                   ...
                   *key, CONFIG_GROUP_INFER_CLASS_ATTRS_ROI_BOTTOM_OFFSET)) {
      detection_filter_params.roiBottomOffset = g_key_file_get_integer(
          key_file,
          group,
      ...
      if ((gint)detection_filter_params.roiBottomOffset < 0) {
        ...
        goto done;
                   ...
                   *key, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MIN_WIDTH)) {
      detection_filter_params.detectionMinWidth = g_key_file_get_integer(
          key_file,
          group,
      ...
      if ((gint)detection_filter_params.detectionMinWidth < 0) {
        ...
        goto done;
                   ...
                   *key, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MIN_HEIGHT)) {
      detection_filter_params.detectionMinHeight = g_key_file_get_integer(
          key_file,
          group,
      ...
      if ((gint)detection_filter_params.detectionMinHeight < 0) {
        ...
        goto done;
                   ...
                   *key, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MAX_WIDTH)) {
      detection_filter_params.detectionMaxWidth = g_key_file_get_integer(
          key_file,
          group,
      ...
      if ((gint)detection_filter_params.detectionMaxWidth < 0) {
        ...
        goto done;
                   ...
                   *key, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MAX_HEIGHT)) {
      detection_filter_params.detectionMaxHeight = g_key_file_get_integer(
          key_file,
          group,
      ...
      g_free(list);
                   ...
                   *key, CONFIG_GROUP_INFER_CLASS_ATTRS_NMS_IOU_THRESHOLD)) {
      detection_params.nmsIOUThreshold = g_key_file_get_double(
          key_file,
          group,
          CONFIG_GROUP_INFER_CLASS_ATTRS_NMS_IOU_THRESHOLD,
          &error);
      CHECK_ERROR(error);
      if (detection_params.nmsIOUThreshold < 0 ||
          detection_params.nmsIOUThreshold > 1) {
        g_printerr(
            "Error: Invalid nms iou threshold (%.2f) specified for group %s."
            "Enter a value between 0 & 1. \n",
            detection_params.nmsIOUThreshold,
            group);
        goto done;
      }
    } else if (!g_strcmp0(*key, CONFIG_GROUP_INFER_CLASS_ATTRS_TOP_K)) {
      detection_params.topK = g_key_file_get_integer(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_TOP_K, &error);
      CHECK_ERROR(error);
      if (detection_params.topK < 0) {
        g_printerr(
            "Error: Invalid topk value %d specified for group %s."
            " topk should be greater than of equal to '0'\n",
            detection_params.topK,
            group);
        goto done;
      }
    } else {
      g_printerr(
          "Unknown key '%s' for group [%s]\n", *key, CONFIG_GROUP_PROPERTY);
    }
  }
...
static gboolean gst_nvinfer_parse_other_attribute(
    GstNvInfer* nvinfer,
    GKeyFile* key_file,
  ...
  if (!g_strcmp0(key, CONFIG_GROUP_INFER_PROCESS_MODE)) {
    if ((*nvinfer->is_prop_set)[PROP_PROCESS_MODE])
      return TRUE;
    guint val = g_key_file_get_integer(
        key_file, group_name, CONFIG_GROUP_INFER_PROCESS_MODE, &error);
    CHECK_ERROR(error);
    ...
    nvinfer->classifier_type = g_key_file_get_string(
        key_file, group_name, CONFIG_GROUP_INFER_CLASSIFIER_TYPE, &error);
    CHECK_ERROR(error);
  } else if (!g_strcmp0(key, CONFIG_GROUP_INFER_INTERVAL)) {
    if ((*nvinfer->is_prop_set)[PROP_INTERVAL])
      return TRUE;
    nvinfer->interval = g_key_file_get_integer(
        key_file, group_name, CONFIG_GROUP_INFER_INTERVAL, &error);
    CHECK_ERROR(error);
    if ((gint)nvinfer->interval < 0) {
      g_printerr(
          "Error: Negative value (%d) specified for interval\n",
          nvinfer->interval);
      goto done;
    }
    ...
    if (g_key_file_get_boolean(
            key_file, group_name, CONFIG_GROUP_INFER_SYMMETRIC_PADDING, &error))
    ...
    nvinfer->max_input_object_width = g_key_file_get_integer(
        key_file,
        group_name,
    ...
    nvinfer->max_input_object_height = g_key_file_get_integer(
        key_file,
        group_name,
    ...
    if ((*nvinfer->is_prop_set)[PROP_OPERATE_ON_GIE_ID] ||
      ...
      return TRUE;
    nvinfer->operate_on_gie_id = g_key_file_get_integer(
        key_file, group_name, CONFIG_GROUP_INFER_GIE_ID_FOR_OPERATION, &error);
    CHECK_ERROR(error);
    ...
    int val = g_key_file_get_integer(
        key_file, group_name, CONFIG_GROUP_INFER_SCALING_COMPUTE_HW, &error);
    CHECK_ERROR(error);
    ...
    nvinfer->transform_config_params.compute_mode =
        (NvBufSurfTransform_Compute)val;
  } else if (!g_strcmp0(key, CONFIG_GROUP_INFER_SCALING_FILTER)) {
    int val = g_key_file_get_integer(
        key_file, group_name, CONFIG_GROUP_INFER_SCALING_FILTER, &error);
    CHECK_ERROR(error);
    ...
    g_printerr(
        "Unknown or legacy key specified '%s' for group [%s]\n",
        key,
...
static gboolean gst_nvinfer_parse_props(
    GstNvInfer* nvinfer,
  ...
  if (g_key_file_has_key(
    ...
    gboolean value = g_key_file_get_boolean(
        key_file,
        CONFIG_GROUP_PROPERTY,
        CONFIG_GROUP_INFER_IS_CLASSIFIER_LEGACY,
        &error);
    CHECK_ERROR(error);
    guint new_value = (value) ? NvDsInferNetworkType_Classifier
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_INFER_UNIQUE_ID)) {
      if (nvinfer && (*nvinfer->is_prop_set)[PROP_UNIQUE_ID])
        continue;
      init_params->uniqueID = g_key_file_get_integer(
          key_file,
          CONFIG_GROUP_PROPERTY,
      ...
      gchar* str = g_key_file_get_string(
          key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_LABEL, &error);
      CHECK_STR_ERROR(error);

      if (!get_absolute_file_path(
              cfg_file_path, str, init_params->labelsFilePath)) {
      ...
      init_params->gpuID = g_key_file_get_integer(
          key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_GPU_ID, &error);
      CHECK_ERROR(error);

      if (cudaGetDeviceCount(&devices) != cudaSuccess) {
        g_printerr(
            "Error: Could not get cuda device count (%s)\n",
      ...
      init_params->dlaCore = g_key_file_get_integer(
          key_file,
          CONFIG_GROUP_PROPERTY,
      ...
      init_params->maxGPUMemPer = g_key_file_get_integer(
          key_file,
          CONFIG_GROUP_PROPERTY,
      ...
      if (nvinfer && (*nvinfer->is_prop_set)[PROP_BATCH_SIZE])
        continue;
      init_params->maxBatchSize = g_key_file_get_integer(
          key_file,
          CONFIG_GROUP_PROPERTY,
      ...
      if (init_params->maxBatchSize <= 0 ||
        ...
        g_printerr(
            "Error: %s(%d) should be in the range [%d,%d]\n",
            CONFIG_GROUP_INFER_BATCH_SIZE,
            nvinfer->max_batch_size,
            1,
      ...
      init_params->workspaceSize = g_key_file_get_integer(
          key_file,
          CONFIG_GROUP_PROPERTY,
      ...
      if (length != 3) {
        g_printerr(
            "Error. '%s' array length is %lu. Should be 3 as [c;h;w] order.\n",
            CONFIG_GROUP_INFER_INFER_DIMENSIONS,
            length);
        goto done;
      }
      ...
      if (!get_absolute_file_path(
              cfg_file_path, str, init_params->modelEngineFilePath)) {
      ...
      if (!get_absolute_file_path(
              cfg_file_path, str, init_params->int8CalibrationFilePath)) {
      ...
      gsize length = 0;
      init_params->outputLayerNames = g_key_file_get_string_list(
          key_file,
          CONFIG_GROUP_PROPERTY,
      ...
      init_params->overwriteOpTensor = g_key_file_get_integer(
          key_file,
          CONFIG_GROUP_PROPERTY,
      ...
      if (!get_absolute_file_path(
              cfg_file_path, str, init_params->ipTensorFilePath)) {
      ...
      gsize length = 0;
      char** op_file_list = g_key_file_get_string_list(
          key_file,
          CONFIG_GROUP_PROPERTY,
      ...
      gsize length = 0;
      init_params->outputIOFormats = g_key_file_get_string_list(
          key_file,
          CONFIG_GROUP_PROPERTY,
      ...
      init_params->networkScaleFactor = g_key_file_get_double(
          key_file,
          CONFIG_GROUP_PROPERTY,
      ...
      if (length > _MAX_CHANNELS) {
        g_printerr(
            "Error. Maximum  length of %d is allowed for '%s'\n",
            _MAX_CHANNELS,
            CONFIG_GROUP_INFER_OFFSETS);
        g_free(dbl_list);
        goto done;
      ...
      gchar* str = g_key_file_get_string(
          key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_MEANFILE, &error);
      CHECK_STR_ERROR(error);

      if (!get_absolute_file_path(
              cfg_file_path, str, init_params->meanImageFilePath)) {
      ...
      if (!get_absolute_file_path(
              cfg_file_path, str, init_params->customLibPath)) {
      ...
      CHECK_STR_ERROR(error);
      g_strlcpy(
          init_params->customBBoxInstanceMaskParseFuncName,
          str,
          _MAX_STR_LENGTH);
      g_free(str);
      ...
      g_free(str);
                   ...
                   *key, CONFIG_GROUP_INFER_CUSTOM_PARSE_CLASSIFIER_FUNC)) {
      gchar* str = g_key_file_get_string(
          key_file,
          CONFIG_GROUP_PROPERTY,
      ...
      CHECK_STR_ERROR(error);
      g_strlcpy(
          init_params->customClassifierParseFuncName, str, _MAX_STR_LENGTH);
      g_free(str);
      ...
      gchar* str = g_key_file_get_string(
          key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_MODEL, &error);
      CHECK_STR_ERROR(error);

      if (!get_absolute_file_path(
              cfg_file_path, str, init_params->modelFilePath)) {
      ...
      gchar* str = g_key_file_get_string(
          key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_PROTO, &error);
      CHECK_STR_ERROR(error);

      if (!get_absolute_file_path(
              cfg_file_path, str, init_params->protoFilePath)) {
      ...
      gchar* str = g_key_file_get_string(
          key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_UFF, &error);
      CHECK_STR_ERROR(error);

      if (!get_absolute_file_path(
              cfg_file_path, str, init_params->uffFilePath)) {
      ...
      g_free(str);
                   ...
                   *key, CONFIG_GROUP_INFER_UFF_INPUT_DIMENSIONS_LEGACY)) {
      /* Parse here if infer-dims has not been set, else ignore input-dims /
       * uff-input-dims */
      if (!g_key_file_has_key(
              key_file,
              CONFIG_GROUP_PROPERTY,
        ...
        if (length != 4) {
          g_printerr(
              "Error. '%s' array length is %lu. Should be 4[a;b;c;ORDER].\n",
              CONFIG_GROUP_INFER_UFF_INPUT_DIMENSIONS_LEGACY,
              length);
          goto done;
        }
        switch (int_list[3]) {
          ...
          default:
            g_printerr(
                "Error. Invalid value for '%s', UFF input order :'%d'\n",
                CONFIG_GROUP_INFER_UFF_INPUT_DIMENSIONS_LEGACY,
                int_list[3]);
            goto done;
            break;
        ...
        g_printerr(
            "Warning: Ignoring 'input-dims' parameter since 'infer-dims' has been set.\n");
    } else if (!g_strcmp0(
                   *key, CONFIG_GROUP_INFER_UFF_INPUT_DIMENSIONS_LEGACY_V2)) {
      /* Parse here if infer-dims has not been set, else ignore input-dims /
       * uff-input-dims */
      if (!g_key_file_has_key(
              key_file,
        ...
        g_printerr(
            "Warning: Ignoring 'uff-input-dims' parameter since 'infer-dims' has been set.\n");
    } else if (!g_strcmp0(*key, CONFIG_GROUP_INFER_NET_INPUT_ORDER)) {
      gint val = g_key_file_get_integer(
          key_file,
          CONFIG_GROUP_PROPERTY,
      ...
      switch (val) {
        ...
        default:
          g_printerr(
              "Error. Invalid value for '%s', network input order :'%d'\n",
              CONFIG_GROUP_INFER_NET_INPUT_ORDER,
              val);
          goto done;
          break;
      ...
      if (!get_absolute_file_path(
              cfg_file_path, str, init_params->tltEncodedModelFilePath)) {
      ...
      gchar* str = g_key_file_get_string(
          key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_ONNX, &error);
      CHECK_STR_ERROR(error);

      if (!get_absolute_file_path(
              cfg_file_path, str, init_params->onnxFilePath)) {
      ...
      init_params->numDetectedClasses = g_key_file_get_integer(
          key_file,
          CONFIG_GROUP_PROPERTY,
      ...
      g_printerr(
          "Warn: 'enable-dbscan' parameter has been deprecated."
      ...
      if (g_key_file_has_key(
              ...
              nullptr)) {
        g_printerr(
            "Warn: Ignoring 'enable-dbscan' parameter since"
            " 'cluster-mode' has been set.\n");
      } else if (g_key_file_get_boolean(
                     key_file,
                     CONFIG_GROUP_PROPERTY,
      ...
      init_params->classifierThreshold = g_key_file_get_double(
          key_file,
          CONFIG_GROUP_PROPERTY,
      ...
      if (init_params->classifierThreshold < 0) {
        g_printerr(
            "Error: Negative value specified for %s(%.2f)\n",
            CONFIG_GROUP_INFER_CLASSIFIER_THRESHOLD,
            init_params->classifierThreshold);
        goto done;
      }
    } else if (!g_strcmp0(*key, CONFIG_GROUP_INFER_SEGMENTATION_THRESHOLD)) {
      init_params->segmentationThreshold = g_key_file_get_double(
          key_file,
          CONFIG_GROUP_PROPERTY,
      ...
      switch (val) {
        ...
        default:
          g_printerr(
              "Error. Invalid value for '%s', Segmentation output order :'%d'\n",
              CONFIG_GROUP_INFER_SEGMENTATION_OUTPUT_ORDER,
              val);
          goto done;
          break;
      ...
      init_params->disableOutputHostCopy = g_key_file_get_integer(
          key_file,
          CONFIG_GROUP_PROPERTY,
...
gboolean gst_nvinfer_parse_config_file(
  ...
  if (!g_key_file_load_from_file(
          cfg_file, cfg_file_path, G_KEY_FILE_NONE, &error)) {
  ...
  if (init_params->networkType == NvDsInferNetworkType_Detector ||
    ...
    if (g_key_file_has_group(
            cfg_file, CONFIG_GROUP_INFER_CLASS_ATTRS_PREFIX "all")) {
      if (!gst_nvinfer_parse_class_attrs(
              cfg_file,
              ...
              color_params)) {
        g_printerr(
            "Error while parsing group %s\n",
            CONFIG_GROUP_INFER_CLASS_ATTRS_PREFIX "all");
        goto done;
      }
      g_key_file_remove_group(
          cfg_file, CONFIG_GROUP_INFER_CLASS_ATTRS_PREFIX "all", nullptr);
    }
    ...
    nvinfer->perClassColorParams = new std::vector<GstNvInferColorParams>(
    ...
    for (group = groups; *group; group++) {
      if (!strncmp(
        ...
        if (class_index == 0 && endptr == key1) {
          g_printerr(
              "Invalid group [%s]. Class attributes should be specified using group name '" CONFIG_GROUP_INFER_CLASS_ATTRS_PREFIX
              "<class-id>'\n",
              *group);
          goto done;
        }
...
/* Parse nvinfer config file for context params. Returns FALSE in case of an
 * error. */
