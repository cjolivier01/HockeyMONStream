...
static void gst_nvds_osd_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec);
static void gst_nvds_osd_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec);
...
static GstCaps* gst_nvds_osd_transform_caps(
...
static gboolean gst_nvds_osd_set_caps(GstBaseTransform* trans, GstCaps* incaps, GstCaps* outcaps) {
  ...
  nvdsosd->conv_buf = nvll_osd_set_params(nvdsosd->nvdsosd_context, nvdsosd->width, nvdsosd->height);
...
static GstFlowReturn gst_nvds_osd_transform_ip(GstBaseTransform* trans, GstBuffer* buf) {
  ...
  snprintf(context_name, sizeof(context_name), "%s_(Frame=%u)", GST_ELEMENT_NAME(nvdsosd), nvdsosd->frame_num);
  ...
  for (l = full_obj_meta_list; l != NULL; l = l->next) {
    ...
    if (nvdsosd->draw_mask && object_meta->mask_params.data && object_meta->mask_params.size > 0) {
      ...
      if (segment_cnt == MAX_OSD_ELEMS) {
        ...
        if (nvll_osd_draw_segment_masks(nvdsosd->nvdsosd_context, nvdsosd->frame_mask_params) == -1) {
          GST_ELEMENT_ERROR(nvdsosd, RESOURCE, FAILED, ("Unable to draw rectangles"), (NULL));
          return GST_FLOW_ERROR;
        }
        segment_cnt = 0;
  ...
  if (nvdsosd->nvdsosd_mode == MODE_GPU) {
    if (nvll_osd_apply(nvdsosd->nvdsosd_context, NULL, surface) == -1) {
      GST_ELEMENT_ERROR(nvdsosd, RESOURCE, FAILED, ("Unable to draw shapes onto video frame by GPU"), (NULL));
      return GST_FLOW_ERROR;
    }
  }
...
static void gst_nvds_osd_class_init(GstNvDsOsdClass* klass) {
  ...
  g_object_class_install_property(
      gobject_class,
      PROP_SHOW_CLOCK,
      g_param_spec_boolean("display-clock", "clock", "Whether to display clock", FALSE, G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class,
      PROP_SHOW_TEXT,
      g_param_spec_boolean("display-text", "text", "Whether to display text", TRUE, G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class,
      PROP_SHOW_BBOX,
      g_param_spec_boolean("display-bbox", "text", "Whether to display bounding boxes", TRUE, G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class,
      PROP_SHOW_MASK,
      g_param_spec_boolean("display-mask", "text", "Whether to display instance mask", TRUE, G_PARAM_READWRITE));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_uint(
          ...
          MAX_FONT_SIZE,
          DEFAULT_FONT_SIZE,
          (GParamFlags)((int)G_PARAM_READWRITE | (int)G_PARAM_STATIC_STRINGS | (int)GST_PARAM_MUTABLE_READY)));
  ...
  gst_element_class_add_pad_template(gstelement_class, gst_static_pad_template_get(&nvdsosd_src_factory));
  gst_element_class_add_pad_template(gstelement_class, gst_static_pad_template_get(&nvdsosd_sink_factory));
...
static void gst_nvds_osd_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec) {
  ...
  switch (prop_id) {
    ...
    case PROP_PROCESS_MODE:
      ...
      if (nvdsosd->nvdsosd_mode > MODE_GPU) {
        g_print("WARN !! Invalid mode selected, Falling back to GPU\n");
        nvdsosd->nvdsosd_mode = nvdsosd->nvdsosd_mode > MODE_GPU ? MODE_GPU : nvdsosd->nvdsosd_mode;
      }
      break;
 ...
 */
static gboolean gst_nvds_osd_parse_color(GstNvDsOsd* nvdsosd, guint clock_color) {
  nvdsosd->clock_text_params.font_params.font_color.red = (gfloat)((clock_color & 0xff000000) >> 24) / 255;
  nvdsosd->clock_text_params.font_params.font_color.green = (gfloat)((clock_color & 0x00ff0000) >> 16) / 255;
  nvdsosd->clock_text_params.font_params.font_color.blue = (gfloat)((clock_color & 0x0000ff00) >> 8) / 255;
  nvdsosd->clock_text_params.font_params.font_color.alpha = (gfloat)((clock_color & 0x000000ff)) / 255;
  return TRUE;
}
