...
bool TrackerMiscDataManager::init(
  ...
  for (uint32_t setInd = 0; setInd < maxBufferPoolSize; setInd++) {
    ...
    allocatePastFrame(
        pNewBuf, batchSize, maxTargetsPerStream, maxShadowTrackingAge);
    allocateReid(pNewBuf, batchSize, maxTargetsPerStream, reidFeatureSize);
    allocateTerminatedTracks(
        pNewBuf, batchSize, maxTargetsPerStream, maxTerminatedFrameHistory);
...
void TrackerMiscDataManager::allocatePastFrame(
    NvTrackerMiscDataBuffer* pNewBuf,
    uint32_t batchSize,
    uint32_t maxTargetsPerStream,
    uint32_t maxShadowTrackingAge) {
  if (!m_PastFrame) {
...
void TrackerMiscDataManager::releasePastFrame(
    NvTrackerMiscDataBuffer* pBuffer) {
  if (!m_PastFrame) {
    return;
  }
  /** Past frame memory */
  ...
  for (uint32_t streamInd = 0; streamInd < objBatch.numAllocated; streamInd++) {
    NvDsTargetMiscDataStream& objStream = objBatch.list[streamInd];
    for (uint32_t targetInd = 0; targetInd < objStream.numAllocated;
         targetInd++) {
      NvDsTargetMiscDataObject& objList = objStream.list[targetInd];
      delete[] objList.list;
    }
...
void TrackerMiscDataManager::allocateReid(
    NvTrackerMiscDataBuffer* pNewBuf,
    uint32_t batchSize,
    uint32_t maxTargetsPerStream,
    uint32_t reidFeatureSize) {
  ...
  CUDA_CHECK(cudaMalloc(
      reinterpret_cast<void**>(&reidTensor.ptr_dev),
      batchSize * maxTargetsPerStream * reidFeatureSize * sizeof(float)));
  CUDA_CHECK(cudaMallocHost(
      reinterpret_cast<void**>(&reidTensor.ptr_host),
      batchSize * maxTargetsPerStream * reidFeatureSize * sizeof(float)));
...
void TrackerMiscDataManager::allocateTerminatedTracks(
    NvTrackerMiscDataBuffer* pNewBuf,
    uint32_t batchSize,
    uint32_t maxTargetsPerStream,
...
void TrackerMiscDataManager::releaseTerminatedTracks(
    NvTrackerMiscDataBuffer* pBuffer) {
  if (!m_OutputTerminatedTracks) {
    return;
  }
...
void TrackerMiscDataManager::resetTerminatedTracks(
    NvTrackerMiscDataBuffer* pBuffer) {
  if (!m_OutputTerminatedTracks) {
    return;
  }
...
void TrackerMiscDataManager::allocateShadowTracks(
    NvTrackerMiscDataBuffer* pNewBuf,
    uint32_t batchSize,
    uint32_t maxTargetsPerStream) {
...
void TrackerMiscDataManager::releaseShadowTracks(
    NvTrackerMiscDataBuffer* pBuffer) {
  if (!m_OutputShadowTracks) {
    return;
  }
...
void TrackerMiscDataManager::resetShadowTracks(
    NvTrackerMiscDataBuffer* pBuffer) {
  if (!m_OutputShadowTracks) {
    return;
  }
...
NvTrackerMiscDataBuffer* TrackerMiscDataManager::pop() {
  ...
  if (m_Cond.wait_for(lock, MAX_BUFFER_WAIT_TIMEOUT, [this]() {
