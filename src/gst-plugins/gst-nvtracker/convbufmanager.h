...
#define MAX_BUFFER_POOL_SIZE \
  2 // Set surface buffer pool size as a fixed number.
...
class ConvBufManager {
 public:
  ...
  NvBufSurface* convertBatchAsync(
 ...
 private:
  ...
  bool m_Initialized;
  /** Buffer pool is empty when surface transform is not required such as IOU
   * tracker. */
