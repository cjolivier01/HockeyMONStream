...
static gpointer copy_nvtracker2_past_frame_meta(
    gpointer data,
    gpointer user_data) {
...
static void free_nvtracker2_past_frame_meta(
    gpointer meta_data,
    gpointer user_data) {
...
static gpointer copy_nvtracker2_batch_reid_meta(
    gpointer data,
    gpointer user_data) {
  NvDsUserMeta* user_meta = (NvDsUserMeta*)data;
  if (user_meta && user_meta->user_meta_data) {
...
static void free_nvtracker2_batch_reid_meta(
    gpointer meta_data,
    gpointer user_data) {
  NvDsUserMeta* user_meta = (NvDsUserMeta*)meta_data;
  if (user_meta && user_meta->user_meta_data) {
...
static gpointer copy_nvtracker2_obj_reid_meta(
    gpointer data,
    gpointer user_data) {
  NvDsUserMeta* user_meta = (NvDsUserMeta*)data;
  if (user_meta && user_meta->user_meta_data) {
    NvDsObjReid* pReidObj = (NvDsObjReid*)user_meta->user_meta_data;
    if ((pReidObj == NULL) || (pReidObj->featureSize == 0) ||
        (pReidObj->ptr_dev == NULL) || (pReidObj->ptr_host == NULL)) {
      return NULL;
    }
...
static void free_nvtracker2_obj_reid_meta(
    gpointer meta_data,
    gpointer user_data) {
  NvDsUserMeta* user_meta = (NvDsUserMeta*)meta_data;
  if (user_meta && user_meta->user_meta_data) {
...
static gpointer copy_nvtracker2_terminated_meta(
    gpointer data,
    gpointer user_data) {
  NvDsUserMeta* user_meta = (NvDsUserMeta*)data;
  if (user_meta && user_meta->user_meta_data) {
...
static void free_nvtracker2_terminated_meta(
    gpointer meta_data,
    gpointer user_data) {
  NvDsUserMeta* user_meta = (NvDsUserMeta*)meta_data;
  if (user_meta && user_meta->user_meta_data) {
...
static gpointer copy_nvtracker2_shadowTracks_meta(
    gpointer data,
    gpointer user_data) {
  NvDsUserMeta* user_meta = (NvDsUserMeta*)data;
  if (user_meta && user_meta->user_meta_data) {
...
static void free_nvtracker2_shadowTracks_meta(
    gpointer meta_data,
    gpointer user_data) {
  NvDsUserMeta* user_meta = (NvDsUserMeta*)meta_data;
  if (user_meta && user_meta->user_meta_data) {
...
static gpointer copy_nvtracker2_obj_visibility_meta(
    gpointer data,
    gpointer user_data) {
  NvDsUserMeta* user_meta = (NvDsUserMeta*)data;
  if (user_meta && user_meta->user_meta_data) {
...
static void free_nvtracker2_obj_visibility_meta(
    gpointer meta_data,
    gpointer user_data) {
  NvDsUserMeta* user_meta = (NvDsUserMeta*)meta_data;
  if (user_meta && user_meta->user_meta_data) {
...
static gpointer copy_nvtracker2_obj_foot_loc_meta(
    gpointer data,
    gpointer user_data) {
  NvDsUserMeta* user_meta = (NvDsUserMeta*)data;
  if (user_meta && user_meta->user_meta_data) {
...
static void free_nvtracker2_obj_foot_loc_meta(
    gpointer meta_data,
    gpointer user_data) {
  NvDsUserMeta* user_meta = (NvDsUserMeta*)meta_data;
  if (user_meta && user_meta->user_meta_data) {
...
static gpointer copy_nvtracker2_obj_convex_hull_meta(
    gpointer data,
    gpointer user_data) {
  NvDsUserMeta* user_meta = (NvDsUserMeta*)data;
  if (user_meta && user_meta->user_meta_data) {
    NvDsObjConvexHull* pConvexHull =
        (NvDsObjConvexHull*)user_meta->user_meta_data;
...
static void free_nvtracker2_obj_convex_hull_meta(
    gpointer meta_data,
    gpointer user_data) {
  NvDsUserMeta* user_meta = (NvDsUserMeta*)meta_data;
  if (user_meta && user_meta->user_meta_data) {
...
bool NvTrackerProc::init(const TrackerConfig& config) {
  ...
  if (m_Config.subBatchSizes.size() == 0) {
    ...
    // A single batch, hence pad_index and seq_index will have a 1:1
    // correspondence
    ...
    if (!m_Config.dynamicSubBatching) {
      ...
      for (uint32_t i = 0; i < m_Config.subBatchesConfig.size(); i++) {
        ...
        // Create a mapping from pad index (source id) to seq_index for each
        // sub-batch This is required since seq_index needs to range from 0 to
        // (max batch size -1 ) where as, pad index of a stream in a sub-batch
        ...
        for (uint32_t j = 0; j < subBatch.size(); j++) {
          ...
          if (m_PadIndexSeqIndexMap.find(padIndex) !=
              m_PadIndexSeqIndexMap.end()) {
            LOG_ERROR(
                "gstnvtracker:: Check the config \"sub-batches\"!!  Source id %d has re-occured!!",
                padIndex);
            deInit();
            return false;
          }
      ...
      for (uint32_t i = 0; i < m_Config.subBatchSizes.size(); i++) {
        ...
        m_DispatchMap[i].sbThread =
            thread(&NvTrackerProc::processSubBatch, this, &m_DispatchMap.at(i));
        m_MaxSubBatchSize =
            max(m_MaxSubBatchSize, (uint32_t)m_Config.subBatchSizes[i]);
      }
    }
  }
...
void NvTrackerProc::deInit() {
  ...
  if (m_Config.numTransforms > 0 && m_Config.inputTensorMeta == false) {
    while (!m_ConvBufMgr.isQueueFull()) {
      /** printf("m_ConvBufMgr.getFreeQueueSize() %d,
       * m_ConvBufMgr.getActualPoolSize() %d\n",
  ...
  for (auto it = m_SubBatchAllocationMap.begin();
       it != m_SubBatchAllocationMap.end();
       it++) {
    it->second.clear();
  }
...
bool NvTrackerProc::removeSource(
    uint32_t sourceId,
  ...
  if (m_Config.subBatchSizes.size() == 0) {
    ...
    if (m_TrackerLibRemoveStreams) {
      ...
      if (m_PadIndexSsidMap.find(sourceId) != m_PadIndexSsidMap.end()) {
        ...
        NvMOTStatus status =
            m_TrackerLibRemoveStreams(m_BatchContextHandle, ssId);
    ...
    if (m_PadIndexSsidMap.find(sourceId) != m_PadIndexSsidMap.end()) {
      SurfaceStreamId ssId = m_PadIndexSsidMap[sourceId];
      {
        ...
        if (m_SsidSubBatchMap.find(ssId) == m_SsidSubBatchMap.end()) {
          ...
          /** Since the source is going to be removed, erase the entry from
           * mapping too */
          auto it = m_SsidSubBatchMap.find(ssId);
          m_SsidSubBatchMap.erase(it);
          // If dynamic sub-batch allocation is enabled, remove the entry from
          // allocation map as well
          if (m_SubBatchAllocationMap.size() > 0 &&
              (m_SubBatchAllocationMap.find(sbId) !=
               m_SubBatchAllocationMap.end())) {
            std::map<SeqIndex, std::pair<SurfaceStreamId, uint32_t>>&
                singleSubBatch = m_SubBatchAllocationMap.find(sbId)->second;
            SeqIndex seqIdxEntry = 0;
            for (auto jt = singleSubBatch.begin(); jt != singleSubBatch.end();
                 jt++) {
              ...
              if (ssId == ssIdEntry) {
                ...
                if (seqIdxEntry != m_PadIndexSeqIndexMap[padIndexEntry]) {
                  LOG_ERROR(
                      "removeSource : Sequence index in m_SubBatchAllocationMap and m_PadIndexSeqIndexMap do not match !!\n");
                  return false;
                }
                m_PadIndexSeqIndexMap.erase(padIndexEntry);
                LOG_INFO(
                    "Removed stream : sub-batch = %d \t seq index = %d \t pad index = %d\n",
                    sbId,
                    seqIdxEntry,
                    padIndexEntry);
                break;
        ...
        /** We can release the dispatch map lock now because a dispatch info is
         * kept valid until its thread joins. */
        lk.unlock();
  ...
  if (removeObjectIdMapping) {
    for (auto it = m_ObjectIdOffsetMap.begin();
         it != m_ObjectIdOffsetMap.end();) {
      if (getStreamId(it->first) ==
          sourceId) /** SurfaceStreamId has source id to delete */
      {
...
bool NvTrackerProc::submitInput(const InputParams& inputParams) {
  ...
  snprintf(
      contextName,
      sizeof(contextName),
      "%s_nvtracker_convert_buffer(Frame=%u)",
      m_Config.gstName,
      m_BatchId);
    ...
    while (m_Running) {
      ...
      if (!m_Running) {
        break;
          ...
          m_ConvBufMgr.isQueueEmpty() && m_Config.inputTensorMeta == false) {
        continue;
      }

      /** Buffer conversion is asynchronous. Call syncBufferSet before low level
       * library accessing transformed surface */
      if (m_Config.inputTensorMeta == false) {
        procParams.pConvBuf = m_ConvBufMgr.convertBatchAsync(
            inputParams.pSurfaceBatch, &procParams.bufSetSyncObjs);
      } else {
        ...
        preprocSurface->surfaceList =
            new NvBufSurfaceParams[preprocSurface->batchSize];
        memset(
            preprocSurface->surfaceList,
            0,
        ...
        for (NvDsMetaList* l_user = batch_meta->batch_user_meta_list;
          ...
          if (std::find(uids.begin(), uids.end(), m_Config.tensorMetaGieId) ==
              uids.end())
            continue;
          ...
          if (preproc_meta->tensor_meta->gpu_id != (guint)m_Config.gpuId) {
            LOG_ERROR(
                "gstnvtracker: GPU ID mismatch of tensor meta and tracker.\n");
          ...
          if (!mem ||
              !gst_memory_is_type(mem, GST_NVDSPREPROCESS_MEMORY_TYPE)) {
          ...
          if (procParams.pConvBuf == nullptr) {
            ...
            for (uint32_t i = 0; (i < preproc_buf->surf->batchSize) &&
                 (preprocSurface->numFilled <
                  inputParams.pSurfaceBatch->batchSize);
                 i++) {
...
bool NvTrackerProc::initTrackerLib() {
  ...
  *(void**)(&m_TrackerLibRetrieveMiscData) =
      dlsym(m_TrackerLibHandle, "NvMOT_RetrieveMiscData");
  if ((error = dlerror()) != NULL) {
    LOG_INFO("gstnvtracker: Optional NvMOT_RetrieveMiscData not implemented\n");
  }

  *(void**)(&m_TrackerLibRemoveStreams) =
      dlsym(m_TrackerLibHandle, "NvMOT_RemoveStreams");
  if ((error = dlerror()) != NULL) {
...
bool NvTrackerProc::initTrackerQuery(
  ...
  if (m_TrackerLibQuery) {
    ...
    for (uint i = 0; i < contextHandleList.size(); i++) {
      ...
      if (m_Config.trackerConfigFilePerSubBatch.size() > i) {
        trackerConfigFile =
            (char*)m_Config.trackerConfigFilePerSubBatch.at(i).c_str();
      } else if (m_Config.trackerConfigFilePerSubBatch.size()) {
        /** If config file is not specified for this sub-batch, use the last
         * config file in the list */
        trackerConfigFile =
            (char*)m_Config.trackerConfigFilePerSubBatch.back().c_str();
      }
      ...
      NvMOTStatus status = m_TrackerLibQuery(
          configFilePathSize, trackerConfigFile, &query_sub_batch);
      if (status != NvMOTStatus_OK) {
        LOG_ERROR(
            "gstnvtracker: Got error querying low-level tracker for caps.\n");
        if (status == NvMOTStatus_Invalid_Path) {
          LOG_ERROR(
              "  Config file path is invalid: %s\n",
      ...
      if (i == 0) {
        ...
        // For the query variables that can be different across sub-batches :
        // Take the max value For the query variables that need to match across
        // sub-batches : verify that they have the same values
        if (query.computeConfig != query_sub_batch.computeConfig) {
          LOG_ERROR(
              "gstnvtracker: All sub-batches should have the same computeConfig setting.\n");
          return false;
        }
        query.numTransforms =
            max(query.numTransforms, query_sub_batch.numTransforms);
        if (query.colorFormats[0] != query_sub_batch.colorFormats[0]) {
          LOG_ERROR(
              "gstnvtracker: All sub-batches should have the same colorFormats setting.\n");
          return false;
        }
        ...
        query.maxShadowTrackingAge = max(
            query.maxShadowTrackingAge, query_sub_batch.maxShadowTrackingAge);
        query.outputReidTensor =
            max(query.outputReidTensor, query_sub_batch.outputReidTensor);
        query.reidFeatureSize =
            max(query.reidFeatureSize, query_sub_batch.reidFeatureSize);
        ...
        query.outputTerminatedTracks =
            max(query.outputTerminatedTracks,
                query_sub_batch.outputTerminatedTracks);
        query.maxTrajectoryBufferLength =
            max(query.maxTrajectoryBufferLength,
                query_sub_batch.maxTrajectoryBufferLength);
...
bool NvTrackerProc::initTrackerQuery(
    const NvMOTContextHandle& contextHandle,
    uint32_t sbId) {
  if (m_TrackerLibQuery) {
    NvMOTQuery query_sub_batch;
    memset(&query_sub_batch, 0, sizeof(query_sub_batch));
    ...
    if (m_Config.trackerConfigFilePerSubBatch.size() > sbId) {
      trackerConfigFile =
          (char*)m_Config.trackerConfigFilePerSubBatch.at(sbId).c_str();
    } else if (m_Config.trackerConfigFilePerSubBatch.size()) {
      /** If config file is not specified for this sub-batch, use the last
       * config file in the list */
      trackerConfigFile =
...
bool NvTrackerProc::initConvBufPool() {
  ...
  bool ret = m_ConvBufMgr.init(
      m_Config.batchSize,
      m_Config.gpuId,
      m_Config.compute_hw,
      createParams,
      m_Config.numTransforms == 0);
  if (!ret) {
...
bool NvTrackerProc::initMiscDataPool() {
  bool ret = m_MiscDataMgr.init(
      m_Config.batchSize,
      m_Config.gpuId,
      m_Config.maxTargetsPerStream,
      m_Config.maxShadowTrackingAge,
      m_Config.reidFeatureSize,
      m_Config.maxMiscDataPoolSize,
      m_Config.pastFrame,
...
NvMOTContextHandle NvTrackerProc::initTrackerContext(uint32_t sbId) {
  ...
  config.maxStreams = (m_Config.subBatchSizes.size() == 0)
      ? m_Config.batchSize
      : m_Config.subBatchSizes[sbId];
  config.maxBufSurfAddrSize = m_Config.maxConvBufPoolSize *
      m_Config.batchSize; /** max num of addresses in buffer pool */
  ...
  if (customConfigFilePath.empty()) {
    LOG_WARNING(
        "gstnvtracker: ll-config-file not specified for sub-batch : %d\n",
        sbId);
  }
...
void NvTrackerProc::queueFrames(
  ...
  for (NvDsFrameMetaList* l = batchMeta.frame_meta_list; l != nullptr;
    ...
    SurfaceStreamId ssId =
        getSurfaceStreamId(pFrameMeta->pad_index, pFrameMeta->surface_index);
    ...
    if (m_PadIndexSsidMap.find(pFrameMeta->pad_index) ==
        m_PadIndexSsidMap.end()) {
      m_PadIndexSsidMap[pFrameMeta->pad_index] = ssId;
    }
  }
}

void NvTrackerProc::fillMOTFrame(
    SurfaceStreamId ssId,
  ...
  if (procParams.pConvBuf != nullptr) {
    ...
    *(motFrame.bufferList) =
        procParams.pConvBuf->surfaceList + frameMeta.batch_id;
  } else {
...
void NvTrackerProc::updatePastFrameMeta(
  ...
  NvDsTargetMiscDataBatch* pPastFrameObjBatch =
      &pGstObj->misc_data_buffer->pastFrameObjBatch;
  if (!batchList.empty()) {
    const std::map<SurfaceStreamId, NvDsFrameMeta*>& frameMap =
        batchList.front();
    for (uint32_t si = 0; si < pPastFrameObjBatch->numFilled; si++) {
      ...
      NvDsTargetMiscDataStream* pPastFrameObjStream =
          pPastFrameObjBatch->list + si;
      ...
      NvBufSurfaceParams* pInputBuf =
          &procParams.input.pSurfaceBatch
               ->surfaceList[frameMap.at(ssId)->batch_id];
      ...
      for (uint32_t li = 0; li < pPastFrameObjStream->numFilled; li++) {
        NvDsTargetMiscDataObject* pPastFrameObjList =
            pPastFrameObjStream->list + li;
        g_strlcpy(
            pPastFrameObjList->objLabel,
  ...
  pPastFrameObjBatch->priv_data =
      (void*)gst_mini_object_ref((GstMiniObject*)pGstObj);
  ...
  user_meta->base_meta.copy_func =
      (NvDsMetaCopyFunc)copy_nvtracker2_past_frame_meta;
  user_meta->base_meta.release_func =
      (NvDsMetaReleaseFunc)free_nvtracker2_past_frame_meta;
  user_meta->base_meta.batch_meta = pBatchMeta;
  nvds_add_user_meta_to_batch(pBatchMeta, user_meta);
}

void NvTrackerProc::updateBatchReidMeta(
    GstNvTrackerMiscDataObject* pGstObj,
    ProcParams& procParams) {
  ...
  NvDsReidTensorBatch* pReidTensor =
      &pGstObj->misc_data_buffer->reidTensorBatch;
  ...
  user_meta->base_meta.copy_func =
      (NvDsMetaCopyFunc)copy_nvtracker2_batch_reid_meta;
  user_meta->base_meta.release_func =
      (NvDsMetaReleaseFunc)free_nvtracker2_batch_reid_meta;
  user_meta->base_meta.batch_meta = pBatchMeta;
  nvds_add_user_meta_to_batch(pBatchMeta, user_meta);
}

void NvTrackerProc::updateTerminatedTrackMeta(
    GstNvTrackerMiscDataObject* pGstObj,
    ProcParams& procParams) {
  ...
  user_meta->base_meta.meta_type =
      (NvDsMetaType)NVDS_TRACKER_TERMINATED_LIST_META;
  user_meta->base_meta.copy_func =
      (NvDsMetaCopyFunc)copy_nvtracker2_terminated_meta;
  user_meta->base_meta.release_func =
      (NvDsMetaReleaseFunc)free_nvtracker2_terminated_meta;
  user_meta->base_meta.batch_meta = pBatchMeta;
  nvds_add_user_meta_to_batch(pBatchMeta, user_meta);
}

void NvTrackerProc::updateShadowTrackMeta(
    GstNvTrackerMiscDataObject* pGstObj,
    ProcParams& procParams) {
  ...
  user_meta->base_meta.copy_func =
      (NvDsMetaCopyFunc)copy_nvtracker2_shadowTracks_meta;
  user_meta->base_meta.release_func =
      (NvDsMetaReleaseFunc)free_nvtracker2_shadowTracks_meta;
  user_meta->base_meta.batch_meta = pBatchMeta;
  nvds_add_user_meta_to_batch(pBatchMeta, user_meta);
}
...
void NvTrackerProc::updateUserMeta(
  ...
  /** mini_obj_ref number is alreadt 1 after initialization, so descrease by 1
   * here. */
...
void NvTrackerProc::updateBatchMeta(
    const NvMOTTrackedObjBatch& procResult,
    const ProcParams& procParams,
...
void NvTrackerProc::updateFrameMeta(
  ...
  for (uint32_t i = 0; i < trackedObjList.numFilled; i++) {
     ...
     * If none is found, then this is tracked from previous frames and possibly
     * missed by the detector in this frame. */
    ...
    bool clipRet =
        clipBBox(pInputBuf->width, pInputBuf->height, left, top, width, height);
    if (NULL != pInObj) {
      ...
      if (!clipRet) {
        ...
        LOG_WARNING(
            "gstnvtracker: obj %lu Class mismatch! %d -> %d\n",
            pObjectMeta->object_id,
        ...
        pObjectMeta->tracker_bbox_info.org_bbox_coords.height =
            height_unclipped;
        ...
        updateObjectProjectionMeta(
            pObjectMeta, pTrackedObj, pBatchMeta, scaleWidth, scaleHeight);
      }
    } else if (clipRet) {
      ...
      if (it != m_ClassInfoMap.end()) {
        ...
        *pTextParams = it->second.textParams;
        pTextParams->display_text =
            strdup(it->second.displayTextString.c_str());
...
void NvTrackerProc::updateObjectReidMeta(
    NvDsObjectMeta* pObjectMeta,
  ...
  if (m_Config.outputReidTensor && pTrackedObj->reid.featureSize > 0 &&
    ...
    user_meta->base_meta.copy_func =
        (NvDsMetaCopyFunc)copy_nvtracker2_obj_reid_meta;
    user_meta->base_meta.release_func =
        (NvDsMetaReleaseFunc)free_nvtracker2_obj_reid_meta;
    nvds_add_user_meta_to_obj(pObjectMeta, user_meta);
  }
}

void NvTrackerProc::updateObjectProjectionMeta(
    NvDsObjectMeta* pObjectMeta,
    NvMOTTrackedObj* pTrackedObj,
    NvDsBatchMeta* pBatchMeta,
    float scaleWidth,
  ...
  if (m_Config.outputVisibility) {
    ...
    *pVis = pTrackedObj->visibility;
    ...
    user_meta->base_meta.copy_func =
        (NvDsMetaCopyFunc)copy_nvtracker2_obj_visibility_meta;
    user_meta->base_meta.release_func =
  ...
  if (m_Config.outputFootLocation) {
    {
      ...
      user_meta->base_meta.meta_type =
          (NvDsMetaType)NVDS_OBJ_IMAGE_FOOT_LOCATION;
      user_meta->base_meta.copy_func =
          (NvDsMetaCopyFunc)copy_nvtracker2_obj_foot_loc_meta;
      user_meta->base_meta.release_func =
    ...
    {
      /** Output estimated foot location in 2D coordinates on the estimated
       * world ground. */
      NvDsUserMeta* user_meta = nvds_acquire_user_meta_from_pool(pBatchMeta);
      ...
      user_meta->base_meta.meta_type =
          (NvDsMetaType)NVDS_OBJ_WORLD_FOOT_LOCATION;
      user_meta->base_meta.copy_func =
          (NvDsMetaCopyFunc)copy_nvtracker2_obj_foot_loc_meta;
      user_meta->base_meta.release_func =
          (NvDsMetaReleaseFunc)free_nvtracker2_obj_foot_loc_meta;
  ...
  if (m_Config.outputConvexHull &&
      pTrackedObj->convexHull.numPointsAllocated != 0) {
    NvDsUserMeta* user_meta = nvds_acquire_user_meta_from_pool(pBatchMeta);
    ...
    for (uint32_t i = 0; i < pTrackedObj->convexHull.numPoints; i++) {
      pConvexHull->list[2 * i] =
          (int)(pTrackedObj->convexHull.list[2 * i] * scaleWidth);
    ...
    pConvexHull->numPointsAllocated =
        pTrackedObj->convexHull.numPointsAllocated;
    user_meta->user_meta_data = (void*)pConvexHull;
    user_meta->base_meta.meta_type = (NvDsMetaType)NVDS_OBJ_IMAGE_CONVEX_HULL;
    user_meta->base_meta.copy_func =
        (NvDsMetaCopyFunc)copy_nvtracker2_obj_convex_hull_meta;
    user_meta->base_meta.release_func =
...
guint64 NvTrackerProc::objectIdMapping(
    const guint64& objectId,
  ...
  if (m_Config.trackingIdResetMode & TrackingIdResetMode_FromZeroAfterEOS) {
    ...
    return (objectId & 0xffffffff00000000) +
        (guint32)(objectId - m_ObjectIdOffsetMap.at(ssId));
  } else {
    return objectId;
  }
}

void NvTrackerProc::allocateConvexHullMemory(
    NvMOTTrackedObj* list,
    const uint32_t numAllocated) {
  if ((!m_Config.outputConvexHull) || m_Config.maxConvexHullSize == 0 ||
      list == nullptr) {
    return;
  }
...
void NvTrackerProc::releaseConvexHullMemory(
    NvMOTTrackedObj* list,
    const uint32_t numAllocated) {
  if ((!m_Config.outputConvexHull) || m_Config.maxConvexHullSize == 0 ||
      list == nullptr) {
...
void NvTrackerProc::allocateProcessMemory(
  ...
  if (procInput.frameList == nullptr) {
    LOG_ERROR(
        "gstnvtracker: Failed to allocate resources (frame) for batch processing.\n");
    m_TrackerLibError = true;
  } else {
    for (uint32_t i = 0; i < m_Config.batchSize; i++) {
      ...
      if (procInput.frameList[i].objectsIn.list == nullptr ||
        ...
        LOG_ERROR(
            "gstnvtracker: Failed to allocate resources (objToTrack) for batch processing.\n");
        m_TrackerLibError = true;
      }
      procInput.frameList[i].objectsIn.numAllocated =
          m_Config.maxTargetsPerStream;
    }
  }
  ...
  if (procResult.list == nullptr) {
    ...
    for (uint32_t i = 0; i < m_Config.batchSize; i++) {
      procResult.list[i].list =
          new NvMOTTrackedObj[m_Config.maxTargetsPerStream];
      if (procResult.list[i].list == nullptr) {
        LOG_ERROR(
            "gstnvtracker: Failed to allocate resources (trackedObj) for batch processing.\n");
        m_TrackerLibError = true;
      }
      procResult.list[i].numAllocated = m_Config.maxTargetsPerStream;
      allocateConvexHullMemory(
          procResult.list[i].list, procResult.list[i].numAllocated);
    }
  }
...
void NvTrackerProc::releaseProcessMemory(
    NvMOTProcessParams& procInput,
    NvMOTTrackedObjBatch& procResult) {
  if (procInput.frameList != nullptr) {
    for (uint32_t i = 0; i < m_Config.batchSize; i++) {
...
void NvTrackerProc::processBatch() {
  ...
  while (m_Running) {
    ...
    /* Only process when low level tracker works fine; otherwise send
     * unprocessed as output. The gst pipeline will send quit signal and call
     * tracker plugin stop function to terminate all other threads. */
    if (!m_TrackerLibError.load()) {
      /* In case the batch contains two frames with the same ssId,
       * sort the frames based on frameNum in ascending order.
       * such as {(ssId 1, frameNum 1); (ssId 2, frameNum 1); (ssId 1, frameNum
       * 2)} In this case, low level lib is called multiple times to track based
       * on frameNum in ascending order. */
      std::vector<std::map<SurfaceStreamId, NvDsFrameMeta*>> batchList;
      ...
      for (uint listInd = 0; listInd < batchList.size(); listInd++) {
        ...
        if (m_Config.inputTensorMeta == false) {
          m_ConvBufMgr.syncBuffer(
              procParams.pConvBuf, &procParams.bufSetSyncObjs);
        }
        ...
        if (NvMOTStatus_OK != status) {
          LOG_ERROR(
              "gstnvtracker: Low-level tracker lib returned error %d\n",
              status);
          m_TrackerLibError = true;
        }

        if (m_TrackerLibRetrieveMiscData) {
          status = m_TrackerLibRetrieveMiscData(
              m_BatchContextHandle, &procInput, &miscDataResult);
          if (NvMOTStatus_OK != status) {
            LOG_ERROR(
                "gstnvtracker: When flushing previous frames, low-level tracker lib returned error %d\n",
                status);
            m_TrackerLibError = true;
          }
        }
...
bool NvTrackerProc::mapStreamToSubBatch(
    SurfaceStreamId ssId,
    uint32_t pad_index,
  ...
  if (dynamicSubBatching) {
    ...
    for (sbSizeCnt = 1; sbSizeCnt <= m_MaxSubBatchSize; sbSizeCnt++) {
      for (sbId = 0; sbId < m_Config.subBatchSizes.size(); sbId++) {
        if (m_SubBatchAllocationMap.find(sbId) ==
            m_SubBatchAllocationMap.end()) {
        ...
        if ((curSize < sbSizeCnt) && (curSize < m_Config.subBatchSizes[sbId])) {
          ...
          std::map<SeqIndex, std::pair<SurfaceStreamId, uint32_t>>&
              singleSubBatch = m_SubBatchAllocationMap[sbId];
          SeqIndex seqIdx = 0;
          for (seqIdx = 0; seqIdx < m_Config.subBatchSizes[sbId]; seqIdx++) {
            if (singleSubBatch.find(seqIdx) == singleSubBatch.end()) {
              ...
              m_SubBatchAllocationMap[sbId][seqIdx] =
                  std::make_pair(ssId, pad_index);
              m_SsidSubBatchMap[ssId] = sbId;
              m_PadIndexSeqIndexMap[pad_index] = seqIdx;
              LOG_INFO(
                  "Added stream : sub-batch = %d \t seq index = %d \t pad index = %d\n",
                  sbId,
                  seqIdx,
                  pad_index);
              return true;
          ...
          if (seqIdx >= m_Config.subBatchSizes[sbId]) {
            LOG_ERROR(
                "gstnvtracker: Sequence Index %d is out of range for sub-batch id %d which has a size of %d \n",
                seqIdx,
                sbId,
                m_Config.subBatchSizes[sbId]);
            return false;
    ...
    if (sbId >= m_Config.subBatchesConfig.size()) {
      LOG_ERROR(
          "gstnvtracker: Source id %d is not mapped to any sub-batch !! Check \"sub-batches\" configuraion. \n",
          pad_index);
      return false;
    }
    m_SsidSubBatchMap[ssId] = sbId;
...
bool NvTrackerProc::dispatchSubBatches(ProcParams procParams) {
  ...
  procParams.batchId = m_BatchId;
  // Should not be unlocked here since "nextBatchRef" will be used to add the
  // "SubBatchId" values pendingBatchLock.unlock();
  ...
  for (uint listInd = 0; listInd < batchList.size(); listInd++) {
    ...
    for (it = frameMap.begin(); it != frameMap.end(); ++it) {
      ...
      if (m_SsidSubBatchMap.find(ssId) == m_SsidSubBatchMap.end()) {
        ...
        if (!mapStreamToSubBatch(
                ssId, pFrameMeta->pad_index, m_Config.dynamicSubBatching)) {
          pendingBatchLock.unlock();
          return false;
        }
      }
    ...
    for (uint sbInd = 0; sbInd < subbBatchList.size(); sbInd++) {
      std::map<SurfaceStreamId, NvDsFrameMeta*>& subBatch =
          subbBatchList.at(sbInd);
      if (subBatch.size() == 0)
        continue;
      DispatchReq req = {};
      ...
      if (it != m_DispatchMap.end()) {
        ...
        LOG_ERROR(
            "gstnvtracker: No thread corresponding to sub-batch %d\n", sbInd);
        lk.unlock();
        pendingBatchLock.unlock();
        return false;
      }
...
void NvTrackerProc::processSubBatch(DispatchInfo* pDispatchInfo) {
  ...
  if (!miscDataMgr.init(
          ...
          m_Config.maxTrajectoryBufferLength)) {
    LOG_ERROR(
        "gstnvtracker: Failed to init TrackerMiscDataManager context for sub-batch %x\n",
        sbId);
    m_TrackerLibError = true;
  }
  ...
  while (pDispatchInfo->running) {
    ...
    if (!m_TrackerLibError.load()) {
      ...
      if (m_Config.inputTensorMeta == false) {
        ...
        if (it != m_ConvBufComplete.end()) {
          if (m_ConvBufComplete[req.batchId] == false) {
            ...
            // LOG_DEBUG("gstnvtracker: ConvBuf transformation is already
            // complete for batch %d.!\n", req.batchId);
          }
          lkBatch.unlock();
        } else {
          lkBatch.unlock();
      ...
      snprintf(
          contextName,
          sizeof(contextName),
          "%s_nvtracker_process(SubBatchId=%u Frame=%u)",
          m_Config.gstName,
          sbId,
          procParams.batchId);
      nvtx_helper_push_pop(contextName);
      ...
      NvMOTStatus status =
          m_TrackerLibProcess(contextHandle, &procInput, &procResult);
      nvtx_helper_push_pop(NULL);
      if (NvMOTStatus_OK != status) {
        LOG_ERROR(
      ...
      if (m_TrackerLibRetrieveMiscData) {
        status = m_TrackerLibRetrieveMiscData(
            contextHandle, &procInput, &miscDataResult);
        if (NvMOTStatus_OK != status) {
          LOG_ERROR(
              "gstnvtracker: When flushing previous frames, low-level tracker lib returned error %d\n",
              status);
  ...
  req_complete:
    ...
    if (it != m_PendingBatch.end()) {
      ...
      if (itSB != it->second.end()) {
        ...
        LOG_ERROR(
            "gstnvtracker: completed sub-batch req is not in a pending batch!\n");
        m_TrackerLibError = true;
      }
      lkBatch.unlock();
    } else {
    ...
    if (trackerLibProcessError) {
      if ((m_Config.subBatchErrRecoveryTrialCnt == -1) ||
        ...
        // The low level lib returned an error while processing the current
        // sub-batch Deinit and Init the lowlevel lib to recover from the error
        ...
        if (contextHandle == NULL) {
          LOG_ERROR(
              "gstnvtracker: Failed to init context for sub-batch %d\n", sbId);
          m_TrackerLibError = true;
        }
        if (!initTrackerQuery(contextHandle, sbId)) {
          LOG_ERROR(
              "gstnvtracker: Failed to initilaize MOT query for sub-batch %d\n",
              sbId);
          m_TrackerLibError = true;
        }
        errRecoveryTrials++;
      } else {
        // Max allowed trials to recover from sub-batch  processing error were
        // unsuccessful Signal error
        m_TrackerLibError = true;
      }
    }
  } /** while (pDispatchInfo->running) */
 ...
 * Terminologies :
 * Pad index : pad index is a value that's included in "NvDsFrameMeta" by
 * nvstreammux This corresponds to the index of the sink pad to which the source
 * is connected. This term is used interchangeably with source id / stream id.
 * SurfaceStreamId : Composition of unique ID for each surface from each source.
 *      Created using the function getSurfaceStreamId() which takes pad index
 * and surface id as input. sub-batch id : Id of a sub-batch. Sub-batch id
 * ranges from 0 to (no of sub-batches - 1) sub-batch size : Number of streams
 * in a sub-batch as derived from the "sub-batches" configuration seq index  :
 * Position of a stream inside a sub-batch. For each sub-batch, seq index ranges
 * from 0 to (sub-batch size -1). Seq index is used to fill the value
 * "seq_index" in "NvMOTFrame"
 * **************************************************************************
 ...
 * <sub-batch id, seq index> to a <SurfaceStreamId , pad index> pair as shown in
 * examples below. Load balancing logic makes sure that all the sub-batches are
 * loaded equally
 *
 ...
 * <ssid3,n3>));(1,(0,<ssid2,n2>)) m_PadIndexSeqIndexMap =
 * (n1->0);(n2->0);(n3->1)
 *
 ...
 * m_SubBatchAllocationMap =  (0,(0, <ssid1,n1>));(0,(1, <ssid3,n3>));(0,(2,
 * <ssid5,n5>));(1,(1,<ssid4,n4>)) m_PadIndexSeqIndexMap =
 * (n1->0);(n3->1);(n4->1);(n5->2)
 *
 * Event : Remove stream with pad index = n4
 * m_SubBatchAllocationMap =   (0,(0, <ssid1,n1>));(0,(1, <ssid3,n3>));(0,(2,
 * <ssid5,n5>)) m_PadIndexSeqIndexMap = (n1->0);(n3->1);(n5->2)
 *
 ...
 * <ssid5,n5>));(1,(0,<ssid6,n6>)); m_PadIndexSeqIndexMap =
 * (n3->1);(n5->2);(n6->0)
 *
