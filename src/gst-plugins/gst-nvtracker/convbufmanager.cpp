...
bool ConvBufManager::init(
  ...
  m_TransformConfigParams.compute_mode =
      static_cast<NvBufSurfTransform_Compute>(compute_hw);
  ...
  cudaReturn = cudaStreamCreateWithFlags(
      &m_TransformConfigParams.cuda_stream, cudaStreamNonBlocking);
  if (cudaReturn != cudaSuccess) {
    LOG_ERROR(
        "gstnvtracker: Failed to create cuda stream for buffer conversion: %s\n",
        cudaGetErrorName(cudaReturn));
    return false;
  }
  ...
  m_TransformParams.src_rect = (NvBufSurfTransformRect*)calloc(
      batchSize, sizeof(NvBufSurfTransformRect));
  m_TransformParams.dst_rect = (NvBufSurfTransformRect*)calloc(
      batchSize, sizeof(NvBufSurfTransformRect));

  /** Create the buffers. The proper way is to set number of buffer sets as a
   * fixed number. */
...
#endif
        /** Need to loop for now because the batch mapping only happens for
         * filled buffers */
...
void ConvBufManager::deInit() {
  ...
  while (!m_FreeQueue.empty()) {
    ...
    if (pBuffer->memType == NVBUF_MEM_SURFACE_ARRAY) {
      ...
      if (ret < 0) {
        LOG_WARNING(
            "gstnvtracker: Got %d unmapping nvbufsurface %p\n", ret, pBuffer);
      }
      ret = NvBufSurfaceUnMapEglImage(pBuffer, -1);
      if (ret < 0) {
        LOG_WARNING(
            "gstnvtracker: Got %d unmapping egl image for nvbufsurface %p\n",
            ret,
            pBuffer);
      }
    }
...
NvBufSurface* ConvBufManager::convertBatchAsync(
    NvBufSurface* pBatchIn,
  ...
  if (err != NvBufSurfTransformError_Success) {
    LOG_ERROR(
        "gstnvtracker: NvBufSurfTransformSetSessionParams failed with error %d",
        err);
    return nullptr;
  }
  ...
  for (uint32_t frameInd = 0; frameInd < batchSize; frameInd++) {
    ...
    m_TransformParams.dst_rect[frameInd].width =
        pBuffer->surfaceList[frameInd].width;
    m_TransformParams.dst_rect[frameInd].height =
        pBuffer->surfaceList[frameInd].height;
  }
  *bufSetSyncObjs = nullptr;
  err = NvBufSurfTransformAsync(
      pBatchIn, pBuffer, &m_TransformParams, bufSetSyncObjs);
  if (err != NvBufSurfTransformError_Success) {
    LOG_ERROR(
        "gstnvtracker: NvBufSurfTransform failed with error %d while converting buffer",
        err);
...
void ConvBufManager::syncBuffer(
    NvBufSurface* pBuffer,
    NvBufSurfTransformSyncObj_t* bufSetSyncObjs) {
  if (m_IntentionallyEmpty) {
    return;
  }
