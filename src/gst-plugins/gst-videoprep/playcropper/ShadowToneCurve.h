...
// so the curve cannot reverse tones even at the maximum supported lift.
HM_SHADOW_HOST_DEVICE inline float evaluate_shadow_lift_curve(float sample, float lift_percent) {
  ...
  const float ocio_adjustment = 1.0f + amount * ((kShadowLiftMaximumOcioAdjustment - 1.0f) / kShadowLiftMaximumPercent);
  ...
  return sample < y1 ? out_left : out_right;
}
