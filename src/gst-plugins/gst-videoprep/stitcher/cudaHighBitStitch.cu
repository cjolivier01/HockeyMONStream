...
__global__ void convertHalf4ToBgr10A2Kernel(
    const uint8_t* input,
    size_t input_pitch,
    int input_width,
    int input_height,
  ...
  const float source_x = cos_angle * x + sin_angle * y + (1.0f - cos_angle) * center_x - sin_angle * center_y;
  const float source_y = -sin_angle * x + cos_angle * y + sin_angle * center_x + (1.0f - cos_angle) * center_y;
  auto* output_row = reinterpret_cast<uint32_t*>(output + static_cast<size_t>(y) * output_pitch);
  if (source_x < 0.0f || source_y < 0.0f || source_x > input_width - 1 || source_y > input_height - 1) {
    output_row[x] = 0;
    return;
  }
...
#pragma unroll
  for (int channel = 0; channel < 4; ++channel) {
    ...
    value = playcropper::clamp_shadow_value(value, 0.0f, 255.0f);
    channels[channel] = channel < 3 ? static_cast<uint32_t>(lrintf(value * (1023.0f / 255.0f)))
                                    : static_cast<uint32_t>(lrintf(value * (3.0f / 255.0f)));
  }
  // BGR10A2_LE stores B, G, R, A from the least- to most-significant field.
  output_row[x] = channels[2] | (channels[1] << 10) | (channels[0] << 20) | (channels[3] << 30);
}
...
cudaError_t convertHalf4ToBgr10A2(
    ...
    float exposure,
    cudaStream_t stream) {
  const bool supported_output = output &&
      (output->colorFormat == NVBUF_COLOR_FORMAT_BGRA_10_10_10_2_709 ||
       output->colorFormat == NVBUF_COLOR_FORMAT_BGRA_10_10_10_2_2020);
  if (!input || !supported_output || !output->dataPtr || input_width < 2 || input_height < 2 || output->width < 1 ||
      output->height < 1) {
    return cudaErrorInvalidValue;
  }
  const float radians = static_cast<float>(-rotation_degrees * M_PI / 180.0);
  ...
  const dim3 block(16, 16);
  const dim3 grid((output->width + block.x - 1) / block.x, (output->height + block.y - 1) / block.y);
  convertHalf4ToBgr10A2Kernel<<<grid, block, 0, stream>>>(
      reinterpret_cast<const uint8_t*>(input),
      input_pitch,
      input_width,
      input_height,
