...
static void gst_gst_my_audio_filter_set_property(
    GObject* object,
    guint property_id,
    const GValue* value,
    GParamSpec* pspec);
static void gst_gst_my_audio_filter_get_property(
    GObject* object,
    guint property_id,
    GValue* value,
    GParamSpec* pspec);
...
static gboolean gst_gst_my_audio_filter_setup(
    GstAudioFilter* filter,
    const GstAudioInfo* info);
static GstFlowReturn gst_gst_my_audio_filter_transform(
    GstBaseTransform* trans,
    GstBuffer* inbuf,
    GstBuffer* outbuf);
static GstFlowReturn gst_gst_my_audio_filter_transform_ip(
    GstBaseTransform* trans,
    GstBuffer* buf);
static void gst_gst_my_audio_filter_init(GstGstMyAudioFilter* myaudiofilter);

// Chain
static GstFlowReturn gst_my_filter_chain(
    GstPad* pad,
    GstObject* parent,
    GstBuffer* buf);
// Events
static gboolean gst_my_filter_sink_event(
    GstPad* pad,
    GstObject* parent,
    GstEvent* event);

// Queries
static gboolean gst_my_filter_src_query(
    GstPad* pad,
    GstObject* parent,
    GstQuery* query);
// State change
static void gst_my_filter_change_state(
    GstElement* element,
    GstState oldstate,
    GstState newstate,
    GstState pending);
...
G_DEFINE_TYPE_WITH_CODE(
    GstGstMyAudioFilter,
    gst_gst_my_audio_filter,
    GST_TYPE_AUDIO_FILTER,
    GST_DEBUG_CATEGORY_INIT(
        gst_gst_my_audio_filter_debug_category,
        "myaudiofilter",
        0,
...
void gst_gst_my_audio_filter_set_property(
    GObject* object,
    guint property_id,
    const GValue* value,
    GParamSpec* pspec) {
  ...
  switch (property_id) {
    case PROP_SILENT:
      myaudiofilter->silent = g_value_get_boolean(value);
      g_print(
          "Silent argument was changed to %s\n",
          myaudiofilter->silent ? "true" : "false");
      break;
    default:
...
void gst_gst_my_audio_filter_get_property(
    GObject* object,
    guint property_id,
    GValue* value,
    GParamSpec* pspec) {
 ...
 *
 */
static void gst_gst_my_audio_filter_class_init(
    GstGstMyAudioFilterClass* klass) {
...
static GstFlowReturn gst_my_filter_chain(
  ...
  if (!myaudiofilter->silent) {
    g_print(
        "Have data of size %" G_GSIZE_FORMAT " bytes!\n",
...
static gboolean gst_my_filter_stop_processing(
    GstGstMyAudioFilter* myaudiofilter) {
  if (!myaudiofilter->silent) {
...
static gboolean plugin_init(GstPlugin* plugin) {
  ...
  return gst_element_register(
      plugin, "myaudiofilter", GST_RANK_NONE, GST_TYPE_GST_AUDIO_FILTER);
}
...
GST_PLUGIN_DEFINE(
    GST_VERSION_MAJOR,
    GST_VERSION_MINOR,
    nvdsgst_myaudiofilter,
    "My audio filter plugin",
    plugin_init,
    VERSION,
    "LGPL",
    PACKAGE_NAME,
