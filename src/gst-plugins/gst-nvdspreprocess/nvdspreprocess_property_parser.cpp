...
#define CHECK_INT_VALUE_NON_NEGATIVE(prop_name, value, group)       \
  G_STMT_START {                                                    \
    if ((gint)value < 0) {                                          \
      PARSE_ERROR(                                                  \
          "Integer property '%s' in group '%s' can have value >=0", \
          prop_name,                                                \
          group);                                                   \
    }                                                               \
  }                                                                 \
...
#define CHECK_INT_VALUE_RANGE(prop_name, value, group, min, max)              \
  G_STMT_START {                                                              \
    if ((gint)value < min || (gint)value > max) {                             \
      PARSE_ERROR(                                                            \
          "Integer property '%s' in group '%s' can have value >=%d and <=%d", \
          prop_name,                                                          \
          group,                                                              \
          min,                                                                \
          max);                                                               \
...
#define EXTRACT_STREAM_ID(for_key)                                   \
  {                                                                  \
    ...
    source_index =                                                   \
        g_ascii_strtoull(tmp[g_strv_length(tmp) - 1], &endptr1, 10); \
    g_strfreev(tmp);                                                 \
  }
...
static gboolean nvdspreprocess_parse_property_group(
    GstNvDsPreProcess* nvdspreprocess,
    gchar* cfg_file_path,
    GKeyFile* key_file,
    gchar* group);

static gboolean nvdspreprocess_parse_common_group(
    GstNvDsPreProcess* nvdspreprocess,
    gchar* cfg_file_path,
    GKeyFile* key_file,
    gchar* group,
...
static gboolean nvdspreprocess_parse_user_configs(
    GstNvDsPreProcess* nvdspreprocess,
    gchar* cfg_file_path,
    GKeyFile* key_file,
    gchar* group);
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, NVDSPREPROCESS_PROPERTY_ENABLE)) {
      gboolean val = g_key_file_get_boolean(
          key_file, group, NVDSPREPROCESS_PROPERTY_ENABLE, &error);
      CHECK_ERROR(error, group);
      nvdspreprocess->enable = val;
    } else if (!g_strcmp0(*key, NVDSPREPROCESS_PROPERTY_UNIQUE_ID)) {
      gboolean val = g_key_file_get_integer(
          key_file, group, NVDSPREPROCESS_PROPERTY_UNIQUE_ID, &error);
      CHECK_ERROR(error, group);
      nvdspreprocess->unique_id = val;
    } else if (!g_strcmp0(*key, NVDSPREPROCESS_PROPERTY_GPU_ID)) {
      gboolean val = g_key_file_get_integer(
          key_file, group, NVDSPREPROCESS_PROPERTY_GPU_ID, &error);
      CHECK_ERROR(error, group);
      nvdspreprocess->gpu_id = val;
    } else if (!g_strcmp0(*key, NVDSPREPROCESS_PROPERTY_PROCESS_ON_FRAME)) {
      gboolean val = g_key_file_get_boolean(
          key_file, group, NVDSPREPROCESS_PROPERTY_PROCESS_ON_FRAME, &error);
      CHECK_ERROR(error, group);
      nvdspreprocess->process_on_frame = val;
    } else if (!g_strcmp0(
                   *key, NVDSPREPROCESS_PROPERTY_MAINTAIN_ASPECT_RATIO)) {
      gboolean val = g_key_file_get_boolean(
          key_file,
          group,
      ...
      gboolean val = g_key_file_get_boolean(
          key_file, group, NVDSPREPROCESS_PROPERTY_SYMMETRIC_PADDING, &error);
      CHECK_ERROR(error, group);
      ...
      nvdspreprocess->property_set.processing_height = TRUE;
                   ...
                   *key, NVDSPREPROCESS_PROPERTY_SCALING_BUF_POOL_SIZE)) {
      guint val = g_key_file_get_integer(key_file, group, *key, &error);
      CHECK_ERROR(error, group);
      ...
      target_unique_ids_list = g_key_file_get_integer_list(
          key_file, group, *key, &target_unique_ids_list_len, &error);
      ...
      for (gsize icnt = 0; icnt < target_unique_ids_list_len; icnt++) {
        nvdspreprocess->target_unique_ids.push_back(
            target_unique_ids_list[icnt]);
        GST_CAT_INFO(
            NVDSPREPROCESS_CFG_PARSER_CAT,
      ...
      network_shape_list = g_key_file_get_integer_list(
          key_file, group, *key, &network_shape_list_len, &error);
      ...
      int val = g_key_file_get_integer(
          key_file, group, NVDSPREPROCESS_PROPERTY_SCALING_FILTER, &error);
      CHECK_ERROR(error, group);
      ...
      nvdspreprocess->property_set.tensor_data_type = TRUE;
                   ...
                   *key, NVDSPREPROCESS_PROPERTY_SCALING_POOL_MEMORY_TYPE)) {
      int val = g_key_file_get_integer(key_file, group, *key, &error);
      CHECK_ERROR(error, group);
      ...
      nvdspreprocess->property_set.scaling_pool_memory_type = TRUE;
                   ...
                   *key, NVDSPREPROCESS_PROPERTY_SCALING_POOL_COMPUTE_HW)) {
      int val = g_key_file_get_integer(key_file, group, *key, &error);
      CHECK_ERROR(error, group);
      ...
      GET_STRING_PROPERTY(
          group, *key, nvdspreprocess->tensor_params.tensor_name);
      GST_CAT_INFO(
          NVDSPREPROCESS_CFG_PARSER_CAT,
      ...
      if (!get_absolute_file_path(
              cfg_file_path, str.c_str(), nvdspreprocess->custom_lib_path)) {
      ...
      nvdspreprocess->property_set.custom_lib_path = TRUE;
                   ...
                   *key, NVDSPREPROCESS_PROPERTY_TENSOR_PREPARATION_FUNCTION)) {
      GET_STRING_PROPERTY(
          group, *key, nvdspreprocess->custom_tensor_function_name);
      GST_CAT_INFO(
          NVDSPREPROCESS_CFG_PARSER_CAT,
          "Parsed %s=%s in group '%s'\n",
          *key,
...
static gboolean nvdspreprocess_parse_common_group(
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, NVDSPREPROCESS_GROUP_SRC_IDS)) {
      src_list = g_key_file_get_integer_list(
          key_file, group, *key, &src_list_len, &error);
      if (src_list == nullptr) {
        CHECK_ERROR(error, group);
      }
      src_ids.clear();
      ...
      GET_STRING_PROPERTY(
          group, *key, preprocess_group->custom_transform_function_name);
      GST_CAT_INFO(
          NVDSPREPROCESS_CFG_PARSER_CAT,
          "Parsed %s=%s in group '%s'\n",
          NVDSPREPROCESS_GROUP_CUSTOM_INPUT_PREPROCESS_FUNCTION,
      ...
      GST_DEBUG_OBJECT(
          nvdspreprocess,
          "Custom Transformation Function = %s\n",
          preprocess_group->custom_transform_function_name.c_str());
    } else if (!g_strcmp0(*key, NVDSPREPROCESS_GROUP_OPERATE_ON_CLASS_IDS)) {
      class_list = g_key_file_get_integer_list(
          key_file, group, *key, &class_list_len, &error);
      ...
      GST_CAT_INFO(
          ...
          "Parsed %s=%d in group '%s'\n",
          *key,
          preprocess_group->min_input_object_width,
          group);
      ...
      GST_CAT_INFO(
          ...
          "Parsed %s=%d in group '%s'\n",
          *key,
          preprocess_group->min_input_object_height,
          group);
      ...
      GST_CAT_INFO(
          ...
          "Parsed %s=%d in group '%s'\n",
          *key,
          preprocess_group->max_input_object_width,
          group);
      ...
      GST_CAT_INFO(
          ...
          "Parsed %s=%d in group '%s'\n",
          *key,
          preprocess_group->max_input_object_height,
          group);
      ...
      GST_CAT_INFO(
          ...
          "Parsed %s=%d in group '%s'\n",
          *key,
          preprocess_group->interval,
          group);
      nvdspreprocess->property_set.interval = TRUE;
    } else if (!g_strcmp0(*key, NVDSPREPROCESS_GROUP_ROI_COLOR)) {
      gsize roi_color_list_len = 0;
      gdouble* roi_color_list = g_key_file_get_double_list(
          key_file, group, *key, &roi_color_list_len, &error);
      if (roi_color_list == nullptr) {
        CHECK_ERROR(error, group);
      }
      if (roi_color_list_len != 4) {
        g_printerr(
            "Error: Group %s, Number of Color params should be exactly 4 floats {r, g, b, a} between 0 and 1",
            group);
        goto done;
      }
      ...
      roi_list = g_key_file_get_integer_list(
          key_file, group, *key, &roi_list_len, &error);
      if (roi_list == nullptr) {
        CHECK_ERROR(error, group);
      }
      ...
      if ((roi_list_len & 3) == 0) {
        ...
        printf(
            "ERROR: %s : roi list length for source %d is not a multiple of 4\n",
            __func__,
            (int)source_index);
        goto done;
      }
      ...
      GST_DEBUG(
          "Parsing roi-params source_index = %ld num-roi = %d roilistlen = %ld\n",
          source_index,
          num_roi_per_stream,
          roi_list_len);
      ...
      for (guint i = 0; i < roi_list_len; i = i + 4) {
        ...
        GST_DEBUG(
            "parsed ROI left=%f top=%f width=%f height=%f\n",
            roi_info.roi.left,
            roi_info.roi.top,
            roi_info.roi.width,
            roi_info.roi.height);
      ...
      if (same_roi_for_all_srcs) {
        /* same roi of replicated_src is used for all the sources within the
         * group*/
        preprocess_group->framemeta_map.emplace(
            source_index,
            preprocess_group
                ->framemeta_map[preprocess_group->replicated_src_id]);
      } else {
  ...
  if (nvdspreprocess->process_on_frame) {
    if (preprocess_group->process_on_roi) {
      if (!(nvdspreprocess->property_set.src_ids &&
        ...
        printf(
            "ERROR: Some preprocess group config properties not set in preprocess config file\n");
        return FALSE;
      }
    } else {
      if (!(nvdspreprocess->property_set.src_ids &&
    ...
    if (!preprocess_group->process_on_all_objects) {
      if (!(nvdspreprocess->property_set.src_ids &&
            ...
            nvdspreprocess->property_set.roi_params_src)) {
        printf(
            "ERROR: Some preprocess group config properties not set in sgie preprocess config file\n");
        return FALSE;
      }
    } else {
      if (!(nvdspreprocess->property_set.src_ids &&
...
static gboolean nvdspreprocess_parse_user_configs(
  ...
  for (key = keys; *key; key++) {
    ...
    GST_DEBUG_OBJECT(
        nvdspreprocess,
        "parsed user-config key = %s value = %s\n",
        *key,
        val.c_str());
    CHECK_ERROR(error, group);
...
gboolean nvdspreprocess_parse_config_file(
  ...
  if (!NVDSPREPROCESS_CFG_PARSER_CAT) {
    ...
    GST_DEBUG_CATEGORY_INIT(
        NVDSPREPROCESS_CFG_PARSER_CAT, "nvdspreprocess", 0, NULL);
    level = gst_debug_category_get_threshold(NVDSPREPROCESS_CFG_PARSER_CAT);
    if (level < GST_LEVEL_ERROR)
      gst_debug_category_set_threshold(
          NVDSPREPROCESS_CFG_PARSER_CAT, GST_LEVEL_ERROR);
  }
  ...
  for (group = groups; *group; group++) {
    ...
    if (!strcmp(*group, NVDSPREPROCESS_PROPERTY)) {
      ret = nvdspreprocess_parse_property_group(
          nvdspreprocess, cfg_file_path, cfg_file, *group);
      if (!ret) {
      ...
      ret = nvdspreprocess_parse_common_group(
          nvdspreprocess, cfg_file_path, cfg_file, *group, group_index);
      if (!ret) {
        g_print("NVDSPREPROCESS_CFG_PARSER: Group '%s' parse failed\n", *group);
        goto done;
      }
      ...
      ret = nvdspreprocess_parse_user_configs(
          nvdspreprocess, cfg_file_path, cfg_file, *group);
      if (!ret) {
        g_print("NVDSPREPROCESS_CFG_PARSER: Group '%s' parse failed\n", *group);
        goto done;
  ...
  GST_DEBUG_OBJECT(
      nvdspreprocess,
      "network-input-shape[0] = %d, sum-total-rois=%d\n",
      nvdspreprocess->tensor_params.network_input_shape[0],
      sum_total_rois);
