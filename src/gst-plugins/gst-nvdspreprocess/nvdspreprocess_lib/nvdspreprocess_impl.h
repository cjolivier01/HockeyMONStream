...
typedef struct {
  ...
  /** Holds the per-channel offsets for mean subtraction. This is
         an alternative to the mean image file. The number of offsets in the
     array must be equal to the number of input channels. */
...
/**
 * Provides pre-processing functionality like mean subtraction and
 * normalization.
 */
class NvDsPreProcessTensorImpl {
 public:
  ...
  NvDsPreProcessStatus prepare_tensor(
 ...
 private:
  ...
  NvDsPreProcessNetworkInputOrder m_InputOrder =
