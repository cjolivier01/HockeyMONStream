...
NvDsPreProcessStatus CustomTensorPreparation(
  ...
  /** Prepare Tensor */
  status =
      ctx->tensor_impl->prepare_tensor(batch, tensorParam, buf->memory_ptr);
...
CustomCtx* initLib(CustomInitParams initparams) {
  ...
  ctx->custom_mean_norm_params.pixel_normalization_factor =
      std::stof(initparams.user_configs
  ...
  if (!initparams.user_configs[NVDSPREPROCESS_USER_CONFIGS_MEAN_FILE].empty()) {
    ...
    if (!get_absolute_file_path(
            ...
            initparams.user_configs[NVDSPREPROCESS_USER_CONFIGS_MEAN_FILE]
                .c_str(),
            abs_path)) {
  ...
  std::string offsets_str =
      initparams.user_configs[NVDSPREPROCESS_USER_CONFIGS_OFFSETS];
  ...
  if (!offsets_str.empty()) {
    ...
    printf(
        ...
        ctx->custom_mean_norm_params.offsets[1],
        ctx->custom_mean_norm_params.offsets[2]);
  }

  status = normalization_mean_subtraction_impl_initialize(
      &ctx->custom_mean_norm_params,
      &initparams.tensor_params,
      ctx->tensor_impl,
