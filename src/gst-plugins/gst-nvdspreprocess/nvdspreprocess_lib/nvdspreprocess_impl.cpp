...
CudaStream::CudaStream(uint flag, int priority) {
  ...
  if (err != cudaSuccess) {
    printf(
        "cudaStreamCreateWithPriority failed with err %d : %s\n",
        (int)err,
...
CudaStream::~CudaStream() {
  if (m_Stream != nullptr) {
    ...
    if (err != cudaSuccess) {
      printf(
          "cudaStreamDestroy failed with err %d : %s\n",
          (int)err,
          cudaGetErrorName(err));
    }
  }
...
CudaDeviceBuffer::CudaDeviceBuffer(size_t size) : CudaBuffer(size) {
  ...
  if (err != cudaSuccess) {
    printf(
        "cudaMalloc failed with err %d : %s\n",
        (int)err,
        cudaGetErrorName(err));
  }
...
NvDsPreProcessTensorImpl::NvDsPreProcessTensorImpl(
    ...
    int id)
    : m_UniqueID(id), m_NetworkSize(size), m_NetworkInputFormat(format) {}
...
bool NvDsPreProcessTensorImpl::setScaleOffsets(
  ...
  if (!offsets.empty()) {
    m_ChannelMeans.assign(
        offsets.begin(), offsets.begin() + m_NetworkSize.channels);
  }
  return true;
}
...
bool NvDsPreProcessTensorImpl::setInputOrder(
    const NvDsPreProcessNetworkInputOrder order) {
  m_InputOrder = order;
  return true;
}
...
NvDsPreProcessStatus NvDsPreProcessTensorImpl::readMeanImageFile() {
  ...
  if (cudaReturn != cudaSuccess) {
    printf(
        "Failed to copy mean data to mean data buffer (%s)\n",
...
NvDsPreProcessStatus NvDsPreProcessTensorImpl::allocateResource() {
  if (!m_MeanFile.empty() || m_ChannelMeans.size() > 0) {
    ...
    m_MeanDataBuffer = std::make_unique<CudaDeviceBuffer>(
        (size_t)m_NetworkSize.width * m_NetworkSize.height *
        m_NetworkSize.channels * sizeof(float));
  ...
  /* Create the mean data buffer from per-channel offsets. */
    ...
    if (cudaReturn != cudaSuccess) {
      printf(
          "Failed to copy mean data to mean data cuda buffer(%s)",
          cudaGetErrorName(cudaReturn));
      return NVDSPREPROCESS_CUDA_ERROR;
    }
  }
...
NvDsPreProcessStatus NvDsPreProcessTensorImpl::prepare_tensor(
  ...
  for (unsigned int i = 0; i < batch_size; i++) {
    void* outPtr = (void*)((uint8_t*)devBuf +
                           i * m_NetworkSize.channels * m_NetworkSize.width *
                               m_NetworkSize.height *
    ...
    std::ofstream outfile1(
        "impl_in_batch_" + std::to_string(batch_num1) + ".bin");
    for (unsigned int j = 0; j < m_NetworkSize.height; j++) {
      outfile1.write(
          (char*)batch->units[i].converted_frame_ptr + j * batch->pitch,
    ...
    if (convertFcn || convertFcnHalf) {
      ...
      if (tensorParam.params.data_type == NvDsDataType_FP16) {
        convertFcnHalf(
            (half*)outPtr,
            (unsigned char*)batch->units[i].converted_frame_ptr,
            m_NetworkSize.width,
            m_NetworkSize.height,
            batch->pitch,
            m_Scale,
    ...
    std::ofstream outfile2(
        "impl_out_batch_" + std::to_string(batch_num2) + ".bin");
    outfile2.write(
        (char*)outPtr,
        4 * m_NetworkSize.channels * m_NetworkSize.width *
            m_NetworkSize.height);
    outfile2.close();
    batch_num2++;
#endif
...
extern "C" NvDsPreProcessStatus normalization_mean_subtraction_impl_initialize(
    ...
    int unique_id) {
  if (tensor_params->network_input_order ==
    ...
    printf(
        "network-input-order = %d not supported\n",
        tensor_params->network_input_order);
  }
  ...
  if (!custom_params->meanImageFilePath.empty() &&
    ...
    printf(
        "Cannot access mean image file %s\n",
