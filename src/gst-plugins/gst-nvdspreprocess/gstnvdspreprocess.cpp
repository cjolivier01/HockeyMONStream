...
#define DEFAULT_SCALING_BUF_POOL_SIZE \
  6 /** Inter Buffer Pool Size for Scale & Converted ROIs */
...
static void gst_nvdspreprocess_set_property(
    GObject* object,
    guint prop_id,
    const GValue* value,
    GParamSpec* pspec);
static void gst_nvdspreprocess_get_property(
    GObject* object,
    guint prop_id,
    GValue* value,
    GParamSpec* pspec);

static gboolean gst_nvdspreprocess_set_caps(
...
static GstFlowReturn gst_nvdspreprocess_submit_input_buffer(
    GstBaseTransform* btrans,
    gboolean discont,
    GstBuffer* inbuf);
static GstFlowReturn gst_nvdspreprocess_generate_output(
    GstBaseTransform* btrans,
    GstBuffer** outbuf);
...
static gboolean gst_nvdspreprocess_src_query(
    GstPad* pad,
    GstObject* parent,
    GstQuery* query);
 ...
 * element.
 */
static void gst_nvdspreprocess_class_init(GstNvDsPreProcessClass* klass) {
  ...
  gobject_class->set_property =
      GST_DEBUG_FUNCPTR(gst_nvdspreprocess_set_property);
  gobject_class->get_property =
      GST_DEBUG_FUNCPTR(gst_nvdspreprocess_get_property);

  gstbasetransform_class->set_caps =
      GST_DEBUG_FUNCPTR(gst_nvdspreprocess_set_caps);
...
static void gst_nvdspreprocess_init(GstNvDsPreProcess* nvdspreprocess) {
  ...
  nvdspreprocess->transform_config_params.compute_mode =
      NvBufSurfTransformCompute_Default;
  nvdspreprocess->transform_params.transform_filter =
      NvBufSurfTransformInter_Default;

  gst_pad_set_query_function(
      GST_BASE_TRANSFORM_SRC_PAD(nvdspreprocess),
...
static void gst_nvdspreprocess_finalize(GObject* object) {
  ...
  if (nvdspreprocess->custom_lib_path) {
    ...
    if (nvdspreprocess->custom_lib_handle) {
      deInitLib = dlsym_ptr<void(void*)>(
          nvdspreprocess->custom_lib_handle, "deInitLib");
    ...
    case PROP_CONFIG_FILE: {
      ...
      nvdspreprocess->config_file_parse_successful =
          nvdspreprocess_parse_config_file(
              nvdspreprocess, nvdspreprocess->config_file_path);
...
static gboolean gst_nvdspreprocess_start(GstBaseTransform* btrans) {
  ...
  if (!nvdspreprocess->config_file_path ||
      strlen(nvdspreprocess->config_file_path) == 0) {
    GST_ELEMENT_ERROR(
        nvdspreprocess,
        LIBRARY,
        SETTINGS,
  ...
  nvtx_str =
      "GstNvDsPreProcess: UID=" + std::to_string(nvdspreprocess->unique_id);
  auto nvtx_deleter = [](nvtxDomainHandle_t d) { nvtxDomainDestroy(d); };
  std::unique_ptr<nvtxDomainRegistration, decltype(nvtx_deleter)>
      nvtx_domain_ptr(nvtxDomainCreate(nvtx_str.c_str()), nvtx_deleter);

  nvdspreprocess->custom_initparams.tensor_params =
      nvdspreprocess->tensor_params;
  nvdspreprocess->custom_initparams.unique_id = nvdspreprocess->unique_id;
  nvdspreprocess->custom_initparams.config_file_path =
  ...
  if (nvdspreprocess->custom_lib_path) {
    ...
    nvdspreprocess->custom_lib_handle =
        dlopen(nvdspreprocess->custom_lib_path, RTLD_NOW);
    std::function<CustomCtx*(CustomInitParams)> initLib;

    if (nvdspreprocess->custom_lib_handle) {
      GST_DEBUG_OBJECT(nvdspreprocess, "Custom Library Opened Successfully\n");
      initLib = dlsym_ptr<CustomCtx*(CustomInitParams)>(
          nvdspreprocess->custom_lib_handle, "initLib");
      nvdspreprocess->custom_lib_ctx =
          initLib(nvdspreprocess->custom_initparams);
      ...
      for (guint gcnt = 0; gcnt < nvdspreprocess->nvdspreprocess_groups.size();
           gcnt++) {
        if (!nvdspreprocess->nvdspreprocess_groups[gcnt]
          ...
          nvdspreprocess->nvdspreprocess_groups[gcnt]->custom_transform =
              dlsym_ptr<NvDsPreProcessStatus(
                  NvBufSurface*, NvBufSurface*, CustomTransformParams&)>(
                  nvdspreprocess->custom_lib_handle,
          ...
          if (!nvdspreprocess->nvdspreprocess_groups[gcnt]->custom_transform) {
            GST_ELEMENT_ERROR(
                ...
                STREAM,
                FAILED,
                ("Custom Transformation function not found"),
                (NULL));
            return FALSE;
          }
        }
  ...
  cudaReturn = cudaStreamCreateWithFlags(
      &nvdspreprocess->convert_stream, cudaStreamNonBlocking);
  if (cudaReturn != cudaSuccess) {
    GST_ELEMENT_ERROR(
        nvdspreprocess,
        RESOURCE,
  ...
  nvdspreprocess->transform_config_params.cuda_stream =
  ...
  nvdspreprocess->transform_params.dst_rect =
      new NvBufSurfTransformRect[nvdspreprocess->max_batch_size];
  nvdspreprocess->transform_params.transform_flag = NVBUFSURF_TRANSFORM_FILTER |
   ...
   * nvdspreprocess->scaling_buf_pool_size buffers at start and keeps reusing
   * them. */
  nvdspreprocess->scaling_pool = gst_buffer_pool_new();
  scaling_pool_config =
      gst_buffer_pool_get_config(nvdspreprocess->scaling_pool);
  gst_buffer_pool_config_set_params(
      scaling_pool_config,
      nullptr,
      sizeof(GstNvDsPreProcessMemory),
      nvdspreprocess->scaling_buf_pool_size,
      nvdspreprocess->scaling_buf_pool_size);

  /* Based on the network input requirements decide the buffer pool color
   * format. */
  switch (nvdspreprocess->tensor_params.network_color_format) {
    ...
    case NvDsPreProcessFormat_GRAY:
      if (nvdspreprocess->transform_config_params.compute_mode ==
  ...
  GST_DEBUG_OBJECT(
      nvdspreprocess,
      "network-input-order = %d\n",
      nvdspreprocess->custom_initparams.tensor_params.network_input_order);

  if (nvdspreprocess->tensor_params.network_input_order ==
      NvDsPreProcessNetworkInputOrder_kNCHW) {
    if (nvdspreprocess->processing_height !=
    ...
    g_print(
        "Using user provided processing height = %d and processing width = %d\n",
        nvdspreprocess->processing_height,
        nvdspreprocess->processing_width);
  } else {
    GST_ELEMENT_ERROR(
  ...
  /* Create a new GstNvDsPreProcess Allocator instance. Allocator has methods to
   * allocate and free custom memories. */
  ...
  GST_DEBUG_OBJECT(
      nvdspreprocess,
      "Scaling pool batch-size = %d\n",
      nvdspreprocess->max_batch_size);

  scaling_pool_allocator = gst_nvdspreprocess_allocator_new(
      &allocator_info, 1, nvdspreprocess->gpu_id, FALSE);

  memset(&allocation_params, 0, sizeof(allocation_params));

  gst_buffer_pool_config_set_allocator(
      scaling_pool_config, scaling_pool_allocator, &allocation_params);

  if (!gst_buffer_pool_set_config(
          nvdspreprocess->scaling_pool, scaling_pool_config)) {
    GST_ELEMENT_ERROR(
        nvdspreprocess,
        RESOURCE,
        FAILED,
        ("Failed to set config on buffer scaling_pool"),
        (nullptr));
    goto error;
  }
  ...
  if (!gst_buffer_pool_set_active(nvdspreprocess->scaling_pool, TRUE)) {
    GST_ELEMENT_ERROR(
        ...
        RESOURCE,
        FAILED,
        ("Failed to set buffer pool to active"),
        (nullptr));
    goto error;
  }
    ...
    default:
      GST_ELEMENT_ERROR(
          ...
          ("Tensor data type : %d is not Supported\n",
           (int)nvdspreprocess->tensor_params.data_type),
          (nullptr));
      goto error;
  }
  ...
  GST_DEBUG_OBJECT(
      nvdspreprocess,
      "Tensor Buffer Pool size = %ld data-type=%d\n",
      nvdspreprocess->tensor_params.buffer_size,
      nvdspreprocess->tensor_params.data_type);
#ifdef DEBUG_TENSOR
  tensor_pool_allocator = gst_nvdspreprocess_allocator_new(
      NULL,
      nvdspreprocess->tensor_params.buffer_size,
...
#endif
  memset(
      &tensor_pool_allocation_params, 0, sizeof(tensor_pool_allocation_params));

  gst_buffer_pool_config_set_allocator(
      tensor_pool_config,
      tensor_pool_allocator,
      &tensor_pool_allocation_params);

  if (!gst_buffer_pool_set_config(
          nvdspreprocess->tensor_pool, tensor_pool_config)) {
    GST_ELEMENT_ERROR(
        nvdspreprocess,
        RESOURCE,
        FAILED,
        ("Failed to set config on tensor buffer pool"),
        (nullptr));
    goto error;
  }
  ...
  if (!gst_buffer_pool_set_active(nvdspreprocess->tensor_pool, TRUE)) {
    GST_ELEMENT_ERROR(
        ...
        RESOURCE,
        FAILED,
        ("Failed to set tensor buffer pool to active"),
        (nullptr));
    goto error;
  }

  /** class for acquiring/releasing buffer from tensor pool */
  nvdspreprocess->acquire_impl =
      std::make_unique<NvDsPreProcessAcquirerImpl>(nvdspreprocess->tensor_pool);
  ...
  nvdspreprocess->output_thread = g_thread_new(
      "nvdspreprocess-process-thread",
...
static gboolean gst_nvdspreprocess_stop(GstBaseTransform* btrans) {
  ...
  while (!g_queue_is_empty(nvdspreprocess->preprocess_queue)) {
    g_cond_wait(
        &nvdspreprocess->preprocess_cond, &nvdspreprocess->preprocess_lock);
  }
...
static gboolean gst_nvdspreprocess_set_caps(
  ...
  CHECK_CUDA_STATUS(
      cudaSetDevice(nvdspreprocess->gpu_id), "Unable to set cuda device");
...
static GstFlowReturn scale_and_fill_data(
  ...
  if (nvdspreprocess->maintain_aspect_ratio) {
    ...
    if (!nvdspreprocess->symmetric_padding) {
      ...
      cudaReturn = cudaMemset2DAsync(
          ...
          dest_frame->planeParams.pitch[0],
          0,
          pixel_size * offset_right,
          dest_frame->height,
          nvdspreprocess->convert_stream);
      if (cudaReturn != cudaSuccess) {
        GST_ERROR_OBJECT(
...
#ifdef __aarch64__
  if (nvdspreprocess->scaling_pool_compute_hw !=
  ...
  GST_DEBUG_OBJECT(
      nvdspreprocess, "Fill the input buffer for batch transformation\n");

  /* Create temporary src and dest surfaces for NvBufSurfTransform API. */
  nvdspreprocess->batch_insurf
      .surfaceList[nvdspreprocess->batch_insurf.numFilled] = *src_frame;

  nvdspreprocess->batch_outsurf
      .surfaceList[nvdspreprocess->batch_outsurf.numFilled] = *dest_frame;

  /* Set the source ROI. Could be entire frame or an object. */
  nvdspreprocess->transform_params
      .src_rect[nvdspreprocess->batch_insurf.numFilled] = {
  ...
  nvdspreprocess->transform_params
      .dst_rect[nvdspreprocess->batch_outsurf.numFilled] = {
      offset_top, offset_left, dest_width, dest_height};

  nvdspreprocess->batch_insurf.numFilled++;
  nvdspreprocess->batch_outsurf.numFilled++;

  nvdspreprocess->batch_insurf.batchSize =
      nvdspreprocess->batch_insurf.numFilled;
  nvdspreprocess->batch_outsurf.batchSize =
      nvdspreprocess->batch_outsurf.numFilled;
...
static gboolean dump_rois(
  ...
  for (guint i = 0; i < batch->units.size(); i++) {
    ...
    in_mat = cv::Mat(
        ...
        nvdspreprocess->processing_width,
        CV_8UC4,
        outsurf->surfaceList[i].mappedAddr.addr[0],
...
#endif
    cv::imwrite(
        "out_" + std::to_string(cnt) + "__src__" + std::to_string(src_id) +
            "__roi__" + std::to_string(roi_cnt) + ".jpeg",
        out_mat);
    ...
    if (USE_EGLIMAGE) {
      if (NvBufSurfaceMapEglImage(outsurf, 0) != 0) {
        GST_ELEMENT_ERROR(
            ...
            STREAM,
            FAILED,
            ("%s:buffer map eglimage failed", __func__),
            (NULL));
        return FALSE;
      }
...
static gboolean batch_transformation(
  ...
  err = NvBufSurfTransformAsync(
      in_surf, out_surf, &params.transform_params, &params.sync_obj);
...
static gboolean gst_nvdspreprocess_sink_event(
  ...
  if ((GstNvDsCustomEventType)GST_EVENT_TYPE(event) == GST_NVEVENT_ROI_UPDATE) {
    ...
    // if stream exists
    if (nvdspreprocess->src_to_group_map->find(source_id) !=
        nvdspreprocess->src_to_group_map->end()) {
      int group_id = nvdspreprocess->src_to_group_map->at(source_id);

      GstNvDsPreProcessGroup* preprocess_group =
          nvdspreprocess->nvdspreprocess_groups[group_id];
...
static gboolean group_transformation(
    GstNvDsPreProcess* nvdspreprocess,
  ...
  nvtx_str = "convert_buf batch_num=" +
      std::to_string(nvdspreprocess->current_batch_num);
  ...
  if (nvdspreprocess->custom_lib_path && nvdspreprocess->custom_lib_handle &&
    ...
    status = group->custom_transform(
        &nvdspreprocess->batch_insurf, &nvdspreprocess->batch_outsurf, params);
    ...
    ret = TRUE;
    GST_DEBUG_OBJECT(
        nvdspreprocess, "Custom tranformation from library successfull\n");
  } else {
    ret = batch_transformation(
        &nvdspreprocess->batch_insurf, &nvdspreprocess->batch_outsurf, params);
    if (!ret) {
      GST_ELEMENT_ERROR(
          nvdspreprocess,
          STREAM,
          FAILED,
          ("Transformation from plugin failed with error"),
          (NULL));
      return FALSE;
    }
    GST_DEBUG_OBJECT(
        nvdspreprocess, "Custom tranformation from plugin successfull\n");
  }
...
static void release_user_meta_at_batch_level(
    gpointer data,
    gpointer user_data) {
  ...
  if (preprocess_batchmeta->tensor_meta != nullptr) {
    auto private_data_pair =
        (std::pair<GstNvDsPreProcess*, NvDsPreProcessCustomBuf*>*)
  ...
  gst_buffer_unref((GstBuffer*)preprocess_batchmeta
                       ->private_data); // unref conversion pool buffer
...
static void attach_user_meta_at_batch_level(
    GstNvDsPreProcess* nvdspreprocess,
    NvDsPreProcessBatch* batch,
  ...
  if (status == NVDSPREPROCESS_SUCCESS) {
    ...
    preprocess_batchmeta->roi_vector =
        custom_tensor_params.seq_params.roi_vector;
    ...
    preprocess_batchmeta->tensor_meta->private_data =
        new std::pair(nvdspreprocess, nvdspreprocess->tensor_buf);
    ...
    preprocess_batchmeta->tensor_meta->raw_tensor_buffer =
        ((NvDsPreProcessCustomBufImpl*)nvdspreprocess->tensor_buf)
            ->memory->dev_memory_ptr;
    preprocess_batchmeta->tensor_meta->tensor_shape =
        custom_tensor_params.params.network_input_shape;
    ...
    preprocess_batchmeta->tensor_meta->data_type =
        custom_tensor_params.params.data_type;
    preprocess_batchmeta->tensor_meta->tensor_name =
        custom_tensor_params.params.tensor_name;

    GST_DEBUG_OBJECT(
        nvdspreprocess,
        "attached network shape in tensor %d : %d : %d : %d\n",
        preprocess_batchmeta->tensor_meta->tensor_shape[0],
        preprocess_batchmeta->tensor_meta->tensor_shape[1],
        preprocess_batchmeta->tensor_meta->tensor_shape[2],
        preprocess_batchmeta->tensor_meta->tensor_shape[3]);
  } else {
    preprocess_batchmeta->roi_vector.clear();
    for (guint i = 0; i < batch->units.size(); i++) {
...
static GstFlowReturn gst_nvdspreprocess_on_frame(
    GstNvDsPreProcess* nvdspreprocess,
    GstBuffer* inbuf,
    NvBufSurface* in_surf) {
  ...
  for (guint gcnt = 0; gcnt < num_groups; gcnt++) {
    ...
    GST_DEBUG_OBJECT(
        nvdspreprocess,
        "num filled in batch meta = %d\n",
        batch_meta->num_frames_in_batch);
    NvDsMetaList* l_frame = NULL;

    /* Process group only when interval_counter is 0. */
    if (preprocess_group->interval_counter++ %
            (preprocess_group->interval + 1) >
        0) {
      continue;
    }
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
      ...
      if (std::find(src_ids.begin(), src_ids.end(), source_id) ==
              src_ids.end() &&
          src_ids[0] != -1) {
        GST_DEBUG_OBJECT(
            nvdspreprocess,
            "Group %d : No Source %d => skipping\n",
            gcnt,
            source_id);
        continue;
      }
      ...
      GST_DEBUG_OBJECT(
          nvdspreprocess,
          "Group %d : Processsing Source ID = %d \n",
          gcnt,
          source_id);

      g_mutex_lock(&nvdspreprocess->framemeta_map_lock);
      auto get_preprocess_frame_meta =
          preprocess_group->framemeta_map.find(framemeta_map_idx);

      if (get_preprocess_frame_meta == preprocess_group->framemeta_map.end() &&
          src_ids[0] != -1) {
        g_print(
            "Group %d : Configuration for Source ID = %d not found\n",
            gcnt,
            source_id);
        ...
        GST_DEBUG_OBJECT(
            nvdspreprocess,
            "Group %d : Source ID %d : Got roi-vecsize = %ld\n",
            gcnt,
            source_id,
            roi_vector.size());
        ...
        for (guint n = 0; n < preprocess_frame.roi_vector.size(); n++) {
          ...
          if (preprocess_group->process_on_roi) {
            GST_DEBUG_OBJECT(
                nvdspreprocess,
                "Group %d : Source ID %d : Processing on ROIS\n",
                gcnt,
                source_id);

            /* Clipping the excess width of roi's */
            if (roi_meta.roi.left + roi_meta.roi.width >
                (in_surf->surfaceList + batch_index)->width) {
              roi_meta.roi.width = (in_surf->surfaceList + batch_index)->width -
                  roi_meta.roi.left;
            }

            /* Clipping the excess height of roi's */
            if (roi_meta.roi.top + roi_meta.roi.height >
                (in_surf->surfaceList + batch_index)->height) {
              roi_meta.roi.height =
                  (in_surf->surfaceList + batch_index)->height -
                  roi_meta.roi.top;
            }
            ...
            GST_DEBUG_OBJECT(
                nvdspreprocess,
                "filling ROI left=%f top=%f width=%f height=%f\n",
                rect_params.left,
                rect_params.top,
                rect_params.width,
                rect_params.height);
            ...
            GST_DEBUG_OBJECT(
                nvdspreprocess,
                "Group %d : Source ID %d : Processing on Full Frames\n",
                gcnt,
                source_id);
            ...
            GST_DEBUG_OBJECT(
                nvdspreprocess,
                "filling FULL FRAME left=%f top=%f width=%f height=%f\n",
                rect_params.left,
                rect_params.top,
                rect_params.width,
                rect_params.height);
          ...
          if (batch == nullptr) {
            ...
            /* acquiing the conv_gst_buf buffer from scaling_pool which store
             * the transformed output buffer */
            flow_ret = gst_buffer_pool_acquire_buffer(
                nvdspreprocess->scaling_pool, &conv_gst_buf, nullptr);
          ...
          /** Scale the roi part to the network resolution maintaining aspect
           * ratio */
          ...
          roi_meta.converted_buffer =
              (NvBufSurfaceParams*)memory->surf->surfaceList + idx;
          ...
          if (preprocess_group->draw_roi) {
            ...
            display_meta->rect_params[0].border_color =
                preprocess_group->roi_color;
            nvds_add_display_meta_to_frame(frame_meta, display_meta);
          }

          /* Adding a Unit (ROI/Crop/Full Frame) to the current batch. Set the
           * frames members. */
          ...
          /** push the batch to the queue if batch units exceeds nvdspreprocess
           * batch size **/
          if (batch->units.size() == nvdspreprocess->max_batch_size) {
            ...
            for (guint g_count = 0; g_count < num_groups; g_count++) {
              ...
              if (group->custom_transform_function_name.empty() ||
                  !g_strcmp0(
                      group->custom_transform_function_name.c_str(),
            ...
            /* Push the batch info structure in the processing queue and notify
             * the process thread that a new batch has been queued. */
            g_mutex_lock(&nvdspreprocess->preprocess_lock);
            g_queue_push_tail(nvdspreprocess->preprocess_queue, batch.get());
            g_cond_broadcast(&nvdspreprocess->preprocess_cond);
            g_mutex_unlock(&nvdspreprocess->preprocess_lock);

            /* Set batch to nullptr so that a new NvDsPreProcessBatch structure
  ...
  if (batch != nullptr) {
    ...
    /* Push the last batch info structure in the processing queue and notify the
    ...
    /* Batch subm-itted. Set last batch to nullptr so that a new
     * NvDsPreProcessBatch structure can be allocated if required. */
    conv_gst_buf = nullptr;
    batch.release();
  }
  return GST_FLOW_OK;
...
static inline gboolean should_process_object(
    GstNvDsPreProcess* nvdspreprocess,
    GstNvDsPreProcessGroup*& preprocess_group,
  ...
  if (nvdspreprocess->operate_on_gie_id > -1 &&
      object_meta->unique_component_id != nvdspreprocess->operate_on_gie_id) {
    return FALSE;
  }

  /* skip the object if object does not belong to secondary classifier class */
  std::vector<gint> operate_on_class_ids =
      preprocess_group->operate_on_class_ids;
  if (operate_on_class_ids.size() > 0 &&
      (std::find(
           operate_on_class_ids.begin(),
           operate_on_class_ids.end(),
           object_meta->class_id) == operate_on_class_ids.end())) {
    return FALSE;
  }
...
static GstFlowReturn gst_nvdspreprocess_on_objects(
    GstNvDsPreProcess* nvdspreprocess,
    GstBuffer* inbuf,
    NvBufSurface* in_surf) {
  GstFlowReturn flow_ret = GST_FLOW_ERROR;
      ...
      if (get_preprocess_frame_meta == preprocess_group->framemeta_map.end() &&
        ...
        for (guint n = 0; n < preprocess_frame.roi_vector.size(); n++) {
          ...
          for (NvDsMetaList* l_obj = frame_meta->obj_meta_list; l_obj != NULL;
            ...
            /** Scale the object part to the network resolution maintaining
             * aspect ratio */
            if (scale_and_fill_data(
                    nvdspreprocess,
                    in_surf->surfaceList + batch_index,
            ...
            /* Adding a Unit (Full objects/Cropped objects) to the current
             * batch. Set the frames members. */
            NvDsPreProcessUnit unit;
            unit.converted_frame_ptr = memory->frame_memory_ptrs[idx];
            unit.obj_meta = nullptr;
            unit.frame_meta = frame_meta;
            ...
            /** push the batch to the queue if batch units exceeds
             * nvdspreprocess batch size **/
            if (batch->units.size() == nvdspreprocess->max_batch_size) {
              /** transform the group according to num filled from batch_meta */
              if (!group_transformation(nvdspreprocess, preprocess_group)) {
                GST_ELEMENT_ERROR(
              ...
              /* Push the batch info structure in the processing queue and
               * notify the process thread that a new batch has been queued. */
              g_mutex_lock(&nvdspreprocess->preprocess_lock);
              g_queue_push_tail(nvdspreprocess->preprocess_queue, batch.get());
              g_cond_broadcast(&nvdspreprocess->preprocess_cond);
              g_mutex_unlock(&nvdspreprocess->preprocess_lock);

              /* Set batch to nullptr so that a new NvDsPreProcessBatch
...
static GstFlowReturn gst_nvdspreprocess_submit_input_buffer(
  ...
  nvtxRangeId_t buf_process_range =
      nvtxDomainRangeStartEx(nvdspreprocess->nvtx_domain, &eventAttrib);
...
NvDsPreProcessCustomBuf* NvDsPreProcessAcquirerImpl::acquire() {
  ...
  return new NvDsPreProcessCustomBufImpl{
      {memory->dev_memory_ptr}, gstbuf, memory};
}
...
static gpointer gst_nvdspreprocess_output_loop(gpointer data) {
  ...
  eventAttrib.messageType = NVTX_MESSAGE_TYPE_ASCII;
  std::string nvtx_str;
  nvtx_str = "gst-nvdspreprocess_output-loop_uid=" +
      std::to_string(nvdspreprocess->unique_id);
  ...
  while (!nvdspreprocess->stop) {
    ...
    batch.reset((NvDsPreProcessBatch*)g_queue_pop_head(
        nvdspreprocess->preprocess_queue));
    g_cond_broadcast(&nvdspreprocess->preprocess_cond);
    ...
    if (batch->push_buffer) {
      nvtxDomainRangeEnd(
          nvdspreprocess->nvtx_domain, batch->nvtx_complete_buf_range);

      nvds_set_output_system_timestamp(
          batch->inbuf, GST_ELEMENT_NAME(nvdspreprocess));

      GstFlowReturn flow_ret = gst_pad_push(
          GST_BASE_TRANSFORM_SRC_PAD(nvdspreprocess), batch->inbuf);
    ...
    nvtx_str = "dequeueOutputAndAttachMeta batch_num=" +
        std::to_string(batch->inbuf_batch_num);
    eventAttrib.message.ascii = nvtx_str.c_str();
    nvtxDomainRangePushEx(nvdspreprocess->nvtx_domain, &eventAttrib);
    ...
    if (nvdspreprocess->custom_lib_path && nvdspreprocess->custom_lib_handle &&
      ...
      custom_tensor_params.params.buffer_size =
          custom_tensor_params.params.buffer_size * batch->units.size() /
          nvdspreprocess->tensor_params.network_input_shape[0];
      ...
      status = nvdspreprocess->custom_tensor_function(
          nvdspreprocess->custom_lib_ctx,
          batch.get(),
          nvdspreprocess->tensor_buf,
          custom_tensor_params,
    ...
    std::ofstream outfile(
        "tensorout_batch_" + std::to_string(batch_num) + ".bin");
    outfile.write(
        (char*)((NvDsPreProcessCustomBufImpl*)nvdspreprocess->tensor_buf)
            ->memory->dev_memory_ptr,
        custom_tensor_params.params.buffer_size);
    outfile.close();
    batch_num++;
    ...
    attach_user_meta_at_batch_level(
        nvdspreprocess, batch.get(), custom_tensor_params, status);
 ...
 */
static gboolean nvdspreprocess_plugin_init(GstPlugin* plugin) {
  GST_DEBUG_CATEGORY_INIT(
      gst_nvdspreprocess_debug, "nvdspreprocess", 0, "preprocess plugin");

  return gst_element_register(
      plugin, "nvdspreprocess", GST_RANK_PRIMARY, GST_TYPE_NVDSPREPROCESS);
}
