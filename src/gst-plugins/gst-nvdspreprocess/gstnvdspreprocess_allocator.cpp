...
#define GST_NVDSPREPROCESS_ALLOCATOR(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST(            \
      (obj), GST_TYPE_NVDSPREPROCESS_ALLOCATOR, GstNvDsPreProcessAllocator))
#define GST_NVDSPREPROCESS_ALLOCATOR_CLASS(klass) \
  (G_TYPE_CHECK_CLASS_CAST(                       \
      (klass),                                    \
      GST_TYPE_NVDSPREPROCESS_ALLOCATOR,          \
      GstNvDsPreProcessAllocatorClass))
...
#define _do_init                          \
  GST_DEBUG_CATEGORY_INIT(                \
      gst_nvdspreprocess_allocator_debug, \
      "nvdspreprocessallocator",          \
      0,                                  \
      "nvdspreprocess allocator");
#define gst_nvdspreprocess_allocator_parent_class parent_class
G_DEFINE_TYPE_WITH_CODE(
    GstNvDsPreProcessAllocator,
    gst_nvdspreprocess_allocator,
    GST_TYPE_ALLOCATOR,
    _do_init);
 ...
 * allocator. */
static GstMemory* gst_nvdspreprocess_allocator_alloc(
    GstAllocator* allocator,
    gsize size,
    GstAllocationParams* params) {
  GstNvDsPreProcessAllocator* preprocess_allocator =
      GST_NVDSPREPROCESS_ALLOCATOR(allocator);
  ...
  if (preprocess_allocator->info == NULL) {
    if (preprocess_allocator->debug_tensor) {
      cudaReturn = cudaMallocHost(
          &tmem->dev_memory_ptr, preprocess_allocator->raw_buf_size);
    } else {
      cudaReturn =
          cudaMalloc(&tmem->dev_memory_ptr, preprocess_allocator->raw_buf_size);
    }
    if (cudaReturn != cudaSuccess) {
      GST_ERROR(
          "failed to allocate cuda malloc for tensor with error %s",
  ...
  if (NvBufSurfaceCreate(
          &tmem->surf,
          preprocess_allocator->info->batch_size,
          &create_params) != 0) {
    GST_ERROR(
        "Error: Could not allocate internal buffer pool for nvdspreprocess");
    delete nvmem;
    return nullptr;
  }
  ...
  if (tmem->surf->memType == NVBUF_MEM_SURFACE_ARRAY) {
    if (NvBufSurfaceMapEglImage(tmem->surf, -1) != 0) {
      GST_ERROR(
          "Error: Could not map EglImage from NvBufSurface for nvdspreprocess");
      delete nvmem;
      return nullptr;
    }
  ...
  tmem->frame_memory_ptrs.assign(
      preprocess_allocator->info->batch_size, nullptr);

  for (guint i = 0; i < preprocess_allocator->info->batch_size; i++) {
#if defined(__aarch64__)
    if (tmem->surf->memType == NVBUF_MEM_SURFACE_ARRAY) {
      if (cuGraphicsEGLRegisterImage(
              &tmem->cuda_resources[i],
      ...
      if (cuGraphicsResourceGetMappedEglFrame(
              &tmem->egl_frames[i], tmem->cuda_resources[i], 0, 0) !=
          CUDA_SUCCESS) {
...
static void gst_nvdspreprocess_allocator_free(
    GstAllocator* allocator,
    GstMemory* memory) {
  GstNvDsPreProcessAllocator* preprocess_allocator =
      GST_NVDSPREPROCESS_ALLOCATOR(allocator);
...
static gpointer gst_nvdspreprocess_memory_map(
    GstMemory* mem,
    gsize maxsize,
    GstMapFlags flags) {
...
static void gst_nvdspreprocess_memory_unmap(GstMemory* mem) {}

/* Standard boiler plate. Assigning implemented function pointers. */
static void gst_nvdspreprocess_allocator_class_init(
    GstNvDsPreProcessAllocatorClass* klass) {
  GstAllocatorClass* allocator_class = GST_ALLOCATOR_CLASS(klass);

  allocator_class->alloc =
      GST_DEBUG_FUNCPTR(gst_nvdspreprocess_allocator_alloc);
 ...
 * the memory type. */
static void gst_nvdspreprocess_allocator_init(
    GstNvDsPreProcessAllocator* allocator) {
...
/* Create a new allocator of type GST_TYPE_NVDSPREPROCESS_ALLOCATOR and
 * initialize members. */
GstAllocator* gst_nvdspreprocess_allocator_new(
    GstNvDsPreProcessVideoBufferAllocatorInfo* info,
    size_t raw_buf_size,
    guint gpu_id,
    gboolean debug_tensor) {
  GstNvDsPreProcessAllocator* allocator =
      (GstNvDsPreProcessAllocator*)g_object_new(
          GST_TYPE_NVDSPREPROCESS_ALLOCATOR, nullptr);
...
GstNvDsPreProcessMemory* gst_nvdspreprocess_buffer_get_memory(
    GstBuffer* buffer) {
  GstMemory* mem;
