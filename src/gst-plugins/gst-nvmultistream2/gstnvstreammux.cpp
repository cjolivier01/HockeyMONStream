...
#define _do_init GST_DEBUG_CATEGORY_INIT(gst_nvstreammux_debug, "nvstreammux", 0, "nvstreammux element");
...
class GstNvStreammuxDebug : public INvStreammuxDebug {
 public:
  ...
  void DebugPrint(const char* format, ...) {
    ...
    gst_debug_log_valist(GST_CAT_DEFAULT, GST_LEVEL_DEBUG, "", "", 0, (GObject*)mux, format, args);
    va_end(args);
  }
...
static gboolean gst_nvstreammux_query_latency_unlocked(GstNvStreamMux* self, GstQuery* query) {
  ...
  if (min > max && GST_CLOCK_TIME_IS_VALID(max)) {
    GST_ELEMENT_WARNING(
        ...
        CLOCK,
        (NULL),
        ("Impossible to configure latency: max %" GST_TIME_FORMAT " < min %" GST_TIME_FORMAT
...
static GstFlowReturn gst_nvstreammux_chain(GstPad* pad, GstObject* parent, GstBuffer* buffer) {
  ...
  if (ntp_calc && !gst_nvds_ntp_calculator_have_ntp_sync_values(ntp_calc)) {
    ...
    gst_nvds_ntp_calculator_add_ntp_sync_values(ntp_calc, ntp_time_epoch_ns, ntp_frame_timestamp, avg_frame_time);
  ...
  LOGD("got a buffer\n");
  GstBufferWrapper* gst_buffer = new GstBufferWrapper(
      buffer,
      ENTRY_BUFFER,
...
static GstCaps* remove_width_height(GstCaps* caps) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (i > 0 &&
        gst_caps_is_subset_structure_full(ret_caps, gst_caps_get_structure(caps, i), gst_caps_get_features(caps, i)))
      continue;

    str = gst_structure_copy(str);
    gst_structure_remove_fields(
        str, "width", "height", "pixel-aspect-ratio", "chroma-site", "interlace-mode", "colorimetry", NULL);
...
static GstCaps* remove_framerate_memory_feautures(GstCaps* caps) {
  ...
  for (i = 0; i < n; i++) {
    ...
    gst_structure_set(str, "framerate", GST_TYPE_FRACTION_RANGE, 0, G_MAXINT, G_MAXINT, 1, NULL);
    gst_caps_append_structure_full(ret_caps, str, NULL);
    str = gst_structure_copy(str);
    gst_caps_append_structure_full(ret_caps, str, gst_caps_features_from_string("memory:NVMM"));
  }
...
static gboolean gst_nvstreammux_src_query(GstPad* pad, GstObject* parent, GstQuery* query) {
  ...
  if (gst_nvquery_is_stream_caps(query)) {
    ...
    GstCaps* sinkCaps = GST_CAPS(g_hash_table_lookup(mux->sink_pad_caps, nStreamId + (char*)NULL));
...
static GstCaps* gst_nvstreammux_remove_memory_feature(GstCaps* ret_caps) {
  ...
  if (!ret_caps || !GST_IS_CAPS(ret_caps) || gst_caps_is_empty(ret_caps) || gst_caps_is_any(ret_caps)) {
    return ret_caps;
  }
...
static GstCaps* gst_nvstreammux_intersect_with_sink_caps(GstPad* pad, GstCaps* ret_caps) {
  ...
  if (!pad || !ret_caps || !GST_IS_CAPS(ret_caps) || gst_caps_is_empty(ret_caps) || gst_caps_is_any(ret_caps)) {
    return nullptr;
  }
  ...
  if (gst_nvquery_is_ntp_sync(query)) {
    ...
    if (ntp_calc)
      gst_nvds_ntp_calculator_add_ntp_sync_values(
          ntp_calc, ntpdata.ntp_time_epoch_ns, ntpdata.frame_timestamp, ntpdata.avg_frame_time);
  ...
  if (GST_QUERY_TYPE(query) == GST_QUERY_ACCEPT_CAPS) {
    ...
    if (gst_nvstreammux_is_video(acaps)) {
      if (mux->width && mux->height) {
        ...
        gst_structure_set(str, "width", G_TYPE_INT, mux->width, "height", G_TYPE_INT, mux->height, NULL);
    ...
    peer_query = gst_query_new_accept_caps(acaps); // gst_caps_from_string("audio/x-raw, layout=interleaved, rate=48000,
  ...
  if (strcmp(mimetype, "audio/x-raw") == 0) {
    ...
    if (!gst_pad_has_current_caps(mux->srcpad)) {
      ...
      if (!mux->pad_task_created) {
        mux->pad_task_created = gst_pad_start_task(mux->srcpad, gst_nvstreammux_src_push_loop, mux, NULL);
      }
    }
  ...
  if (!gst_pad_has_current_caps(mux->srcpad)) {
    if (caps) {
      ...
      gst_structure_set(
          ...
          G_TYPE_INT,
          mux->helper->get_num_surfaces_per_frame(),
          NULL);
      ...
      mux->helper->set_frame_duration(
          1000000000UL * ((double)mux->out_videoinfo.fps_d / (double)mux->out_videoinfo.fps_n));
      event = gst_event_new_caps(caps);
...
static void configure_module(GstNvStreamMux* mux) {
  ...
  if (parsed) {
    ...
    GST_WARNING_OBJECT(
        mux, "No config-file provided; falling back to default streammux config %d\n", mux->num_sink_pads);
  }
...
static gboolean gst_nvstreammux_sink_event(GstPad* pad, GstObject* parent, GstEvent* event) {
  ...
  if (name == NULL) {
    ...
    if ((GstNvEventType)GST_EVENT_TYPE(event) == GST_NVEVENT_STREAM_EOS) {
      GST_DEBUG_OBJECT(
          mux, "Received NVEVENT_EOS for stream %d [%s]\n", stream_index, mux->isAudio ? "audiomux" : "videomux");
      entry = new EventWrapper((void*)ev, ENTRY_EVENT, BATCH_SEQUENCE_POST_BATCH);
...
static GstPad* gst_nvstreammux_request_new_pad(
    GstElement* element,
  ...
  if ((wrapper_pad = (mux->helper->get_pad(stream_index))) != NULL) {
    ...
    sinkpad = GST_PAD_CAST(
        g_object_new(GST_TYPE_NVSTREAM_PAD, "name", name, "direction", templ->direction, "template", templ, NULL));
...
static void gst_nvstreammux_src_push_loop(gpointer user_data) {
  ...
  if (iter == NULL || mux->eos_sent) {
    ...
    GST_DEBUG_OBJECT(
        mux, "No Sources found at the input of muxer [%s:%d]", mux->isAudio ? "audiomux" : "videomux", __LINE__);
  ...
  if (mux->sync_inputs) {
    ...
    LOGD(
        "latency_up=%lu has_peer_latency=%d peer_latency_min=%lu max=%lu\n",
        latency_up,
        mux->has_peer_latency,
        mux->peer_latency_min,
        mux->peer_latency_max);
  ...
  if (mux->isAudio) {
    ...
    meta = gst_buffer_add_nvds_meta(
        (GstBuffer*)(out_buf->gst_buffer), batch_meta, NULL, nvds_batch_meta_copy_func, nvds_batch_meta_release_func);
    if (meta == NULL) {
      g_print("line = %d file = %s func = %s\n", __LINE__, __FILE__, __func__);
      exit(-1);
    }
    ...
    LOGD("out_buf=%p batch_meta=%p name=%s %p\n", out_buf, batch_meta, GST_ELEMENT_NAME(mux), batch_meta->meta_mutex.p);
...
static GstStateChangeReturn gst_nvstreammux_change_state(GstElement* element, GstStateChange transition) {
  GstNvStreamMux* mux = GST_NVSTREAMMUX(element);
...
static void gst_nvstreammux_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec) {
  g_print("***********************\ngst_nvstreammux_set_property\n***********************\n");
  ...
  switch (prop_id) {
    ...
    case PROP_ATTACH_SYS_TIME_STAMP:
      ...
      mux->ntp_calc_mode = mux->sys_ts ? GST_NVDS_NTP_CALC_MODE_SYSTEM_TIME : GST_NVDS_NTP_CALC_MODE_RTCP;
      break;
...
static void gst_nvstreammux_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec) {
  GstNvStreamMux* mux = GST_NVSTREAMMUX(object);
...
static void gst_nvstreammux_2_class_init(GstNvStreamMuxClass* klass) {
  ...
  gst_element_class_set_static_metadata(
      gstelement_class,
      "Stream multiplexer 2",
      "Generic",
      "N-to-1 pipe stream multiplexing",
  ...
  nvstreammux_signals[SIGNAL_BUFFER_DROPPED] = g_signal_new(
      "dropped",
