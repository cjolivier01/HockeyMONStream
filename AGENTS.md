...
- `video_out.program_4k_interpolation` selects the GPU resize filter for `ENCODE_PROGRAM_4K_FILE` sinks; the bundled `default` preserves NVIDIA's pre-cubic behavior (nearest on CUDA). Structural overlays may set the corresponding sink-local `interpolation-method` directly, with normal layer precedence.
