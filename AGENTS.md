...
- `src/apps/hstream-ui/StitchingExperimentDialog.*` and `StitchingExperimentBackend.*` own stitching candidate matrices. Each candidate calibrates in an isolated game, replays through the calibration-only GPU stitched preview, and can transactionally promote its existing maps/seam into the selected game without recalibration. `StitchingExperimentStore.*` retains owned experiment history and frame-count selections under the game’s `stitching-experiments/` directory; the main calibration retains its own inputs independently. See `docs/stitching-experiments.md` and `docs/persistent-stitching-frame-cache-design.md`. Optional player-rich frame selection first prepares an ordinary baseline and its rink mask, then runs a separate sampled `hmstitcher -> primary-gie -> ds-fieldmask -> FAKE` analysis; `PlayerFrameScan.*` owns metadata gating/scoring and scan progress from completed inference, and `PlayerFrameSelection.*`/`PlayerFrameOverlap.*` own selection and overlap. The same Program mask-pruning algorithm is mandatory; temporal tracks are not required. The UI can inspect exact chosen pairs, bounded calibration stills and coverage before comparing candidates.
...
- `src/apps/hstream-cli/TensorRtModelCache.cpp` prepares missing YAML ONNX inference engines in a shared per-user cache, including writable downloaded/development models. It stages the ONNX and matches DeepStream's derived engine filename so subsequent CLI/UI/job runs reuse it across games. `OnnxExternalData.cpp` inspects model metadata so external tensor files are also staged and hashed; writable model aliases resolve to regular cached files. The default is `${XDG_CACHE_HOME:-$HOME/.cache}/hstream/tensorrt`, overridable with `HSTREAM_TENSORRT_CACHE_DIR`; existing explicitly configured engines remain usable. See `docs/customModels.md` for cache behavior.
